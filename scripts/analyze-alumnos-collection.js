...
async function analyzeAlumnosCollection() {
  ...
  try {
    ...
    Array.from(allFields).sort().forEach(field => {
      ...
      console.log(`   Ejemplos: ${samples.map(v => 
    ...
    if (instrumentos.size > 0) {
      ...
      console.log('=' * 40);
      Array.from(instrumentos).sort().forEach(instrumento => {
    ...
    console.log('=' * 40);
    console.log(`📞 Con teléfono: ${withPhone}/${snapshot.size} (${(withPhone/snapshot.size*100).toFixed(1)}%)`);
    console.log(`🎂 Con edad: ${withAge}/${snapshot.size} (${(withAge/snapshot.size*100).toFixed(1)}%)`);
    console.log(`🎵 Con instrumento: ${withInstrument}/${snapshot.size} (${(withInstrument/snapshot.size*100).toFixed(1)}%)`);
    console.log(`👥 Con grupos: ${withGroups}/${snapshot.size} (${(withGroups/snapshot.size*100).toFixed(1)}%)`);
    console.log(`📅 Con fecha nacimiento: ${withBirthDate}/${snapshot.size} (${(withBirthDate/snapshot.size*100).toFixed(1)}%)`);
