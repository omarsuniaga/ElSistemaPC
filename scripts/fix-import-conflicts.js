...
const CONFIG = {
  ...
  directories: ["src/**/*.ts", "src/**/*.vue", "src/**/*.js"],

  // Módulos que deben ser siempre estáticos (críticos)
  alwaysStatic: ["vue", "vue-router", "pinia", "@heroicons/vue", "firebase/app", "firebase/auth"],

  // Módulos que pueden ser dinámicos (funcionalidades opcionales)
  canBeDynamic: ["firebase/firestore", "jspdf", "chart.js", "date-fns"],

  // Patrones de archivos que deben usar imports dinámicos
  dynamicFiles: ["/store/", "/services/", "/composables/"],
...
class ImportResolver {
  ...
  convertToStatic(content, moduleName) {
    ...
    const dynamicRegex = new RegExp(
      `import\\s*\\(\\s*['"\`]${moduleName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}['"\`]\\s*\\)`,
      "g"
    )
  ...
  convertToDynamic(content, moduleName, filePath) {
    const staticRegex = new RegExp(
      `import\\s+(?:(?:\\{[^}]*\\}|\\*\\s+as\\s+\\w+|\\w+)\\s+from\\s+)?['"\`]${moduleName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}['"\`]`,
      "g"
    )
