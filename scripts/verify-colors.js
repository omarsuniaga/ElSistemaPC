    ...
    PROBLEMATIC_CLASSES.forEach(pattern => {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        issues.push({
          ...
          lineNumber,
          line,
          recommendation: RECOMMENDED_REPLACEMENTS[match[0]] || 'Use appropriate Tailwind dark mode classes'
        });
      }
    });
  ...
  try {
    ...
    colorClasses.forEach(pattern => {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        if (!line.includes('dark:') && !match[1].includes('white') && !match[1].includes('transparent')) {
          issues.push({
            class: match[1],
            lineNumber,
            line,
...
async function main() {
  ...
  if (totalIssues === 0) {
    console.log('✅ ¡Perfecto! Todos los archivos usan clases de color correctas para modo dark/light');
    return;
  }
  ...
  Object.entries(fileIssues).forEach(([filePath, issues]) => {
    ...
    issues.forEach(issue => {
      if (issue.type === 'missing-dark-variant') {
        console.log(`  ⚡ Línea ${issue.lineNumber}: Posible falta de variante dark para "${issue.class}"`);
  ...
  console.log('2. Asegúrate de que todas las clases de color tengan variantes dark: cuando sea necesario');
...
export {
  findVueFiles,
  checkFileForProblematicClasses,
  checkForMissingDarkModeClasses
};
