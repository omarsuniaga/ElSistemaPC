...
class AdvancedImportResolver {
  ...
  findConflictsInFile(imports, filePath) {
    ...
    const conflictingModules = [...staticModules].filter(module => 
      dynamicModules.has(module)
    );
  ...
  showDetailedSummary() {
    ...
    this.conflicts.forEach((data, filePath) => {
      ...
      data.conflicts.forEach((conflict, index) => {
        ...
        console.log(`     📍 Estático: líneas ${conflict.currentStatic.map(i => i.line).join(', ')}`);
        console.log(`     📍 Dinámico: líneas ${conflict.currentDynamic.map(i => i.line).join(', ')}`);
  ...
  groupFixesByType() {
    ...
    this.fixes.forEach(fix => {
      const type = fix.type === 'convert-to-static' ? 'Convertir a estático' : 'Convertir a dinámico';
      if (!grouped[type]) {
        grouped[type] = [];
      }
  ...
  async applyFixesToFile(filePath, fixes) {
    try {
      ...
      const fixSummary = fixes.map(fix => 
        `// TODO: ${fix.type} para ${fix.module} - ${fix.reason}`
      ).join('\n');
