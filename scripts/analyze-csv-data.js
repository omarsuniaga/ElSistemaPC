...
function findNameSimilarity(csvName, firestoreName) {
  const csvWords = csvName.toLowerCase().split(' ').filter(w => w.length > 2);
  const firestoreWords = firestoreName.toLowerCase().split(' ').filter(w => w.length > 2);
...
async function analyzeCSVData() {
  ...
  try {    // Leer CSV
    ...
    analysis.partialMatches.forEach(match => {
      console.log(`  📝 "${match.csv.nombre}" -> "${match.firestore.nombre}" (${(match.similarity * 100).toFixed(1)}%)`);
    });
