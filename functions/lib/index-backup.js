...
const useFirestoreAuthState = async (sessionId) => {
    ...
    return {
        ...
        saveCreds: async () => {
            await writeData({ creds: creds, keys: keys });
        },
    };
};
let sock = null;
...
const startWhatsappClient = async (sessionId) => {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            functions.logger.info('Connection closed due to ', lastDisconnect === null || lastDisconnect === void 0 ? void 0 : lastDisconnect.error, ', reconnecting ', shouldReconnect);
    ...
    try {
        ...
        if (req.path === '/whatsappApi/qr' && req.method === 'GET') {
            if (qrCodeData) {
                ...
                res.status(200).send({ status: 'connected', message: 'WhatsApp client is already connected.' });
            }
            else if (connectionStatus === 'logged_out') {
                res.status(200).send({ status: 'logged_out', message: 'WhatsApp client logged out. Please request QR again to re-authenticate.' });
            }
            else {
                res.status(202).send({ status: 'waiting_for_qr', message: 'Waiting for QR code to be generated. Please try again shortly.' });
            }
        }
        else if (req.path === '/whatsappApi/send-message' && req.method === 'POST') {
            if (connectionStatus !== 'connected') {
                res.status(400).send({ status: connectionStatus, message: 'WhatsApp client not connected. Cannot send message.' });
                return;
            }
