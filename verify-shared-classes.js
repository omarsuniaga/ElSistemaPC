...
window.verifySharedClassesModule = () => {
  ...
  const classesTab = document.querySelector('[data-testid="classes-tab"]') || 
                     document.querySelector('button[contains="Clases"i]');
  const sharedTab = document.querySelector('[data-testid="shared-classes-tab"]') || 
      ...
      if (classesData) {
        ...
        classes.forEach((cls, i) => {
          if (i < 3) { // Solo mostrar las primeras 3
            ...
            console.log(`      teachers: ${cls.teachers ? JSON.stringify(cls.teachers) : 'undefined'}`);
          }
        });
      }
    }
  ...
  if (sharedTab) {
    ...
    try {
      sharedTab.click();
      setTimeout(() => {
        const sharedClassesList = document.querySelector('[data-testid="shared-classes-list"]') ||
                                  document.querySelector('.shared-classes-container');
        console.log(`   Lista compartidas: ${sharedClassesList ? '✅ Visible' : '❌ No visible'}`);
        
        // Buscar elementos específicos
        const sharedItems = document.querySelectorAll('.shared-class-item, [data-testid="shared-class-item"]');
        console.log(`   Items compartidos: ${sharedItems.length}`);
        
        if (sharedItems.length === 0) {
          console.log('💡 SUGERENCIAS:');
          console.log('   1. Verifica que las clases tengan la propiedad "teachers" con array de IDs');
