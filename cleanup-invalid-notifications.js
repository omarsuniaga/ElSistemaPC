...
async function cleanupInvalidNotifications() {
  ...
  try {
    ...
    console.log(`📊 Encontradas ${teacherNotificationsSnapshot.size} notificaciones de maestros inválidas`);
    ...
    console.log(`📊 Encontradas ${generalNotificationsSnapshot.size} notificaciones generales inválidas`);
    
    // Opción 1: Eliminar completamente las notificaciones inválidas
    const deleteInvalid = confirm('¿Deseas ELIMINAR completamente las notificaciones inválidas? (Recomendado)');
    ...
    if (deleteInvalid) {
      ...
      console.log('🎉 Marcado completado. Las notificaciones inválidas han sido marcadas como limpiadas.');
    }
...
async function listInvalidNotifications() {
  ...
  try {
    ...
    console.log(`📊 Total: ${teacherNotificationsSnapshot.size + generalNotificationsSnapshot.size} notificaciones inválidas`);
