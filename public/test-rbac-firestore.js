  ...
  try {
    ...
    try {
      ...
      RBACPersistenceService = {
        ...
        getNavigationConfig: () =>
          rbac.loadNavigationConfig().then(() => rbac.navigationConfig.value),
    ...
    if (Array.isArray(roles) && roles.length > 0) {
      ...
      roles.forEach((role, index) => {
        console.log(
          `  ${index + 1}. ${role.name} (${role.permissions?.length || 0} permisos) - ${role.isActive ? 'Activo' : 'Inactivo'}`,
        );
      });
    }
    ...
    console.log(
      `✅ Elementos de navegación obtenidos: ${Array.isArray(navigation) ? navigation.length : 'N/A'}`,
    );
    ...
    if (
      ...
      try {
        ...
        console.log(
          `📊 Después de inicialización: ${newRoles?.length || 0} roles, ${newPermissions?.length || 0} permisos`,
        );
      } catch (initError) {
