...
async function testCompleteIntegration() {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(studentIds.length, 5); i++) { // Solo verificar los primeros 5
      ...
      if (studentSnap.exists()) {
        ...
        console.log(`   ✅ ${studentData.nombre} ${studentData.apellido} (${studentId.slice(0, 8)}...)`);
        existingStudents++;
      } else {
    ...
    if (regularSnapshot.empty) {
      ...
      if (emergencyClassSnap.exists()) {
        ...
        if (missingFields.length === 0) {
          ...
          console.log(`   📋 Lista de estudiantes disponible: ${normalizedData.studentIds.length} estudiantes`);
        } else {
        ...
        for (const studentId of normalizedData.studentIds.slice(0, 3)) { // Solo los primeros 3
          const studentRef = doc(db, 'ALUMNOS', studentId);
          const studentSnap = await getDoc(studentRef);
