...
async function verifyEmergencyClassSystem() {
  ...
  try {
    ...
    const studentRows = document.querySelectorAll(
      '[class*="student"], tr[data-student-id], .student-row'
    )
    ...
    const firstStudentButton = document.querySelector(
      'button[class*="presente"], button[class*="ausente"]'
    )
    ...
    const checks = [classNameFound, studentRows.length > 0, attendanceButtons.length > 0]
