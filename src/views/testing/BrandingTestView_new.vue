      ...
      <div class="bg-white rounded-lg shadow-sm border p-6 mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <dd class="text-lg font-semibold text-gray-900">{{ brandingStore.config.version || 'N/A' }}</dd>
          </div>
          <div class="bg-gray-50 p-4 rounded-lg">
            <dt class="text-sm font-medium text-gray-600">Estado</dt>
            <dd class="text-lg font-semibold" :class="brandingStore.isLoading ? 'text-yellow-600' : 'text-green-600'">
              {{ brandingStore.isLoading ? 'Cargando...' : 'Listo' }}
            </dd>
          </div>
          <div class="bg-gray-50 p-4 rounded-lg">
            <dt class="text-sm font-medium text-gray-600">Configuración Actual</dt>
            <dd class="text-lg font-semibold text-gray-900">{{ brandingStore.config.appName || 'Sin configurar' }}</dd>
          </div>
          <div class="bg-gray-50 p-4 rounded-lg">
      ...
      <div class="bg-white rounded-lg shadow-sm border p-6 mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <dd class="text-sm" :class="brandingStore.isLoading ? 'text-yellow-600' : 'text-green-600'">
              {{ brandingStore.isLoading ? 'Cargando...' : 'Listo' }}
            </dd>
          </div>
        </div>
...
function resetToDefaults() {
  if (confirm('¿Estás seguro de que quieres hacer un reset completo? Esto restaurará toda la configuración a valores por defecto.')) {
    try {
