<template>
  <div class="p-4">
    <h1 class="text-2xl mb-4">
      Actividades registradas para el {{ formattedDate }}
    </h1>
    <div v-if="activities.length === 0">
      <p>No se han registrado actividades para esta fecha.</p>
    </div>
    <ul v-else class="space-y-4">
      <li v-for="activity in activities" :key="activity.classId" 
        ...
        <p>
          ...
          {{ activity.data.tarde.filter(id => !activity.data.justificacion?.some(j => j.id === id)).length }}
        </p>
        <p>
...
onMounted(async () => {
  try {
    ...
    activities.value = attendanceStore.attendanceDocuments.filter(doc => doc.fecha === formattedDate.value)
  } catch (error) {
