    ...
    <div class="space-y-3 sm:space-y-5">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3 sm:p-4 md:p-6 overflow-hidden">
        ...
        <div v-else-if="view === 'attendance-form'" class="space-y-3 sm:space-y-4">
          ...
          >
            <div
              v-if="attendanceStore.getObservations"
...
const fetchClassesForDate = async (dateStr: string) => {
  ...
  try {
    ...
    console.log(
      `[AttendanceView] Buscando clases para el maestro ${teacherId} en la fecha ${dateStr} (${dayOfWeek})`
    )
    console.log(`[AttendanceView] Total de clases en el store:`, classesStore.classes.length)
    console.log(
      `[AttendanceView] Clases con maestros:`,
      classesStore.classes.filter((c) => c.teachers?.length > 0).length
    )
    ...
    console.log(
      `[AttendanceView] Clases programadas como encargado para ${dayOfWeek}:`,
    ...
    const sharedClasses = classesStore.classes.filter((cls) => {
      ...
      const isAssistant = cls.teachers?.some(
      ...
      const hasSlotForDay = cls.schedule.slots.some((slot) => {
        ...
        const currentDay = dayOfWeek
          ...
          .replace(/[\u0300-\u036f]/g, "")
        console.log(
          `[AttendanceView] Comparando días - Slot: '${slotDay}' vs Actual: '${currentDay}' (${slotDay === currentDay})`
        )
      ...
      if (hasSlotForDay) {
        console.log(
          `[AttendanceView] ✅ Clase compartida ${cls.name} coincide con el día ${dayOfWeek}`
        )
      } else {
        console.log(
          `[AttendanceView] ❌ Clase compartida ${cls.name} NO coincide con el día ${dayOfWeek}`
        )
      }
    ...
    console.log(
      `[AttendanceView] Clases compartidas como asistente para ${dayOfWeek}:`,
      sharedClasses.length
    )
    console.log(
      `[AttendanceView] Detalles de clases compartidas:`,
      sharedClasses.map((c) => ({
        id: c.id,
        name: c.name,
        teachers: c.teachers,
    ...
    console.log(
      `[AttendanceView] Registros de asistencia para ${dateStr}:`,
      attendanceRecords.length
    )
    console.log(
      `[AttendanceView] Detalles de registros de asistencia:`,
    ...
    console.log(
      `[AttendanceView] Registros adicionales por nombre para ${dateStr}:`,
      attendanceRecordsByName.length
    )

    // Combinar ambos arrays de registros
    const allAttendanceRecords = [...attendanceRecords, ...attendanceRecordsByName]
    console.log(
      `[AttendanceView] Total de registros de asistencia (combinados): ${allAttendanceRecords.length}`
    )
    ...
    for (const record of allAttendanceRecords) {
      ...
      if (existingClass) {
        ...
        console.log(
          `[AttendanceView] Clase ${record.classId} ya existe en el mapa, actualizando...`
        )
        ...
        console.log(
          `[AttendanceView] Clase ${record.classId} NO existe en el mapa, buscando información...`
        )
        ...
        if (classInfo) {
          console.log(
            `[AttendanceView] ✅ Información de clase encontrada para ${record.classId}, agregando al mapa`
          )
          ...
          console.log(
            `[AttendanceView] ❌ Clase no encontrada en store, creando entrada básica para ${record.classId}`
          )
          classMap.set(record.classId, {
            id: record.classId,
    ...
    console.log(
      `[AttendanceView] - Programadas (encargado): ${classesWithStatus.filter((c) => c.type === "scheduled").length}`
    )
    console.log(
      `[AttendanceView] - Compartidas (asistente): ${classesWithStatus.filter((c) => c.type === "shared").length}`
    )
    console.log(
      `[AttendanceView] - Emergentes: ${classesWithStatus.filter((c) => c.type === "emergency").length}`
    )
    console.log(
      `[AttendanceView] - Con asistencia extra: ${classesWithStatus.filter((c) => c.type === "recorded").length}`
    )
...
function handleClassSelect(classId: string) {
  ...
  console.log(
    `[AttendanceView] Navegando a la clase ${classId} en la fecha ${selectedDate.value} con rol ${userRole} -> ${routePath}`
  )
...
const loadAttendanceData = async (className: string) => {
  try {
    console.log("[AttendanceView] loadAttendanceData:", {
      className,
    ...
    console.log(
      "[AttendanceView] After fetchAttendanceDocument, document:",
    ...
    if (
      ...
      console.log(
        "[AttendanceView] Final attendance records after initialization:",
        attendanceStore.attendanceRecords
      )
    }
    ...
    if (
      !attendanceDoc &&
      students.length > 0 &&
      window.localStorage.getItem("attendance-auto-debug") === "true"
    ) {
      console.warn(
        "[AttendanceView] No se encontró documento de asistencia - ejecutando debugging automático"
      )
      try {
        await attendanceStore.debugAttendanceSystem(
          selectedDate.value,
          className,
          authStore.user?.uid ?? ""
        )
  ...
  if (!debugEnabled) {
    ...
    if (attendanceStore.currentAttendanceDoc) {
      ...
      if (totalInArrays !== totalInRecords) {
        console.warn(
          "[AttendanceView] ⚠️ Data inconsistency detected - enable debug mode for details"
        )
      }
    }
    return
...
const handleOpenObservation = async (
  ...
  console.log(
    "[Aqui] handleOpenObservation llamado con estudiante:",
    student,
    "y observación:",
    observation
  )
  ...
  console.log(
    "[AttendanceView] Estado después de setear showObservationsModal:",
    showObservationsModal.value
  )
  ...
  console.log(
    "[AttendanceView] Después de nextTick, showObservationsModal:",
    showObservationsModal.value
  )
}
...
const handleObservationAdded = async (observations: string) => {
  try {
    ...
    console.log(
      `[AttendanceView] Guardando observación para clase ${selectedClass.value} - fecha ${selectedDate.value}:`,
      observations
    )
...
const handleOpenJustification = (student: SelectedStudent) => {
  const studentData = studentsStore
    .getStudentsByClass(selectedClass.value)
...
const loadEmergencyClassAttendanceData = async (emergencyClassData: any) => {
  try {
    console.log(
      "[AttendanceView] Cargando datos de asistencia para clase emergente:",
      emergencyClassData
    )
    isLoading.value = true
...
const fetchInitialData = async () => {
  try {
    ...
    if (currentUserId) {
      const sharedClassesDebug = classesStore.classes.filter((cls) => {
        return cls.teachers?.some(
          (teacher) => teacher.teacherId === currentUserId && teacher.role === TeacherRole.ASSISTANT
        )
      })
      console.log(
        `[AttendanceView] DEBUG: Clases compartidas disponibles para ${currentUserId}:`,
        sharedClassesDebug.length
      )
      console.log(
        `[AttendanceView] DEBUG: Detalles de clases compartidas:`,
        sharedClassesDebug.map((c) => ({
          id: c.id,
          name: c.name,
          teachers: c.teachers,
    ...
    if (props.date && props.classId) {
      console.log(
        `[AttendanceView] fetchInitialData: URL params detected - date: ${props.date}, classId: ${props.classId}`
      )
      const formattedDate =
        props.date.length === 8
      ...
      console.log(
        `[AttendanceView] fetchInitialData: selectedDate: ${selectedDate.value}, selectedClass: ${selectedClass.value}`
      )
      await selectClass(props.classId)
    } else if (route.query.class) {
      console.log(
        `[AttendanceView] fetchInitialData: URL query class detected: ${route.query.class}`
      )
