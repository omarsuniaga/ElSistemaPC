...
async function verifyAttendanceMarksForMonth(month: number, year: number, classId?: string) {
  try {
    ...
    console.log(`Verificadas ${results.filter(Boolean).length} fechas con asistencia para ${month}/${year}`);
...
onMounted(async () => {
  await state.init()
  markedDates.value.push("2025-05-05", "2025-05-06", "2025-05-07", "2025-05-08", "2025-05-09", "2025-05-10")
...
watch(() => state.selectedDate.value, () => {
  updateClassesForSelectedDate();
})
</script>
...
<template>
  <div class="p-4 space-y-4">
    <AttendanceHeader
      ...
      role="maestro"
      @changeView="state.view.value = $event"
