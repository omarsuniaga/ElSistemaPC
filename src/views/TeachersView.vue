<template>
  <div class="teachers-view py-6 dark:bg-gray-800 dark:text-gray-100 min-h-screen">
    ...
    <div
      class="bg-gradient-to-r from-blue-600 to-indigo-700 dark:from-blue-700 dark:to-indigo-800 text-white px-4 py-6 mb-6 rounded-lg shadow"
      role="banner"
    >
      <div
        class="flex flex-col md:flex-row items-center md:items-start md:justify-between gap-4 md:gap-6"
      >
        <div class="flex flex-col md:flex-row items-center mb-4 md:mb-0">
          <div
            class="w-24 h-24 rounded-full bg-white/30 flex items-center justify-center overflow-hidden mb-4 md:mb-0 md:mr-6"
          >
            <img
          ...
          <div class="text-center md:text-left">
            <h1 class="text-xl sm:text-2xl font-bold" tabindex="0">
              {{ teacher?.name || "Profesor" }}
            </h1>
            <p class="text-white/90 text-sm sm:text-base">
              {{ teacher?.specialties?.join(", ") || "Música" }}
            </p>
        ...
        <div class="flex flex-col items-center md:items-end">
          <div
            class="stats-card bg-white/20 p-4 rounded-lg"
            role="region"
            aria-label="Estadísticas del profesor"
          >
            <div class="grid grid-cols-2 gap-4 text-center" role="grid">
              <div>
                <p class="text-sm opacity-90" role="columnheader">Clases</p>
                <p class="text-xl sm:text-2xl font-bold" role="cell">
                  {{ teacherStats.classesCount || 0 }}
                </p>
              </div>
              <div>
                <p class="text-sm opacity-90" role="columnheader">Alumnos</p>
                <p class="text-xl sm:text-2xl font-bold" role="cell">
                  {{ teacherStats.studentsCount || 0 }}
                </p>
              </div>
            </div>
          </div>
