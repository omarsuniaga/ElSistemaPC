...
import { PlusCircleIcon, InformationCircleIcon, UserGroupIcon, TrashIcon, BookOpenIcon, EllipsisVerticalIcon, ArrowLeftIcon, PlusIcon, CalendarIcon, ClockIcon, AcademicCapIcon } from '@heroicons/vue/24/outline'
...
auth.onAuthStateChanged((user) => {
  ...
  console.log('Current user in TeachersHomeView:', user)
})
...
// Updated function to handle class creation with current user as teacher
...
async function handleSubmit(formData: any) {
  ...
  try {
    ...
    if (!currentUser.value) {
      throw new Error('Debe iniciar sesión para crear una clase')
    }
    
    // Serialize and deserialize to remove any non-serializable properties (like functions)
    ...
    const classData: {
      ...
      instrument: deserializedData.instrument || '',
      level: deserializedData.level || 'Iniciación',
      teacherId: currentUser.value.uid,
      studentIds: deserializedData.studentIds || [],
      schedule: {
        slots: Array.isArray(deserializedData.schedule?.slots)
        ? deserializedData.schedule.slots
          : []
      },
...
const openCreateClassModal = () => {
  ...
  // Ensure we're in create mode, not edit
...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 overflow-hidden ">
              ...
              >
                <div class="flex-shrink-0 mr-3">
                  <div class="bg-blue-100 dark:bg-blue-900 w-12 h-12 rounded-full flex items-center justify-center text-blue-800 dark:text-blue-200">
                    {{ (classItem.name ? classItem.name.substring(0, 2) : 'CL').toUpperCase() }}
                  </div>
                </div>
    ...
    <!-- Enhanced Create Class Button -->
    <div class=" bottom-6 flex flex-col space-y-1 mt-6 m-1 items-end z-20">
      <button
        @click="openCreateClassModal"
        class="group relative bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-full shadow-lg transform transition-all duration-300 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
        title="Crear Nueva Clase"
      >
        <PlusIcon class="h-8 w-8" />
        <span class="absolute right-full mr-3 top-1/2 transform -translate-y-1/2 whitespace-nowrap bg-blue-700 text-white px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          Crear Nueva Clase
        </span>
      </button>
    </div>
    ...
    >
      <ClassForm
        :initial-data="selectedGroupId ? (() => { const c = classesStore.classes.find(g => String(g.id) === String(selectedGroupId)); return c ? { ...c, createdAt: c.createdAt ? c.createdAt.toISOString() : undefined, updatedAt: c.updatedAt ? c.updatedAt.toISOString() : undefined } : undefined; })() : { teacherId: currentUserId }"
