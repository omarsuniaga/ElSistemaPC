...
watch(
  ...
  (newVal) => {
    if (newVal) {
      ...
      form.schedule.slots =
        newVal.schedule && Array.isArray(newVal.schedule.slots) && newVal.schedule.slots.length > 0
...
<template>
  <form class="space-y-6" @submit.prevent="handleSubmit">
    ...
    <div>
      ...
      <div
        v-for="(slot, index) in form.schedule.slots"
        :key="index"
      ...
      >
        ...
        <input
          ...
          type="time"
          class="block w-1/4 p-2 border rounded-md"
          placeholder="Inicio"
        />
        <input
        ...
        <button
          type="button"
          class="text-red-500"
          :disabled="form.schedule.slots.length <= 1"
      ...
      <button
        ...
        :disabled="form.schedule.slots.length >= 5"
        @click="addSession"
      >
        Agregar horario
      </button>
    ...
    <div class="flex justify-end space-x-3 pt-4 border-t">
      <button type="button" class="px-4 py-2 border rounded-md" @click="emit('cancel')">
        Cancelar
      </button>
      <button
        type="submit"
