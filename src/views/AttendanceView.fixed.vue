...
const navigateToAttendanceDetailUrl = (date: string, classId: string) => {
  ...
  window.history.replaceState({date: formattedDate, classId}, "", url)
}
...
const updateView = (newView: "calendar" | "class-select" | "attendance-form") => {
  ...
  if (newView === "calendar") {
    window.history.replaceState({view: "calendar"}, "", "/attendance/calendar")
...
const selectClass = async (className: string) => {
  try {
    ...
    if (!students.length) {
      ...
      students = studentsStore.students.filter(
        (s) =>
          (s.clase &&
            (Array.isArray(s.clase) ? s.clase.includes(className) : s.clase === className)) ||
          (s.grupo &&
            (Array.isArray(s.grupo) ? s.grupo.includes(className) : s.grupo === className))
      )
    }
    filteredStudents.value = students
...
watch(
  ...
  (newLength) => {
    console.log(
      `AttendanceView: Cambio detectado en documentos de asistencia. Nuevo total: ${newLength}`
    )
...
<template>
  ...
  >
    ...
    <div class="flex flex-wrap gap-2 mb-4 justify-center sm:justify-start">
      ...
      >
        <i class="fas fa-chart-pie mr-1 sm:mr-2" />
      ...
      <button
        ...
        @click="toggleTrends"
      >
        <i class="fas fa-chart-line mr-1 sm:mr-2" />
        <span class="hidden xs:inline">Tendencias</span>
        <span class="xs:hidden">T</span>
      </button>
      <button class="btn btn-secondary text-xs sm:text-sm" @click="openReportModal">
        <i class="fas fa-file-alt mr-1 sm:mr-2" />
        <span class="hidden xs:inline">Informe</span>
        <span class="xs:hidden">I</span>
      </button>
      <button class="btn btn-secondary text-xs sm:text-sm" @click="openExportModal">
        <i class="fas fa-file-export mr-1 sm:mr-2" />
        <span class="hidden xs:inline">Exportar</span>
        <span class="xs:hidden">E</span>
      </button>
      <button
        class="btn btn-secondary text-xs sm:text-sm"
        @click="exportCurrentClassAttendanceToPDF"
      >
        <i class="fas fa-file-pdf mr-1 sm:mr-2" />
    ...
    <div class="mb-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-md">
      ...
      <div class="flex flex-col sm:flex-row gap-3 items-center">
        ...
        <button
          class="btn btn-primary text-xs sm:text-sm mt-2 sm:mt-5 w-full sm:w-auto"
          @click="sendAttendanceEmail"
        >
          <i class="fas fa-envelope mr-1 sm:mr-2" />
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3 sm:p-4 md:p-6 overflow-hidden">
        <Transition name="fade" mode="out-in">
          ...
          <div
            ...
            key="class-select-view"
            class="max-w-3xl mx-auto"
          >
            <div class="flex justify-between items-center mb-3">
              <h2 class="text-lg font-semibold">Seleccionar Clase</h2>
              <button class="btn btn-secondary btn-sm" @click="updateView('calendar')">
                Volver
              </button>
            </div>
            <DateClassSelector
    ...
    <AttendanceObservation
      ...
      :model-value="showObservationsModal"
      ...
      @update:model-value="showObservationsModal = $event"
