  ...
  try {
    ...
    const grouped = allObservations.reduce((acc: Record<string, ClassGroup>, obs: any) => {
      ...
      acc[obs.classId].activities.push({
        ...
        content: typeof obs.text === 'string' ? obs.text : 
                (obs.text && typeof obs.text.formattedText === 'string' ? 
                 obs.text.formattedText : 
                 (obs.observacion || '(Sin contenido)')),
        timestamp: obs.date || (obs.createdAt ? (typeof obs.createdAt === 'string' ? 
                   obs.createdAt : new Date(obs.createdAt.seconds * 1000).toISOString()) : 
    ...
    groupedByClass.value = Object.values(grouped)
      .map((group: ClassGroup) => ({
        ...
        activities: group.activities.sort((a, b) => 
          normalizeDateForSort(b.timestamp) - normalizeDateForSort(a.timestamp)
        )
      }))
      .sort((a, b) => {
...
<template>
  <div class="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
    <!-- Header -->
    <div class="p-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">      <div class="max-w-7xl mx-auto">
    ...
    <div class="flex-1 overflow-y-auto p-4">
      <div class="max-w-7xl mx-auto">
        ...
        <div v-else-if="groupedByClass.length === 0" class="text-center py-12">
          ...
          <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">No hay observaciones registradas</h3>
        ...
        <div v-else class="space-y-4">
          <div v-for="group in groupedByClass" :key="group.classId" 
               class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            <!-- Class header -->
            <button @click="expanded = expanded === group.classId ? null : group.classId"
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="text-left">
                  <h3 class="font-medium text-gray-900 dark:text-gray-100">{{ group.className }}</h3>
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ group.activities.length }} observaciones
                  </p>
                </div>
              </div>
              <component :is="expanded === group.classId ? ChevronUpIcon : ChevronDownIcon" 
                        class="h-5 w-5 text-gray-400" />
            </button>

            <!-- Activities list -->
            <div v-show="expanded === group.classId" 
              ...
              <div v-for="activity in group.activities" :key="activity.id" 
                ...
                <div class="flex justify-between items-start mb-2">
                  ...
                  <div v-if="activity.presentCount !== undefined && activity.totalCount !== undefined"
                       :class="getAttendanceColor(getAttendanceRate(activity.presentCount, activity.totalCount))"
