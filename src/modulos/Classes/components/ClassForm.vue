...
watch(
  ...
  async (newSlots) => {
    ...
    const completeSlots = newSlots.filter(slot => 
...
watch([() => form.value.teacherId, () => form.value.classroom], async () => {
  const completeSlots = form.value.schedule.slots.filter(slot => 
    slot.day && slot.startTime && slot.endTime
  );
...
const applySuggestionToForm = (suggestion: any) => {
  ...
  const incompleteSlotIndex = form.value.schedule.slots.findIndex(slot => 
    !slot.day || !slot.startTime || !slot.endTime
  );
...
const handleSubmit = async () => {
  ...
  if (hasStudentConflicts.value) {
    errors.value.schedule = 'Algunos estudiantes tendrían conflictos de horario. Ningún alumno puede estar en más de una clase al mismo tiempo.';
    return;
  }
...
<template>
  <PermissionGuard :required-resource="ResourceType.MANAGE_CLASSES" required-action="create">
    ...
    <div>
      ...
      <div v-if="isValidating" class="flex items-center gap-2 mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-md">
        <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600"></div>
        <span class="text-sm text-blue-600 dark:text-blue-400">Validando horarios...</span>
      </div>

      <!-- Alertas de conflictos -->
      <div v-if="hasErrors && !isValidating" class="mt-2 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
        <div class="flex items-start">
          <svg class="h-5 w-5 text-red-400 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>
          </svg>
          <div class="ml-3">
            <h3 class="text-sm font-medium text-red-800 dark:text-red-400">Conflictos de Horario Detectados</h3>
      ...
      </div>      <!-- Alertas de advertencias -->
      <div v-if="hasWarnings && !isValidating" class="mt-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md">
        <div class="flex items-start">
          <svg class="h-5 w-5 text-yellow-400 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
          </svg>
          <div class="ml-3">
            <h3 class="text-sm font-medium text-yellow-800 dark:text-yellow-400">Advertencias de Horario</h3>
      ...
      <div v-if="isValidatingStudents" class="flex items-center gap-2 mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-md">
        <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600"></div>
        <span class="text-sm text-blue-600 dark:text-blue-400">Validando conflictos de estudiantes...</span>
      </div>

      <!-- Alertas específicas de conflictos de estudiantes -->
      <div v-if="hasStudentConflicts && !isValidatingStudents" class="mt-2 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
        <div class="flex items-start">
          <svg class="h-5 w-5 text-red-400 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
          </svg>
          <div class="ml-3">
            <h3 class="text-sm font-medium text-red-800 dark:text-red-400">⚠️ Conflictos de Estudiantes Detectados</h3>
      ...
      <div v-if="!hasStudentConflicts && !isValidatingStudents && props.classData?.studentIds?.length > 0" class="mt-2 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
        <div class="flex items-start">
      ...
      <div v-if="showSuggestions && suggestions.length > 0" class="mt-2 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
        <h3 class="text-sm font-medium text-green-800 dark:text-green-400 mb-2">Horarios Alternativos Sugeridos</h3>
        <div class="space-y-2">
          <div 
      ...
      <div v-for="(slots, index) in form.schedule.slots" :key="index" class="flex items-center gap-2 mt-2">
        ...
        <input 
          ...
          placeholder="Fin"
        />
        <button type="button" @click="removeSession(index)" class="text-red-500 hover:text-red-700">
          <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
          </svg>
        </button>
      </div>
      <button type="button" @click="addSession" class="mt-2 text-blue-600 hover:text-blue-800 text-sm">
        + Agregar horario
      </button>      <p v-if="errors.schedule" class="mt-1 text-sm text-red-500">{{ errors.schedule }}</p>
    </div>

    <!-- Analizador detallado de conflictos de estudiantes -->
    <div v-if="props.classData?.studentIds?.length > 0 && (hasStudentConflicts || (!isValidatingStudents && studentConflicts.size > 0))">
    ...
    <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
      ...
      >
        ...
        <span v-else-if="hasWarnings">{{ props.classData ? 'Actualizar con Advertencias' : 'Crear con Advertencias' }}</span>        <span v-else>{{ props.classData ? 'Actualizar' : 'Crear' }}</span>
      </button>
    </div>
  </form>
