...
const statusColor = (status: ClassStatus): string => {
  switch(status) {
    ...
    case 'terminado': return 'bg-gray-100 text-gray-600';
    case 'sin_ejecutar': return 'bg-blue-100 text-blue-800';
  ...
  <div class="overflow-x-auto">
    <table class="min-w-full divide-y divide-gray-200">
      <thead class="bg-gray-50">
        <tr>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Clase e Instrumento</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Horario y Estatus</th>
      ...
      <tbody class="bg-white divide-y divide-gray-200">
        <tr v-for="classItem in classes" :key="classItem.id">
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex items-center">
              ...
              <div class="ml-2">
                ...
                <span :class="['px-2 inline-flex text-xs leading-5 font-semibold rounded-full', statusColor(getScheduleStatus(classItem.horarioInicio, classItem.horarioFin))]">
                  {{ getScheduleStatus(classItem.horarioInicio, classItem.horarioFin) === 'current' ? 'En curso' : 
                     getScheduleStatus(classItem.horarioInicio, classItem.horarioFin) === 'past' ? 'Finalizado' : 'Programado' }}
                </span>
              </div>
            </div>
          </td>
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <button 
              @click="() => { selectedClass = classItem; showStudentsModal = true; }"
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <span :class="['px-2 inline-flex text-xs leading-5 font-semibold rounded-full', 
                         classItem.nivel === 'Avanzado' ? 'bg-purple-100 text-purple-800' :
                         classItem.nivel === 'Intermedio' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800']">
              {{ classItem.nivel }}
            </span>
          </td>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
            <div class="flex items-center">
              <button 
                ...
                @click="() => { selectedClass = classItem; showObservationsModal = true; }"
  ...
  >
    <div class="p-4">
      ...
      <ul class="divide-y divide-gray-200">
        <li 
          v-for="student in getStudents(selectedClass?.alumnos)" 
          :key="student.id"
          class="py-2"
        >
