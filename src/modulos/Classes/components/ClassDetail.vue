...
const formattedSchedule = computed(() => {
  ...
  const daysArray = Array.isArray(schedule.days)
    ? schedule.days
...
const getDefaultContent = (type?: string) => {
  switch (type) {
    case "attendance":
      return "Nueva toma de asistencia"
    case "content":
      return "Nuevo contenido agregado"
    case "evaluation":
      return "Nueva evaluación registrada"
    case "task":
...
const handleSaveChanges = async (updatedClass: ClassData) => {
  try {
    ...
    handleAddActivity({
      ...
      metadata: {
        timestamp: Date.now(),
        updatedFields: Object.keys(updatedClass).filter(
          (key) =>
            JSON.stringify(updatedClass[key as keyof ClassData]) !==
            JSON.stringify(props.selectedClass[key as keyof ClassData])
        ),
      },
    })
  } catch (error) {
...
<template>
  <div class="flex flex-col h-full bg-gray-100 dark:bg-gray-800 relative">
    ...
    <ClassActivityInput @add-activity="handleAddActivity" @start-editing="handleStartEditing" />
    <ClassQuickActions @start-activity="handleStartActivity" @start-editing="handleStartEditing" />
  </div>
</template>
