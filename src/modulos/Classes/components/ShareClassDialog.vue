  ...
  <div v-if="open" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" @click="$emit('close')">
    <div class="bg-white dark:bg-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" @click.stop>
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          ...
          <div class="space-y-2 max-h-48 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg p-3">
            ...
            >
              ...
              />
              <label :for="`teacher-${teacher.id}`" class="flex-1 text-sm text-gray-900 dark:text-white cursor-pointer">
                {{ teacher.name }}
              </label>
            ...
            <div v-if="teachers.length === 0" class="text-center py-4 text-gray-500 dark:text-gray-400">
        ...
        <!-- Summary -->
        <div v-if="selectedClassId && selectedTeachers.length > 0" class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
          ...
          <ul class="mt-2 space-y-1">
            ...
            >
              • {{ getTeacherName(teacherId) }} - {{ getPermissionText(teacherPermissions[teacherId]) }}
            </li>
          </ul>
        </div>
      </div>
...
watch(selectedTeachers, (newTeachers, oldTeachers) => {
