<template>
  <div class="class-history-tab p-4">
    ...
    <div v-if="classData && classData.history && classData.history.length > 0">
      <ul class="space-y-4">
        <li
          v-for="(entry, index) in classData.history"
          :key="index"
          class="bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm overflow-hidden"
        >
          <div class="bg-blue-50 dark:bg-gray-600 px-4 py-2 flex justify-between items-center">
            ...
            <span
              class="text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 py-1 px-2 rounded-full"
              >{{ entry.type || "Observación" }}</span
            >
          </div>
          ...
          <div class="p-4">
            ...
            <div
              v-if="entry.details"
              class="text-gray-600 dark:text-gray-300 mb-3 whitespace-pre-line"
            >
              {{ entry.details }}
            </div>
    ...
    <div v-else-if="!loadingHistory && classData" class="text-center py-8">
      ...
      <p class="mt-2 text-gray-500 dark:text-gray-400">
        No hay historial de observaciones disponible para esta clase.
      </p>
      <button
