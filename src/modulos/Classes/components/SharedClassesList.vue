  ...
  <div class="shared-classes-list">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Gestiona las clases que has compartido con otros maestros y las que han compartido contigo
          </p>
        </div>
      ...
      <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
          <div class="flex items-center">
            ...
            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Total Compartidas</span>
          </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
          <div class="flex items-center">
            ...
            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Compartidas Conmigo</span>
          </div>
          <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">
    ...
    <div class="p-6">
      ...
      <div v-if="filteredSharedClasses.length === 0" class="text-center py-12">
        <div class="mx-auto h-24 w-24 rounded-full bg-purple-100 dark:bg-purple-900/20 flex items-center justify-center mb-4">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="p-4 bg-gray-50 dark:bg-gray-900/50">
            ...
            <div v-if="getSharedTeachers(classItem).length === 0" class="text-sm text-gray-500 dark:text-gray-400">
              Solo el propietario tiene acceso
            </div>            <div v-else class="space-y-2 max-h-32 overflow-y-auto">
              <template v-for="teacher in getSharedTeachers(classItem)" :key="teacher?.id || 'unknown'">
                ...
                >
                  ...
                  <div class="flex items-center space-x-2">                    <!-- Permission Badge -->
                    <span :class="[
                      'px-2 py-1 text-xs font-medium rounded-full',
                      getPermissionBadgeColor(getTeacherPermissions(classItem, typeof teacher.id === 'string' ? teacher.id : ''))
                    ]">
                      {{ getPermissionText(getTeacherPermissions(classItem, typeof teacher.id === 'string' ? teacher.id : '')) }}
                    </span>
                      <!-- Actions -->
                    <button
...
const canEditClass = (classItem: ClassData): boolean => {
  ...
  return permissions.includes('write') || permissions.includes('manage') || permissions.includes('canEditClass');
};
...
const getSharedTeachers = (classItem: ClassData) => {
  ...
  return classItem.teachers.map(teacherItem => {
    ...
    if (!teacher) {
      ...
      return { id: teacherId, name: demoNames[teacherId as keyof typeof demoNames] || 'Maestro Demo' };
    }
    return teacher;
...
const handleShareClass = async (data: { classId: string; teachers: Array<{ teacherId: string; permissions: string[] }> }) => {
  try {
    ...
    for (const teacherData of data.teachers) {
      ...
      const alreadyExists = existingTeachers.some(teacher => {
        return typeof teacher === 'string' ? teacher === teacherData.teacherId : teacher.teacherId === teacherData.teacherId;
      });
