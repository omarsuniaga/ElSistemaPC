<template>
  <div :class="['space-y-4 p-4 rounded-lg', 'bg-white dark:bg-gray-900', cssClassRef]">
    ...
    <div class="flex justify-between items-center mb-2">
      ...
      <button class="px-2 py-1 bg-red-500 text-white rounded text-xs" @click="debugAttendance">
        DEBUG
      </button>
    </div>
    ...
    <div v-if="loading" class="flex justify-center py-4">
      <div
        class="animate-spin rounded-full h-8 w-8 border-t-2 border-blue-600 dark:border-blue-400"
      />
    </div>
...
const { checkAttendanceExists } = useAttendanceOptimized();
...
const checkAttendanceStatus = async (classId: string, date: string) => {
  ...
  try {
    ...
    console.log('[DateClassSelector] Attendance status set:', {
      key,
      status: attendanceStatus.value[key],
    });
  }
};
  ...
  async () => {
    ...
    try {
      ...
      if (!attendanceStore.attendanceDocuments.length) {
        ...
        console.log(
          '[DateClassSelector] Loaded attendance documents:',
          ...
          c.schedule?.slots?.some(
            (s) =>
              ...
              (typeof s.day === 'string' &&
                s.day
                  .toLowerCase()
                  .includes(format(parseISO(localDate.value), 'EEEE', { locale: es }).toLowerCase())),
          ),
      );
