  ...
  <div class="weekly-schedule-view bg-white dark:bg-gray-900 min-h-screen">
      ...
      <div class="mt-4 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
        <div class="flex flex-wrap gap-6 items-center justify-between">
          ...
          <div class="flex flex-wrap gap-4 items-center">
            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Períodos del día:</span>
          ...
          <div class="flex gap-4 items-center">
            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Vista de horarios:</span>
            <label class="flex items-center space-x-2">
              <input 
                type="radio" 
    ...
    <div class="p-6">
      ...
      <div v-else>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm mb-8">
          <!-- Days Header -->
          <div class="grid grid-cols-8 border-b border-gray-200 dark:border-gray-700">
            <div class="p-4 text-center font-medium text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700">
              Hora
            </div>
            <div 
          ...
          <div class="relative">
            <!-- Time Row Headers -->
            <div v-for="timeSlot in timeSlots" :key="timeSlot" class="grid grid-cols-8 border-b border-gray-200 dark:border-gray-700 last:border-b-0 min-h-[80px]">
              <!-- Hour Label -->
              <div class="flex items-center justify-center p-3 text-sm font-medium text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
              ...
              >
                ...
                <template v-if="timeConfig.viewMode === 'standard'">
                  ...
                  >                    <div class="text-xs font-bold text-white truncate mb-1">
                      ...
                      <span v-if="isSharedWithMe(classItem)" class="ml-1 inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-white/20 text-white">
                        👥
                      </span>
                      <span v-else-if="isMySharedClass(classItem)" class="ml-1 inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-white/20 text-white">
                        📤
                      </span>
                    </div>
                    ...
                    </div>                    <div class="text-xs text-white/60 truncate mt-1 flex items-center justify-between">
                ...
                <div v-if="getClassesForTimeSlot(day.key, timeSlot).length === 0" class="flex items-center justify-center h-full text-gray-300 dark:text-gray-600">
      ...
      <div v-else-if="viewMode === 'list'" class="space-y-4 mb-8">
        <div v-for="day in weekDays" :key="day.key" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm mb-6">
          ...
          <div class="p-6">
            <div v-if="getClassesForDay(day.key).length === 0" class="text-center py-8 text-gray-500 dark:text-gray-400">
              ...
              >                <div class="flex items-start justify-between">
                  <div class="flex-1">
                    <div class="flex items-center space-x-2">
                      ...
                      <span v-if="isSharedWithMe(classItem)" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-400">
                        👥 Compartida conmigo
                      </span>
                      <span v-else-if="isMySharedClass(classItem)" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900/20 text-purple-800 dark:text-purple-400">
                        📤 Mi clase compartida
                      </span>
                    </div>
    ...
    <!-- Class Details Modal -->
    <div v-if="selectedClass" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" @click="closeClassDetails">
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ selectedClass.instrument }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Programa</label>
              <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ getProgramName(selectedClass.level) }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Maestro</label>
              <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ getTeacherName(selectedClass.teacherId) }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Capacidad</label>
              <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ selectedClass.capacity || 'No definida' }}</p>
            </div>
          </div>

          <!-- Schedules -->
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Horarios</label>
            <div class="space-y-2">
              <div 
...
import { getAppConfig, setAppConfig, getActiveTimeRange, isHourInActivePeriods, type AppConfig } from '../service/appConfig';
...
const getClassSchedules = (classItem: ClassData) => {
  ...
  console.log(`Clase "${classItem.name}" tiene horario en formato no reconocido:`, classItem.schedule);
  return [];
};
...
const getClassesForDay = (day: string) => {
  const classes = filteredClasses.value.filter(classItem => {
    ...
    const aSchedule = getClassSchedules(a).find(s => normalizeDayName(s.day) === normalizeDayName(day));
    const bSchedule = getClassSchedules(b).find(s => normalizeDayName(s.day) === normalizeDayName(day));
    if (!aSchedule || !bSchedule) return 0;
    return aSchedule.startTime.localeCompare(bSchedule.startTime);
  });
  
  console.log(`Clases para ${day}:`, classes.map(c => c.name));
  return classes;
};
