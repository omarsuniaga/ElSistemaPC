<template>
  <div class="sticky top-0 z-10 bg-blue-600 dark:bg-blue-900 text-white shadow-md">
    ...
    <div v-if="isMobile" class="px-4 py-3 flex items-center gap-2">
      <button class="text-white" @click="$emit('go-back')">
        <ArrowLeftIcon class="h-6 w-6" />
      </button>
      <div class="flex items-center flex-1 min-w-0">
        <div
          class="h-10 w-10 rounded-full bg-blue-700 dark:bg-blue-800 flex items-center justify-center text-lg font-bold mr-3"
        >
    ...
    <div v-else class="px-6 py-4 flex items-center">
      <div class="flex items-center flex-1 min-w-0">
        <div
          class="h-12 w-12 rounded-full bg-blue-700 dark:bg-blue-800 flex items-center justify-center text-xl font-bold mr-4"
        >
          {{ (selectedClass.name && selectedClass.name.substring(0, 1).toUpperCase()) || "?" }}
        </div>
        <div>
