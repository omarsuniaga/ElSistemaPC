...
<template>
  <div class="sticky bottom-10 bg-gray-100 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 p-3">
    <!-- Attached files preview -->
    <div v-if="attachedFiles.length > 0" class="mb-2 px-4">
      <div v-for="file in attachedFiles" :key="file.id" 
           class="inline-flex items-center bg-gray-200 dark:bg-gray-700 rounded-full px-3 py-1 text-xs mr-2 mb-1">
        <component :is="file.type.startsWith('image/') ? PhotoIcon : file.type.startsWith('audio/') ? MicrophoneIcon : DocumentIcon" 
                  class="h-3.5 w-3.5 mr-1 text-gray-600 dark:text-gray-300" />
        <span class="truncate max-w-[120px]">{{ file.name }}</span>
        <button @click="removeAttachedFile(file.id)" class="ml-1 text-gray-500 hover:text-gray-700">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-full px-4 py-2 flex items-center gap-2 shadow-sm">
      ...
      <button
        ...
        :class="activityInput.trim() || attachedFiles.length > 0 ? 'text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300' : 'text-gray-400 dark:text-gray-600'"
