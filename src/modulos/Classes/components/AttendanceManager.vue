  ...
  <div class="attendance-manager bg-white shadow-lg rounded-lg p-6 dark:bg-gray-800">
    <div class="flex justify-between items-center mb-6">
      ...
      <div class="flex items-center space-x-2">
        ...
        <button @click="showDatePicker = !showDatePicker" class="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
          <CalendarIcon class="h-5 w-5" />
        </button>
        <!-- Simple date picker -->
        <div v-if="showDatePicker" class="absolute mt-2 bg-white dark:bg-gray-800 shadow-lg rounded-lg p-4 z-10">
          <input 
            type="date" 
                ...
                <div class="flex items-center space-x-4">
                  <button 
                    ...
                    :class="[
                      ...
                      student.present ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'
                    ]"
                  >
                    Presente
                  </button>
                  <button 
                    ...
                    :class="[
                      'px-3 py-1 rounded-full text-xs font-medium',
                      student.present === false ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'
                    ]"
                  >
                    Ausente
                  </button>
    ...
    <div v-else class="text-center py-8">
      ...
      <p class="mt-2 text-gray-600 dark:text-gray-400">No hay estudiantes para registrar asistencia</p>
    </div>
  </div>
</template>
...
const downloadAttendanceReport = async () => {
  try {
    ...
    const studentsWithAttendance = props.students.map(student => {
      ...
      return {
        ...student,
        attendance: record && record.present !== null ? [
          {
            present: record.present === true,
            date: currentDate.value.toISOString().split('T')[0],
            notes: record.notes
