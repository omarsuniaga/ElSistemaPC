...
const showActivityIcon = (type: Activity['type']) => {
  switch (type) {
    ...
    case 'task': return DocumentIcon;
    case 'message': return ChatBubbleLeftRightIcon;
    case 'student_added': return UserGroupIcon;
    case 'schedule_change': return ClockIcon;
    case 'student_action': return UserCircleIcon;
...
<template>
  <div class="pb-20 activity-feed">
    <div v-for="(activity, index) in activities" :key="activity.id" class="mb-4">
      <!-- Date divider -->
      <div v-if="index === 0 || formattedDate(activity.timestamp) !== formattedDate(activities[index-1].timestamp)" 
      ...
      <div class="flex items-start gap-2"
           :class="{'justify-end': activity.user !== 'Sistema'}">
        <div 
        ...
        >
          <component :is="showActivityIcon(activity.type)" class="h-5 w-5 text-gray-600 dark:text-gray-400" />
        </div>
        ...
        <div class="max-w-[85%] rounded-lg px-3 py-2"
          ...
          <div class="flex items-center gap-1">
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ formattedTime(activity.timestamp) }}</span>
          </div>
          ...
          <div v-if="activity.metadata" class="mt-2">
            <!-- Attendance metadata -->
            <div v-if="activity.type === 'attendance' && activity.metadata.present !== undefined" 
                 class="text-xs bg-gray-100 dark:bg-gray-700 rounded p-1.5 flex items-center">
              <span class="text-green-600 dark:text-green-400 font-medium mr-2">Presentes: {{ activity.metadata.present }}</span>
              <span class="text-red-600 dark:text-red-400 font-medium">Ausentes: {{ activity.metadata.absent }}</span>
            </div>
            
            <!-- Evaluation metadata -->
            <div v-if="activity.type === 'evaluation' && activity.metadata.score" 
                 class="text-xs bg-gray-100 dark:bg-gray-700 rounded p-1.5">
              <span class="font-medium">Calificación: {{ activity.metadata.score }}</span>
            </div>
            
            <!-- Task metadata -->
            <div v-if="activity.type === 'task' && activity.metadata.dueDate" 
                 class="text-xs bg-gray-100 dark:bg-gray-700 rounded p-1.5">
