  ...
  <div class="max-w-2xl mx-auto p-6 bg-white rounded shadow">
    ...
    <form @submit.prevent="onSubmit">
      <div class="mb-4">
        ...
        <div class="w-full border rounded px-2 py-1 bg-gray-100 text-gray-700 flex items-center justify-between">
          <span>
            {{ selectedInstrument ? selectedInstrument.nombre + ' (' + (selectedInstrument.serial || '') + ')' : 'Instrumento no encontrado' }}
          </span>
          <button
      ...
      <div class="mb-4">
        ...
        <input type="text" v-model="searchStudent" class="w-full border rounded px-2 py-1" placeholder="Escriba al menos 3 letras del nombre del alumno" />
        <ul v-if="studentResults.length" class="border rounded bg-white mt-1 max-h-40 overflow-y-auto">
          <li v-for="alumno in studentResults" :key="alumno.id" @click="selectedStudentId = alumno.id; searchStudent = alumno.nombre + ' ' + alumno.apellido; studentResults = []" class="px-2 py-1 hover:bg-blue-100 cursor-pointer">
      ...
      <div class="flex flex-col sm:flex-row justify-end gap-2 mt-6">
        <button type="button" class="px-3 py-1 bg-gray-300 rounded flex items-center justify-center" @click="onCancel">
          <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
          <span class="hidden sm:inline">Cancelar</span>
        </button>
        <button type="submit" class="px-3 py-1 bg-blue-600 text-white rounded flex items-center justify-center" :disabled="!selectedStudentId">
...
onMounted(async () => {
  ...
  if (idParam) {
    ...
    if (selectedInstrument.value && selectedInstrument.value.isAssign && selectedInstrument.value.asignacion?.studentId) {
      router.replace({
...
watch(searchStudent, (val) => {
  if (val.length >= 3) {
    ...
    studentResults.value = alumnos.value.filter(a =>
