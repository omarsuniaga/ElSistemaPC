<template>
  <div class="max-w-2xl mx-auto p-6 bg-white rounded shadow">
    <h1 class="text-xl font-bold mb-4">{{ isEdit ? 'Editar Instrumento' : 'Registrar Instrumento' }}</h1>
    <form @submit.prevent="onSubmit">
      ...
      <div class="mb-4">
        ...
        <div v-for="(acc, i) in form.accesorios" :key="i" class="flex gap-2 mb-2">
          <input v-model="acc.nombre" placeholder="Nombre" class="border rounded px-2 py-1 flex-1" />
          <input v-model="acc.estado" placeholder="Estado" class="border rounded px-2 py-1 flex-1" />
          <input v-model="acc.observaciones" placeholder="Observaciones" class="border rounded px-2 py-1 flex-1" />
          <button type="button" @click="removeAccesorio(i)" class="text-red-600">Eliminar</button>
        </div>
        <button type="button" @click="addAccesorio" class="px-2 py-1 bg-gray-200 rounded">Agregar accesorio</button>
      </div>
      <div class="flex justify-end gap-2 mt-6">
        <button type="button" class="px-3 py-1 bg-gray-300 rounded" @click="onCancel">Cancelar</button>
        <button type="submit" class="px-3 py-1 bg-blue-600 text-white rounded">{{ isEdit ? 'Guardar Cambios' : 'Registrar' }}</button>
      </div>
    </form>
  </div>
</template>
...
async function onSubmit() {
  if (!form.value.nombre || !form.value.serial || !form.value.marca || !form.value.state || !form.value.familia) return;
  saving.value = true;
