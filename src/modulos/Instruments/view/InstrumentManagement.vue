...
const loadData = async () => {
  ...
  try {
    ...
    availableFamilies.value = [...new Set(instruments.value.map(i => i.familia).filter(Boolean) as string[])];
...
const saveInstrument = async (instrumentData: Partial<Instrument>) => {
  ...
  try {
    ...
    if (isCreating.value) {
      ...
      const newInstrument = await instrumentStore.addInstrument(instrumentData as Omit<Instrument, 'id'>);
...
const deleteInstrument = async () => {
  ...
  const confirmDelete = confirm(`¿Está seguro que desea eliminar el instrumento "${selectedInstrument.value.nombre}"?`);
...
<template>
  <div class="instrument-management p-2 sm:p-4 max-w-7xl mx-auto">
    ...
    <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-md p-4 mb-4">
      ...
      <button @click="loadData" class="mt-2 px-4 py-1 bg-red-600 text-white rounded-md hover:bg-red-700">
        Reintentar
      </button>
    </div>
    ...
    <div v-else class="flex flex-col lg:flex-row gap-4">
        ...
        <div class="bg-white rounded-md shadow-sm overflow-x-auto">
          ...
          <div class="px-4 py-3 bg-gray-50 border-t flex items-center justify-between">
            <div class="text-sm text-gray-700">
              Mostrando <span class="font-medium">{{ (pagination.currentPage - 1) * pagination.itemsPerPage + 1 }}</span> 
              a <span class="font-medium">{{ Math.min(pagination.currentPage * pagination.itemsPerPage, pagination.totalItems) }}</span>
      ...
      <div class="lg:w-1/3">
        ...
        <div v-if="isEditing || isCreating" class="bg-white p-4 rounded-md shadow-sm">
          <h2 class="text-lg font-semibold mb-4">{{ isCreating ? 'Crear Instrumento' : 'Editar Instrumento' }}</h2>
          <InstrumentForm 
        ...
        <!-- Estado vacío -->
        <div v-else class="bg-white p-6 rounded-md shadow-sm flex flex-col items-center justify-center text-center h-64">
