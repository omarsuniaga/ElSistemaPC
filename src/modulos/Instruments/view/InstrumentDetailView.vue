    ...
    <div v-if="instrument" class="space-y-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
          <div>
            ...
            <p class="text-gray-700 dark:text-gray-300">{{ instrument.marca }} {{ instrument.modelo }}</p>
        ...
        <div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
            <p><span class="font-medium text-gray-600 dark:text-gray-400">Estado:</span> {{ instrument.estado }}</p>
            <p><span class="font-medium text-gray-600 dark:text-gray-400">Familia:</span> {{ instrument.familia }}</p>
            <p><span class="font-medium text-gray-600 dark:text-gray-400">Tamaño:</span> {{ instrument.tamaño }}</p>
            <p><span class="font-medium text-gray-600 dark:text-gray-400">Ubicación:</span> {{ instrument.ubicacion }}</p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <div v-if="alumnoActual">
          ...
          <p class="text-sm text-gray-500 mt-1">Asignado desde: <span class="font-medium">{{ fechaAsignacion }}</span></p>
          <p class="text-sm text-gray-500">Nivel durante asignación: <span class="font-medium">{{ nivelAsignado }}</span></p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <ul v-if="instrument.accesorios && instrument.accesorios.length" class="list-disc list-inside space-y-1 text-sm">
          <li v-for="(acc, i) in instrument.accesorios" :key="i">
            <span class="font-medium">{{ acc.nombre }}</span> ({{ acc.estado }})
            <span v-if="acc.observacion" class="text-gray-600 dark:text-gray-400">- {{ acc.observacion }}</span>
          </li>
        </ul>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <p class="text-sm whitespace-pre-wrap">{{ instrument.observaciones || 'Sin observaciones ni historial de mantenimiento detallado registrado.' }}</p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <ul v-if="historialPasado && historialPasado.length" class="space-y-2 text-sm">
          <li v-for="(h, i) in historialPasado" :key="i" class="border-b border-gray-200 dark:border-gray-700 pb-1">
            <span class="font-medium">{{ h.nombreAlumno }}</span>
            <span class="text-gray-600 dark:text-gray-400"> ({{ h.fechaInicio }} a {{ h.fechaFin }})</span>
...
async function quitarAsignacion() {
  ...
  // Confirmación
  if (!confirm(`¿Estás seguro de quitar la asignación de ${instrument.value.nombre} a ${alumnoActual.value?.nombre} ${alumnoActual.value?.apellido}?`)) {
    return;
  }
...
function reasignarInstrumento() {
  ...
  alert('Funcionalidad de reasignar instrumento pendiente de implementación. Debería llevar a un formulario de asignación.');
