...
<template>  <div v-if="instrument" class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
    <div class="bg-indigo-600 dark:bg-indigo-700 p-4 text-white flex flex-col sm:flex-row justify-between items-center gap-2">
    ...
    <div class="p-4">
      <!-- Información básica -->
      <div class="mb-6">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-2 mb-3">Información General</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <span class="inline-block px-2 py-1 text-xs rounded-full" :class="getStatusClass(instrument.estado)">
              {{ instrument.estado || 'No especificado' }}
            </span>
          </div>
        </div>        <div class="mt-3" v-if="instrument.detallesEstado">
          <p class="text-sm text-gray-500 dark:text-gray-400">Detalles del estado</p>
          <p class="text-sm bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white p-2 rounded">{{ instrument.detallesEstado }}</p>
        </div>
      </div>
      <!-- Estuche -->
      <div class="mb-6" v-if="instrument.estuche">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-2 mb-3">Estuche</h3>
        <div v-if="instrument.estuche.tiene">
          <div class="flex items-center mb-2">
            <span class="inline-block mr-2 px-2 py-1 text-xs rounded-full" :class="getStatusClass(instrument.estuche.estado)">
              {{ instrument.estuche.estado || 'No especificado' }}
            </span>
            <span v-if="instrument.estuche.observacion" class="text-sm text-gray-900 dark:text-white">
      ...
      <div class="mb-6" v-if="instrument.accesorios && instrument.accesorios.length > 0">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-2 mb-3">Accesorios</h3>
        <div class="space-y-2">
          <div v-for="(acc, idx) in instrument.accesorios" :key="idx" class="flex justify-between p-2 border border-gray-200 dark:border-gray-700 rounded">
            <div>
      ...
      <div class="mb-6" v-if="instrument.historial && instrument.historial.length > 0">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-2 mb-3">Historial de Uso</h3>
        <div class="space-y-2">
          <div v-for="(entry, idx) in instrument.historial" :key="idx" class="p-2 border border-gray-200 dark:border-gray-700 rounded">
            <div class="flex flex-col sm:flex-row sm:justify-between">
              <span class="font-medium text-gray-900 dark:text-white">{{ entry.nombreAlumno }}</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">
                {{ formatDate(entry.fechaInicio) }} - {{ entry.fechaFin ? formatDate(entry.fechaFin) : 'Actual' }}
              </span>
            </div>
            <div v-if="entry.observaciones" class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ entry.observaciones }}</div>
          </div>
        </div>
      </div>
      <!-- Observaciones -->
      <div v-if="instrument.observaciones">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-2 mb-3">Observaciones</h3>
        <p class="text-sm bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded">{{ instrument.observaciones }}</p>
      </div>
    </div>
  </div>
