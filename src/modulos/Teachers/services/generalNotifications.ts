 ...
 */
export const createStudentRegistrationNotification = async (notificationData: {
  teacherId: string
  studentId: string
  studentName: string
  studentData: any
 ...
 */
export const createGeneralAnnouncementForAllTeachers = async (announcementData: {
  title: string
  message: string
  fromUserId: string
  fromUserName: string
...
export const subscribeToGeneralNotifications = (
  ...
  return onSnapshot(
    ...
    (querySnapshot) => {
      console.log(
        "Listener de notificaciones generales ejecutado, documentos recibidos:",
        querySnapshot.size
      )
