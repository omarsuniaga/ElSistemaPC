...
onMounted(async () => {
  ...
  try {
    ...
    console.log(`✅ [TodaysClassesSection] Datos cargados. Clases hoy: ${todaysClasses.value.length}`);
  } catch (error) {
...
watch([currentTeacherId], async ([newTeacherId], [oldTeacherId]) => {
...
<template>
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4">
    <h2 class="text-base md:text-lg font-semibold text-gray-900 dark:text-white mb-3 flex justify-between items-center">
      ...
      <span v-if="!loading" class="flex items-center gap-1.5 px-2 py-1 bg-amber-100 text-amber-800 rounded text-sm">
