        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center">
          ...
          >
            <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 p-6 text-left align-middle shadow-xl transition-all">
              ...
              <div class="mb-6">
                <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 space-y-3">
                  <div class="flex justify-between">
                    ...
                    <span class="text-sm text-gray-900 dark:text-white font-semibold">{{ invitation?.className }}</span>
                  </div>
                  
                  <div class="flex justify-between">
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Invitado por:</span>
                    <span class="text-sm text-gray-900 dark:text-white">{{ invitation?.fromUserName }}</span>
                  </div>
                  
                  <div class="border-t border-gray-200 dark:border-gray-600 pt-3">
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-2">Permisos otorgados:</span>
                    <div class="space-y-1">
                      <div class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                        <CheckIcon v-if="invitation?.permissions?.canTakeAttendance" class="w-4 h-4 text-green-500 mr-2" />
                        <XMarkIcon v-else class="w-4 h-4 text-red-500 mr-2" />
                        Tomar asistencia
                      </div>
                      <div class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                        <CheckIcon v-if="invitation?.permissions?.canAddObservations" class="w-4 h-4 text-green-500 mr-2" />
                        <XMarkIcon v-else class="w-4 h-4 text-red-500 mr-2" />
                        Agregar observaciones
                      </div>
                      <div class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                        <CheckIcon v-if="invitation?.permissions?.canViewAttendanceHistory" class="w-4 h-4 text-green-500 mr-2" />
                        <XMarkIcon v-else class="w-4 h-4 text-red-500 mr-2" />
              ...
              <!-- Mensaje de error -->
              <div v-if="error" class="mb-4 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-lg">
