...
<template>
  <div class="space-y-6">
    <h2
      class="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4 flex justify-between items-center"
    >
      <span>Próximas Clases (24h)</span>
      <span
        class="text-sm font-normal bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-200 px-2 py-1 rounded-full"
      >
      ...
      <div class="space-y-4">
                  ...
                  <span
                    ...
                    :class="{
                      'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-200':
                        getTimeRemaining(classItem.nextSessionDate) === 'En curso',
                      'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-200':
                        getTimeRemaining(classItem.nextSessionDate).includes('min'),
                      'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-200':
                        getTimeRemaining(classItem.nextSessionDate).includes('h'),
                    }"
                  >
        ...
        <div v-else class="py-12 text-center text-gray-500 dark:text-gray-400">
          ...
          <p class="text-sm mt-2">
            Disfruta de tu tiempo libre o aprovecha para preparar tus próximas clases.
          </p>
        </div>
      </div>
    </div>
