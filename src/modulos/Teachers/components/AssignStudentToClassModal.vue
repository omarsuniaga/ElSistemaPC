      ...
      >
        ...
        <div v-if="student" class="mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
          ...
          <div class="space-y-1 text-sm">
            <p><span class="font-medium">Nombre:</span> {{ student.firstName }} {{ student.lastName }}</p>
            <p v-if="student.email"><span class="font-medium">Email:</span> {{ student.email }}</p>
            <p v-if="student.phone"><span class="font-medium">Teléfono:</span> {{ student.phone }}</p>
            <p v-if="student.instrument"><span class="font-medium">Instrumento:</span> {{ student.instrument }}</p>
          </div>
        </div>
        ...
        <div class="mb-6">
          ...
          >
            ...
            <option 
              ...
              :value="classItem.id"
            >
              {{ classItem.name }} - {{ classItem.instrument || 'Música' }} ({{ classItem.studentIds?.length || 0 }} estudiantes)
            </option>
          </select>
        </div>
        ...
        <div v-if="selectedClass" class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
          ...
          <div class="space-y-1 text-sm text-blue-800 dark:text-blue-300">
            ...
            <p v-if="selectedClass.description"><span class="font-medium">Descripción:</span> {{ selectedClass.description }}</p>
            <p><span class="font-medium">Instrumento:</span> {{ selectedClass.instrument || 'Música general' }}</p>
            <p><span class="font-medium">Nivel:</span> {{ selectedClass.level || 'Todos los niveles' }}</p>
            <p><span class="font-medium">Estudiantes actuales:</span> {{ selectedClass.studentIds?.length || 0 }}</p>
          </div>
        </div>

        <!-- Error de asignación -->
        <div v-if="assignmentError" class="mb-4 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-md">
          <div class="flex">
