          ...
          <div class="flex justify-between items-start">
            <div>
              ...
              <div class="mt-2 flex items-center text-sm">
                <span class="font-medium mr-2 text-gray-700 dark:text-gray-300">Próxima sesión:</span>
                 <!-- Use the prop function for formatting -->
                <span class="text-gray-600 dark:text-gray-400">{{ formatDateTime(classItem.nextSessionDate || getNextSession(classItem)) }}</span>
              </div>

              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Aula: {{ classItem.classroom || 'Sin asignar' }}</p>
              <p class="text-sm text-gray-500 dark:text-gray-400">Estudiantes: {{ classItem.studentIds?.length || 0 }}</p>
            </div>
