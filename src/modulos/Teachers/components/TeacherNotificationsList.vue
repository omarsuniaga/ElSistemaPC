<template>
  <div class="space-y-4">
    ...
    <!-- Error -->
    <div v-else-if="error" class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-lg p-4">
      <div class="flex">
        ...
        <div v-if="notification.type === 'class-invitation'" class="mt-3 pl-11">
          <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 space-y-2">
            <div class="flex justify-between text-sm">
              ...
              <span class="font-medium text-gray-900 dark:text-white">{{ notification.className }}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span class="text-gray-700 dark:text-gray-300">De:</span>
              <span class="font-medium text-gray-900 dark:text-white">{{ notification.fromUserName }}</span>
            </div>
          </div>
    ...
    <div v-else class="text-center py-8">
      ...
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
        {{ activeFilter === 'all' ? 'No tienes notificaciones.' : 'No hay notificaciones en esta categoría.' }}
      </p>
    </div>
  </div>
</template>
...
const pendingInvitationsCount = computed(() =>
  props.notifications.filter(n => n.type === 'class-invitation' && n.status === 'pending').length
);
  ...
  const notificationDate =
    typeof date === 'string'
      ...
      : date instanceof Date
        ? date
        : date.toDate(); // Convert Timestamp-like objects to Date
