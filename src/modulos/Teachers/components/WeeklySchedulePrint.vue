...
const teacherClasses = computed((): ClassItem[] => {
  return classesStore.classes
    .filter((c): c is (typeof c & { teacherId: string }) => 
...
<template>
  <div id="schedule-pdf" class="bg-white p-8 max-w-4xl mx-auto">
    ...
    <!-- Teacher Info -->
    <div class="mb-6">
      <h2 class="text-xl mb-2">Maestro: <span class="font-bold">{{ teacher?.name }}</span></h2>
      ...
      <template v-for="(dayClasses, day) in classesByDay" :key="day">
        <div v-if="dayClasses.length > 0">
          ...
          <div class="space-y-4 pl-4">
            <div v-for="class_ in dayClasses" :key="class_.id" class="border-l-2 border-gray-200 pl-4">
              <div class="text-lg font-semibold">{{ class_.name }} - {{ class_.startTime }} a {{ class_.endTime }}</div>
    ...
    <div class="mt-8 pt-4 border-t text-sm text-gray-500">
      ...
      <p class="mt-2">Estos horarios pueden ser sujetos a cambios, si presenta algún inconveniente por favor preguntar en administración para ser gestionado correctamente.</p>
    </div>
  </div>
</template>
