    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input id="name" v-model="formData.name" type="text" required class="form-input" :disabled="props.isLoading" />
        </div>
        <div>
          <label for="email" class="form-label">Email</label>
          <input id="email" v-model="formData.email" type="email" required class="form-input" :disabled="props.isLoading" />
        </div>
        <div>
          <label for="phone" class="form-label">Teléfono</label>
          <input id="phone" v-model="formData.phone" type="tel" required class="form-input" :disabled="props.isLoading" />
        </div>
        <div>
          <label for="address" class="form-label">Dirección</label>
          <input id="address" v-model="formData.address" type="text" class="form-input" :disabled="props.isLoading" />
        </div>
      </div>
      <!-- Puedes agregar aquí el resto de campos y secciones del formulario -->
      <!-- ...existing code... -->
      <div class="flex justify-end gap-3">
        <button type="button" class="btn btn-secondary" @click="handleCancel" :disabled="props.isLoading">Cancelar</button>
        <button type="submit" class="btn btn-primary" :disabled="props.isLoading">{{ props.isLoading ? 'Guardando...' : 'Guardar' }}</button>
      </div>
    </form>
  </div>
</template>
