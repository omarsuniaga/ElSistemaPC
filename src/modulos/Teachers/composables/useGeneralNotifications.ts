...
export function useGeneralNotifications() {
  ...
  const studentRegistrationNotifications = computed(() =>
    validNotifications.value.filter(
      (n) => n.type === 'student-registration' && n.status === 'unread',
    ),
  );
  ...
  const setupRealtimeListener = () => {
    ...
    unsubscribe = subscribeToGeneralNotifications(authStore.user.uid, (generalNotifications) => {
