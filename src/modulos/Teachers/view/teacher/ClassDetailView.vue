...
<template>
  <div class="container mx-auto px-4 py-6">
    ...
    <!-- Error -->
    <div v-else-if="error" class="bg-red-100 dark:bg-red-900 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-300 px-4 py-3 rounded relative mb-4">
      <strong class="font-bold">Error:</strong>
      <span class="block sm:inline"> {{ error }}</span>
    </div>
    
    <!-- Contenido principal -->
    <div v-else-if="classData" class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
      ...
      <div class="p-6">
        ...
        <div class="mb-6">
          ...
          <div v-if="classData.schedule && classData.schedule.slots && classData.schedule.slots.length > 0" 
               class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            <div v-for="(slot, index) in classData.schedule.slots" :key="index" 
              ...
              <div class="flex flex-wrap items-center justify-between">
                ...
                <div class="flex items-center text-gray-600 dark:text-gray-400">
                  ...
                  <span class="ml-2 text-sm">({{ calculateDuration(slot.startTime, slot.endTime) }})</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <div v-if="classStudents.length > 0" class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            <div v-for="student in classStudents" :key="student.id" 
              ...
              <div @click="toggleStudentExpand(student.id)" 
                ...
                <div class="flex items-center">
                  ...
                  <div>
                    <div class="font-medium text-gray-800 dark:text-gray-200">{{ student.name }}</div>
                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ student.email || 'Sin correo' }}</div>
                  </div>
                </div>
                
                <div class="flex items-center">
                  <div v-if="student.instruments && student.instruments.length > 0" 
                       class="mr-3 px-2 py-1 bg-indigo-100 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200 text-xs rounded-full">
                    {{ student.instruments.length }} {{ student.instruments.length === 1 ? 'instrumento' : 'instrumentos' }}
                  </div>
                  <ChevronDownIcon v-if="expandedStudentId !== student.id" class="w-5 h-5 text-gray-400" />
                  <ChevronUpIcon v-else class="w-5 h-5 text-gray-400" />
                </div>
              </div>
              
              <!-- Detalles expandibles del estudiante -->
              <div v-if="expandedStudentId === student.id" class="px-3 pb-3 pt-1 border-t border-gray-100 dark:border-gray-700">
                <!-- Instrumentos -->
                <div v-if="student.instruments && student.instruments.length > 0" class="mb-3">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <div v-for="instrument in student.instruments" :key="instrument.id" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                  ...
                  <div v-if="student.address" class="text-gray-600 dark:text-gray-400 md:col-span-2">
          ...
          <div v-else class="text-gray-500 dark:text-gray-400 italic bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
