<template>
  <div class="teacher-dashboard">
    ...
    >
      ...
      <!-- Tabs de navegación -->
      <div
        class="flex flex-wrap mt-4 md:mt-6 border-b border-gray-200 dark:border-gray-700 overflow-x-auto"
      >
        <button
    ...
    <section v-else class="dashboard-content space-y-4 md:space-y-6">
      ...
      <div
        ...
        class="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4"
      >
        <h2
          class="text-base md:text-lg font-semibold mb-3 md:mb-4 flex justify-between items-center"
        >
      ...
      >
        ...
        <div
          class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 p-3 md:p-4 flex justify-between items-center"
        >
          <div class="flex items-center gap-2">
...
const getDayOrder = (classItem) => {
  ...
  return normalizedDay in dayOrder
    ...
    : typeof day === 'number' && day >= 0 && day <= 6
      ? day === 0
        ? 7
        : day // Convertir 0 (domingo) a 7 para el orden correcto
...
const handleStudentChange = async (studentIds) => {
  try {
    ...
    if (validatedIds.length !== validStudentIds.length) {
      console.warn(
        `Se eliminaron ${validStudentIds.length - validatedIds.length} IDs de estudiantes inválidos`,
      );
    }
      ...
      try {
        ...
        if (student) {
          ...
          if (!currentGrupos.includes(selectedClassId.value)) {
            ...
            console.log(
              `Actualizado grupo del estudiante ${studentId}: añadido a clase ${selectedClassId.value}`,
            );
          }
        }
      } catch (err) {
    ...
    toast({
      title: 'Error',
      description:
        error.message || 'No se pudieron actualizar los estudiantes. Intente nuevamente.',
      variant: 'destructive',
    });
  }
};
    ...
    if (typeof teachersStore.fetchTeachers === 'function') {
      ...
      if (authStore.user?.uid) {
        ...
        if (teacher) {
          ...
          console.warn(
            '⚠️ No se encontró un maestro con el UID de autenticación:',
            authStore.user.uid,
          );
