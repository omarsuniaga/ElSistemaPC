...
const handleView = async (teacher: any) => {
  ...
  try {
    // Ensure we have the latest data for this teacher
    const teacherData = teachersStore.teachers.find(t => t.id === teacher.id)
    if (teacherData) {
    ...
    error.value = 'Error al cargar los datos del maestro'
  }
}
