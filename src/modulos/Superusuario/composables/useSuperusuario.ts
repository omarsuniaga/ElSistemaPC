...
import { UserRole, ResourceType, PermissionAction, hasPermission } from '@/modulos/Auth/types/permissions';
import {
  SystemConfiguration,
  RoleConfiguration,
  UserRoleAssignment,
...
export function useSuperusuario() {
  ...
  const auditLogs = ref<AuditLog[]>([]);  const systemMetrics = ref<SystemMetrics[]>([]);
  ...
  const canManageSystem = computed(() => 
    currentRole.value ? hasPermission(currentRole.value, ResourceType.SYSTEM_CONFIGURATION, PermissionAction.UPDATE) : false
  );
  
  const canManageRoles = computed(() => 
    currentRole.value ? hasPermission(currentRole.value, ResourceType.USER_ROLES_MANAGEMENT, PermissionAction.UPDATE) : false
  );
  
  const canManagePermissions = computed(() => 
    currentRole.value ? hasPermission(currentRole.value, ResourceType.PERMISSION_MANAGEMENT, PermissionAction.UPDATE) : false
  );
  
  const canViewAuditLogs = computed(() => 
    currentRole.value ? hasPermission(currentRole.value, ResourceType.AUDIT_LOGS, PermissionAction.READ) : false
  );
  
  const canManageBackups = computed(() => 
    currentRole.value ? hasPermission(currentRole.value, ResourceType.BACKUP_RESTORE, PermissionAction.CREATE) : false
  );
  
  const canConfigureModules = computed(() => 
    currentRole.value ? hasPermission(currentRole.value, ResourceType.MODULE_CONFIGURATION, PermissionAction.UPDATE) : false
  );
  
  const isSuperusuario = computed(() => 
    authStore.user?.role === UserRole.SUPERUSUARIO
  );
  ...
  async function loadModuleConfigurations() {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Error al cargar configuraciones de módulos';
  ...
  async function updateModuleConfiguration(id: string, updates: Partial<ModuleConfiguration>) {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Error al actualizar configuración de módulo';
