      ...
      <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
        ...
        <div v-if="systemStatus" class="grid grid-cols-3 gap-4">
          ...
          <div class="text-center p-3 bg-green-100 rounded">
            <div class="text-2xl font-bold text-green-600">{{ systemStatus.permissions.count }}</div>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <div class="text-sm text-orange-100">Sobrescribir con configuración por defecto</div>
              </button>
                <button 
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <div class="text-sm text-purple-100">Configurar maestros para ver todos los estudiantes</div>
              </button>
            </div>
          </div>
        </div>
...
// Methods
const addLog = (message: string, type: 'info' | 'success' | 'error' | 'warning' = 'info', details?: string) => {
...
const forceReinitialize = async () => {
  ...
  try {
    ...
    addLog('Reinicialización forzada completada', 'warning', 'Toda la configuración anterior fue sobrescrita')
  } catch (error) {
...
const testConnection = async () => {
  try {
    ...
    addLog('Conexión con Firestore exitosa', 'success', 'Lectura de roles, permisos y navegación completada')
  } catch (error) {
...
const runDiagnostics = async () => {
  try {
    ...
    addLog('Diagnóstico ejecutado', 'info', 
...
const updateTeacherStudentAccess = async () => {
  try {
    ...
    await rbacManagement.updateTeacherStudentViewPermission('maestro', teacherStudentAccess.value.allStudents)
...
const updateNavigationAccess = async () => {
  try {
    ...
    const navigationUpdates = [{
      ...
      roles: teacherStudentAccess.value.studentMenu ? ['Maestro', 'Maestro Avanzado'] : ['Maestro Avanzado']
    }]
    
    await rbacManagement.updateNavigationForRole('Maestro', navigationUpdates, authStore.user?.uid || 'system')
