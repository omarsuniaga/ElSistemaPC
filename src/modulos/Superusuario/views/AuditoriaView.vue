  ...
  <div class="auditoria-view">
    ...
    <div class="bg-white rounded-b-lg shadow-lg">
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex flex-wrap gap-4 items-center justify-between">
          <div class="flex flex-wrap gap-4">
            ...
            <!-- Action Filter -->
            <select v-model="filters.action" class="px-3 py-2 border border-gray-300 rounded-md text-sm">
            ...
            <!-- Level Filter -->
            <select v-model="filters.level" class="px-3 py-2 border border-gray-300 rounded-md text-sm">
      ...
      <div v-else-if="error" class="p-6 text-center text-red-600">
        ❌ {{ error }}
        <button @click="refreshLogs" class="ml-2 text-blue-600 underline">Reintentar</button>
      </div>
              ...
              <div class="flex items-start space-x-4 flex-1">
                ...
                <div class="flex-1">
                  <div class="flex items-center space-x-2 mb-1">
                    <h3 class="font-medium text-gray-900">{{ getActionDescription(log.action) }}</h3>
                  ...
                  <div v-if="log.details" class="mt-3 p-3 bg-gray-50 rounded-md">
                    ...
                    <div v-if="expandedLogs.includes(log.id)" class="mt-2">
                      <pre class="text-xs text-gray-700 whitespace-pre-wrap">{{ JSON.stringify(log.details, null, 2) }}</pre>
                    </div>
                  </div>
                </div>
              </div>
...
const filteredLogs = computed(() => {
  return auditLogs.value.filter(log => {
    const matchesDateRange = (!filters.value.startDate || log.timestamp >= new Date(filters.value.startDate)) &&
    ...
    const matchesUser = !filters.value.userEmail || log.userEmail?.toLowerCase().includes(filters.value.userEmail.toLowerCase())
