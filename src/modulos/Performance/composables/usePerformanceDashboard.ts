...
export function usePerformanceDashboard() {
  ...
  const topPerformers = computed(() => {
    return [...students.value].sort((a, b) => b.overallScore - a.overallScore).slice(0, 10);
  });
  ...
  const summary = computed((): PerformanceSummary => {
    ...
    const averageAttendance =
      students.value.reduce((sum, s) => sum + s.attendanceMetrics.attendanceRate, 0) / total;

    const classifications = students.value.reduce(
      (acc, s) => {
    ...
    const trends = students.value.reduce(
      (acc, s) => {
