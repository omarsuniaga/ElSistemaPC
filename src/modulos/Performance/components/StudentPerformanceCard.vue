<template>
  <div 
    ...
    :class="[cardClass, props.compact ? 'p-3 space-y-3' : '', props.highlighted ? 'ring-2 ring-blue-500 shadow-lg' : '', (performance.scores.overallScore < 60 || performance.attendance.attendanceRate < 75) ? 'border-orange-300 bg-orange-50' : '']"
  >
    <!-- Header con avatar y info básica -->
    <div class="card-header flex items-center justify-between">
      <div class="student-info flex items-center space-x-3 flex-1 min-w-0">
        <StudentAvatar 
          :firstName="performance.studentName.split(' ')[0]" 
          :lastName="performance.studentName.split(' ').slice(1).join(' ')" 
          size="md"
          class="avatar"
        />
        <div class="student-details flex-1 min-w-0">
          <h3 class="student-name font-semibold text-gray-900 truncate">{{ performance.studentName }}</h3>
    ...
    <div v-if="hasAlerts || (showRecommendations && performance.recommendations && performance.recommendations.length > 0)" class="alerts-section border-t border-gray-100 pt-3 space-y-2">
      <!-- Alertas -->
    ...
    <!-- Acciones rápidas -->
    <div v-if="showActions" class="actions-section border-t border-gray-100 pt-3 flex items-center space-x-2">
      <button 
...
const hasAlerts = computed(() => {
  return props.performance.attendance.attendanceRate < 75 || 
