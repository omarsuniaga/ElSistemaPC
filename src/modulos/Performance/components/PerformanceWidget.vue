<template>
  <div class="performance-widget bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-200 ease-in-out hover:shadow-lg" :class="widgetClass">
    <!-- Header del widget -->
    <div class="widget-header flex justify-between items-start px-5 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
      <div class="widget-title-section flex-1">
        <h3 class="widget-title flex items-center gap-2 text-base font-semibold text-gray-800 dark:text-white">
    ...
    <div v-if="loading" class="widget-loading p-8 text-center">
      <div class="loading-content flex flex-col items-center gap-4">
        ...
        <span class="loading-text text-sm text-gray-600 dark:text-gray-400">Cargando rendimiento...</span>
      </div>
    </div>
    ...
    <div v-else-if="error" class="widget-error p-8 text-center">
      <div class="error-content flex flex-col items-center gap-4">
        ...
        <button class="retry-button px-4 py-2 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700 transition-colors" @click="refreshData">
          Reintentar
        </button>
      </div>
    </div>
    ...
    <div v-else-if="performance" class="widget-content p-5">
      <!-- Score principal -->
      <div class="main-score flex items-center gap-4 mb-6 pb-4 border-b border-gray-100 dark:border-gray-700">
        <div class="score-circle w-16 h-16 rounded-full flex flex-col items-center justify-center text-white flex-shrink-0" :class="scoreCircleClass">
          <span class="score-value text-2xl font-bold leading-none">{{ Math.round(performance.scores.overallScore) }}</span>
        ...
        <div class="metric-row grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
          <div class="metric-item bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
            <div class="metric-header flex items-center gap-2 mb-2">
              ...
              <span class="metric-label text-xs font-medium text-gray-700 dark:text-gray-300">Asistencia</span>
            </div>
            <div class="metric-value-row flex items-center gap-2">
              <div class="metric-bar flex-1 h-1 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                <div 
              ...
              <span class="metric-percentage text-xs font-semibold text-gray-700 dark:text-gray-300 min-w-[2rem] text-right">{{ performance.attendance.attendanceRate }}%</span>
            </div>
          </div>
          ...
          <div class="metric-item bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
            <div class="metric-header flex items-center gap-2 mb-2">
              ...
              <span class="metric-label text-xs font-medium text-gray-700 dark:text-gray-300">Repertorio</span>
            </div>
            <div class="metric-value-row flex items-center gap-2">
              <div class="metric-bar flex-1 h-1 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                <div 
              ...
              <span class="metric-percentage text-xs font-semibold text-gray-700 dark:text-gray-300 min-w-[2rem] text-right">{{ Math.round(performance.repertoire.averageScore) }}%</span>
            </div>
          </div>
        </div>
        ...
        <div class="metric-row grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div class="metric-item bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
            <div class="metric-header flex items-center gap-2 mb-2">
              ...
              <span class="metric-label text-xs font-medium text-gray-700 dark:text-gray-300">Trabajo Individual</span>
            </div>
            <div class="metric-value-row flex items-center gap-2">
              <div class="metric-bar flex-1 h-1 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                <div 
                  ...
                  :style="{ width: `${Math.min(performance.work.individualWork.practiceHours * 10, 100)}%` }"
                ></div>
              </div>
              <span class="metric-percentage text-xs font-semibold text-gray-700 dark:text-gray-300 min-w-[2rem] text-right">{{ performance.work.individualWork.practiceHours }}h</span>
            </div>
          </div>
          ...
          <div class="metric-item bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
            <div class="metric-header flex items-center gap-2 mb-2">
              ...
              <span class="metric-label text-xs font-medium text-gray-700 dark:text-gray-300">Trabajo Grupal</span>
            </div>
            <div class="metric-value-row flex items-center gap-2">
              <div class="metric-bar flex-1 h-1 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                <div 
              ...
              <span class="metric-percentage text-xs font-semibold text-gray-700 dark:text-gray-300 min-w-[2rem] text-right">{{ Math.round(performance.work.collectiveWork.teamworkScore) }}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Tendencia -->
      <div v-if="showTrend && performance.trends?.changeRate !== undefined" class="trend-section bg-gray-50 dark:bg-gray-700 p-3 rounded-lg mb-4">
        <div class="trend-header flex items-center gap-2 mb-2">
          <ChartTrendingUpIcon class="w-4 h-4 text-gray-500 dark:text-gray-400" />
          <span class="trend-label text-xs font-medium text-gray-700 dark:text-gray-300">Tendencia</span>
        </div>
        <div class="trend-indicator flex items-center gap-2" :class="trendClass">
          ...
          <div class="trend-value text-xs font-semibold px-1.5 py-0.5 rounded-md" :class="trendValueClass">
      ...
      <div v-if="highlights.length > 0 || alerts.length > 0" class="highlights-section mb-4">
        ...
        <div v-if="highlights.length > 0" class="highlights mb-3">
          <div class="highlights-header flex items-center gap-2 mb-2">
            ...
            <span class="highlights-label text-xs font-medium text-gray-700 dark:text-gray-300">Destacados</span>
          </div>
        ...
        <div v-if="alerts.length > 0" class="alerts">
          <div class="alerts-header flex items-center gap-2 mb-2">
            ...
            <span class="alerts-label text-xs font-medium text-gray-700 dark:text-gray-300">Atención</span>
          </div>
      ...
      <div v-if="showRecentComments && recentComments.length > 0" class="recent-comments mb-4">
        <div class="comments-header flex items-center gap-2 mb-2">
          ...
          <span class="comments-label text-xs font-medium text-gray-700 dark:text-gray-300">Comentarios Recientes</span>
        </div>
        <div class="comments-list flex flex-col gap-3">
          ...
          >
            <div class="comment-meta flex justify-between items-center mb-1">
              <span class="teacher-name text-xs font-semibold text-gray-800 dark:text-gray-200">{{ comment.teacherName }}</span>
              <span class="comment-date text-xs text-gray-500 dark:text-gray-400">{{ formatCommentDate(comment.date) }}</span>
            </div>
            <div class="comment-text text-xs text-gray-600 dark:text-gray-400 mb-1.5 leading-snug">{{ comment.comment }}</div>
            <div class="comment-category inline-block px-2 py-0.5 rounded-full text-xs font-medium" :class="getCategoryClass(comment.category)">
              {{ comment.category }}
            </div>
          </div>
        </div>
      ...
      <!-- Actions footer -->
      <div v-if="showActions" class="widget-footer flex gap-2 pt-4 border-t border-gray-100 dark:border-gray-700">
        <button 
    ...
    <div v-else class="widget-empty p-8 text-center">
      <div class="empty-content flex flex-col items-center gap-4">
        ...
        <span class="empty-text text-sm text-gray-500 dark:text-gray-400">No hay datos de rendimiento</span>
        <button class="load-button px-4 py-2 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700 transition-colors" @click="refreshData">
          Cargar datos
        </button>
      </div>
    </div>
...
const {
  ...
  performance,
  refresh: refreshData
} = useStudentPerformance(props.studentId);

// Computed properties
const widgetClass = computed(() => ({
  'border-l-4 border-yellow-400 dark:border-yellow-500 bg-gradient-to-r from-yellow-50 dark:from-yellow-900/30 to-white dark:to-gray-800': performance.value && (
...
const trendIconComponent = computed(() => {
  if (!performance.value || performance.value.trends?.changeRate === undefined) return SolidMinusIcon;
...
const trendClass = computed(() => {
  if (!performance.value || performance.value.trends?.changeRate === undefined) return 'text-gray-500 dark:text-gray-400';
  const trend = performance.value.trends.changeRate;
...
const trendValueClass = computed(() => {
  if (!performance.value || performance.value.trends?.changeRate === undefined) return 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300';
  const trend = performance.value.trends.changeRate;
...
const getWorkColor = (value: number, isHours = true) => { // Assuming practiceHours for individual, score for collective
...
const categoryColorMap: Record<string, { bg: string; text: string; border: string }> = {
  'técnica': { bg: 'bg-blue-100 dark:bg-blue-700', text: 'text-blue-700 dark:text-blue-200', border: 'border-blue-500' },
  'expresión': { bg: 'bg-pink-100 dark:bg-pink-700', text: 'text-pink-700 dark:text-pink-200', border: 'border-pink-500' },
  'comportamiento': { bg: 'bg-green-100 dark:bg-green-700', text: 'text-green-700 dark:text-green-200', border: 'border-green-500' },
  'progreso': { bg: 'bg-yellow-100 dark:bg-yellow-700', text: 'text-yellow-700 dark:text-yellow-200', border: 'border-yellow-500' },
  'liderazgo': { bg: 'bg-indigo-100 dark:bg-indigo-700', text: 'text-indigo-700 dark:text-indigo-200', border: 'border-indigo-500' },
  'creatividad': { bg: 'bg-purple-100 dark:bg-purple-700', text: 'text-purple-700 dark:text-purple-200', border: 'border-purple-500' },
  default: { bg: 'bg-gray-100 dark:bg-gray-600', text: 'text-gray-700 dark:text-gray-300', border: 'border-gray-400' }
};
