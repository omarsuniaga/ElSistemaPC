<template>
  <div class="space-y-6 repertoire-analysis">
    <div class="space-y-2 analysis-header">
      <h4 class="text-lg font-semibold text-gray-900 dark:text-white analysis-title">Análisis de Repertorio</h4>
    ...
    <div class="space-y-6 analysis-content">
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-3 stats-grid">
        <div class="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg stat-card">
          ...
          <div>
            <p class="text-2xl font-bold text-gray-900 dark:text-white stat-value">{{ data?.piecesLearned || 5 }}</p>
        ...
        <div class="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg stat-card">
          ...
          <div>
            <p class="text-2xl font-bold text-gray-900 dark:text-white stat-value">{{ data?.difficulty || 'Intermedio' }}</p>
      ...
      <div class="space-y-4 pieces-section">
        <h5 class="text-base font-semibold text-gray-900 dark:text-white section-title">Repertorio Actual</h5>
        ...
        <div class="space-y-4 pieces-list">
          ...
          >
            <div class="flex items-start justify-between piece-header">
              <div>
                <h6 class="font-semibold text-gray-900 dark:text-white piece-title">{{ piece.title }}</h6>
                <p class="text-sm text-gray-600 dark:text-gray-400 piece-composer">{{ piece.composer }}</p>
              </div>
              <div class="flex-shrink-0 piece-status">
                <span 
                  :class="[
                    'status-badge px-2 py-1 text-xs font-medium rounded-full',
                    piece.status === 'completed' ? 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300' :
                    piece.status === 'in-progress' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
                  ]"
                >
            ...
            <div class="flex items-center gap-3 piece-progress">
              <div class="flex-1 h-2 overflow-hidden bg-gray-200 rounded-full dark:bg-gray-700 progress-bar">
                <div 
                  class="h-full transition-all duration-300 bg-blue-500 progress-fill"
                  :style="{ width: `${piece.progress}%` }"
                ></div>
              </div>
              <span class="text-sm font-medium text-gray-600 dark:text-gray-400 progress-text">{{ piece.progress }}%</span>
            </div>
      ...
      <div class="space-y-4 skills-section">
        <h5 class="text-base font-semibold text-gray-900 dark:text-white section-title">Desarrollo de Habilidades</h5>
        
        <div class="space-y-4 skills-grid">
          <div 
            v-for="skill in skills"
            :key="skill.name"
            class="space-y-2 skill-item"
          >
            <div class="flex items-center justify-between skill-header">
              <span class="font-medium text-gray-900 dark:text-white skill-name">{{ skill.name }}</span>
              <span class="text-sm font-semibold text-gray-600 dark:text-gray-400 skill-score">{{ skill.score }}/10</span>
            </div>
            
            <div class="w-full h-2 overflow-hidden bg-gray-200 rounded-full dark:bg-gray-700 skill-bar">
              <div 
