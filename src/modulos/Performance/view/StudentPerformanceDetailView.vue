<template>
  <div class="performance-detail-view bg-gray-50 dark:bg-gray-900 min-h-screen">
    <!-- Header -->
    <div class="header bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
      <div class="header-content max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        ...
        <div class="header-info space-y-4">
          <h1 class="title text-2xl font-bold text-gray-900 dark:text-white">Análisis de Rendimiento</h1>
          <div v-if="student" class="student-info flex items-center">
            ...
            />
            <div>
              <h2 class="student-name text-lg font-semibold text-gray-900 dark:text-white">{{ student.nombre }} {{ student.apellido }}</h2>
              <p class="student-meta text-sm text-gray-600 dark:text-gray-400">{{ student.instrumento }} • {{ student.clase }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="content max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div v-if="loading" class="loading-state text-center py-12">
        <div class="spinner animate-spin h-8 w-8 border-2 border-blue-500 border-t-transparent rounded-full mx-auto mb-4"></div>
      ...
      <div v-else-if="performance" class="performance-content space-y-8">
        ...
        <div class="summary-card bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <h3 class="section-title flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-white mb-6">
            <ChartBarIcon class="w-5 h-5" />
            Resumen General
          </h3>
          
          <div class="performance-overview grid lg:grid-cols-2 gap-8">
            <div class="score-display flex items-center gap-6">              <div class="score-circle relative w-24 h-24">
                <div 
                ...
                >
                  <div class="score-inner absolute inset-2 bg-white dark:bg-gray-800 rounded-full flex flex-col items-center justify-center">
                    <span class="score-value text-2xl font-bold text-gray-900 dark:text-white">{{ Math.round(performance.scores?.overallScore || 0) }}</span>
                    <span class="score-label text-sm text-gray-600 dark:text-gray-400">%</span>
                  </div>
                </div>
              </div>
              <div class="score-info space-y-1">
                <h4 class="score-title text-lg font-semibold text-gray-900 dark:text-white">Rendimiento General</h4>
                <p class="score-description text-gray-600 dark:text-gray-400">{{ getPerformanceLevel(performance.scores?.overallScore || 0) }}</p>
              </div>
            </div>

            <div class="metrics-grid grid grid-cols-2 gap-4">
              <div class="metric-item flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <CalendarIcon class="w-6 h-6 text-blue-500" />                <div>
                  <p class="metric-label text-sm text-gray-600 dark:text-gray-400">Asistencia</p>
                  <p class="metric-value text-lg font-semibold text-gray-900 dark:text-white">{{ Math.round(performance.attendance?.attendanceRate || 0) }}%</p>
                </div>
              </div>
              ...
              <div class="metric-item flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div>
                  ...
                  <p class="metric-value text-lg font-semibold text-gray-900 dark:text-white">{{ Math.round(performance.repertoire?.averageScore || 0) }}%</p>
                </div>
              </div>
              ...
              <div class="metric-item flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div>
                  ...
                  <p class="metric-value text-lg font-semibold text-gray-900 dark:text-white">{{ Math.round(performance.scores?.workScore || 0) }}%</p>
                </div>
              </div>
              ...
              <div class="metric-item flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div>
                  <p class="metric-label text-sm text-gray-600 dark:text-gray-400">Retroalimentación</p>
                  <p class="metric-value text-lg font-semibold text-gray-900 dark:text-white">{{ Math.round(performance.scores?.behaviorScore || 0) }}%</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="analysis-tabs bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div class="tab-content p-6">            <!-- Attendance Analysis -->
            <div v-if="activeTab === 'attendance'" class="tab-panel">              <AttendanceAnalysis 
            ...
            <!-- Repertoire Analysis -->
            <div v-if="activeTab === 'repertoire'" class="tab-panel">              <RepertoireAnalysis 
                :student-id="studentId"
                :data="repertoireData"
              />
            </div>
            ...
            <!-- Work Analysis -->
            <div v-if="activeTab === 'work'" class="tab-panel">              <WorkAnalysis 
                :student-id="studentId"
                :data="performance.work"
              />
            </div>

            <!-- Feedback Analysis -->
            <div v-if="activeTab === 'feedback'" class="tab-panel">              <FeedbackAnalysis 
                :student-id="studentId"
        ...
        <div class="recommendations-card bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <h3 class="section-title flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-white mb-6">
          ...
          <div v-if="generatedRecommendations.length" class="recommendations-list space-y-4">
            ...
            >
              <div class="recommendation-icon flex-shrink-0 w-10 h-10 bg-blue-100 dark:bg-blue-900 rounded-lg flex items-center justify-center text-blue-600 dark:text-blue-400">
                <component :is="getRecommendationIcon(recommendation.type)" class="w-5 h-5" />
              </div>
              <div class="recommendation-content flex-1 space-y-2">
                <h4 class="recommendation-title font-semibold text-gray-900 dark:text-white">{{ recommendation.title }}</h4>
                <p class="recommendation-description text-gray-600 dark:text-gray-400">{{ recommendation.description }}</p>
                <div v-if="recommendation.actions?.length" class="recommendation-actions flex gap-2 mt-3">
                  <button
...
const generatedRecommendations = computed(() => {
  ...
  if (attendance.attendanceRate < 80) {
    recommendations.push({
      ...
      description: 'Se recomienda mejorar la asistencia a clases para obtener mejor aprovechamiento.',
      actions: [
  ...
  if (scores.repertoireScore < 70) {
    recommendations.push({
      ...
      description: 'Se sugiere aumentar las horas de práctica individual para mejorar el rendimiento en repertorio.',
      actions: [
...
const attendanceData = computed(() => {
  ...
  return {
    ...
    trend: performance.value.trends.direction === 'mejorando' ? 'improving' : 
           performance.value.trends.direction === 'decayendo' ? 'declining' : 'stable'
  }
})
...
const repertoireData = computed(() => {
  if (!performance.value) return undefined
  const repertoire = performance.value.repertoire
  const difficultyScore = Math.round((repertoire.technicalProficiency + repertoire.musicalExpression) / 2)
...
const getRecommendationIcon = (type: string) => {
  switch (type) {
    ...
    case 'practice': return MusicalNoteIcon
