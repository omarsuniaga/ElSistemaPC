...
export const usePerformanceStore = defineStore('performance', () => {
  ...
  const topPerformers = computed(() => {
    return getAllStudents.value
      .sort((a, b) => b.overallScore - a.overallScore)
      .slice(0, 10);
  });

  const studentsNeedingAttention = computed(() => {
    return getAllStudents.value.filter(s => 
  ...
  const performanceSummary = computed((): PerformanceSummary => {
    ...
    const averageAttendance = studentsArray.reduce((sum, s) => sum + s.attendanceMetrics.attendanceRate, 0) / total;
    
    const classifications = studentsArray.reduce((acc, s) => {
      acc[s.classification] = (acc[s.classification] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    
    const trends = studentsArray.reduce((acc, s) => {
  ...
  const fetchStudentPerformance = async (studentId: string, forceRefresh = false) => {
    ...
    try {
      ...
      const [attendanceQuery, montajesQuery, observationsQuery, workMetricsQuery] = await Promise.all([
        getCollection('attendance', [{ field: 'studentId', operator: '==', value: studentId }]),
        getCollection('montajes', [{ field: 'participantes', operator: 'array-contains', value: studentId }]),
        getCollection('teacherObservations', [{ field: 'studentId', operator: '==', value: studentId }]),
  ...
  const getPerformanceStats = (studentIds?: string[]) => {
    ...
    return {
      ...
      classifications: targetStudents.reduce((acc, s) => {
        acc[s.classification] = (acc[s.classification] || 0) + 1;
        return acc;
      }, {} as Record<string, number>)
    };
