...
import { getAttendanceDocumentFirebase, saveAttendanceDocumentFirebase, getAllAttendanceDocumentsFirebase, updateObservationsFirebase } from './attendance';
...
export const getAllAttendanceDocumentsOffline = async (): Promise<AttendanceDocument[]> => {
  try {
    // Intentar obtener del caché primero
    const cachedDocs = CacheManager.getFromCache<AttendanceDocument[]>(
      CACHE_PREFIX.ALL_ATTENDANCE
    );
    ...
    if (OfflineMode.isOfflineMode()) {
      console.log(`[Offline] En modo offline, usando datos en caché${hasPendingChanges ? ' (con cambios pendientes)' : ''}`);
      return cachedDocs || [];
    }
...
export const updateObservationsOffline = async (
  ...
  try {
    ...
    // Si estamos offline, guardar para sincronización posterior
    if (OfflineMode.isOfflineMode()) {
      console.log(`[Offline] Guardando observaciones offline para sincronización posterior: ${docId}`);
...
export const addJustificationOffline = async (
  ...
  try {
    ...
    // Si estamos offline, guardar para sincronización posterior
    if (OfflineMode.isOfflineMode()) {
      console.log(`[Offline] Guardando justificación offline para sincronización posterior: ${docId}`);
