 ...
 */
export const registerEmergencyClassFirebase = async (
  emergencyClass: Omit<EmergencyClass, 'id' | 'createdAt' | 'status'>,
): Promise<string> => {
  try {
 ...
 */
export const getEmergencyClassesFirebase = async (
 ...
 */
export const getEmergencyClassesByTeacherFirebase = async (
  teacherId: string,
): Promise<EmergencyClass[]> => {
  try {
    // Intenta primero con la consulta óptima (con índice compuesto)
    try {
      ...
      // Si hay un error de índice, intentar con una consulta más simple
      console.warn(
        'Error de índice en Firestore, usando consulta alternativa para clases del profesor:',
        indexError,
      );
 ...
 */
export const getEmergencyClassByIdFirebase = async (
  emergencyClassId: string,
): Promise<EmergencyClass | null> => {
  try {
    ...
    if (!docSnap.exists()) {
      console.log(
        '[EmergencyClassService] No se encontró la clase emergente con ID:',
        emergencyClassId,
      );
      return null;
    }
