// src/modulos/Attendance/service/firebase/attendanceFirebase.ts
import {
  collection,
  doc,
  getDocs,
...
import { db, storage } from '../../../../firebase'; // Adjusted path
import type {
  AttendanceDocument,
  JustificationData,
  AttendanceRecord,
  ...
  ObservationRecord,
  AttendanceStatus,
} from '../../types/attendance'; // Adjusted path
import { auth } from '../../../../firebase'; // Adjusted path
import { fetchAttendanceByDateFirebase as fetchByDate } from './attendance/fetchByDate'; // Keep as is for now
import { getAllAttendanceDocumentsFirebase } from "../../services/firebase"; // Keep as is for now
...
export const addJustificationToAttendanceFirebase = async (
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      if (
        !data.data.presentes.includes(justification.id) &&
 ...
 */
export const updateAttendanceFirebase = async (
  ...
  try {
    ...
    document.data.presentes = document.data.presentes.filter(
      (id) => id !== attendanceData.studentId,
    );
    ...
    if (attendanceData.status === 'Justificado') {
      ...
      const justificationReason =
        typeof justificationData === 'string'
          ? justificationData
      ...
      const justIndex = document.data.justificacion.findIndex(
        (j) => j.id === attendanceData.studentId,
      );
      ...
      if (justIndex === -1) {
        ...
        document.data.justificacion.push({
          ...
          documentUrl:
            (typeof justificationData !== 'string' ? justificationData?.documentUrl : null) || null,
          approvalStatus: 'pending',
          createdAt: new Date(),
          timeLimit: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days from now
        });
 ...
 */
export const registerAttendanceFirebase = async (
  attendanceData: AttendanceRecord,
): Promise<string> => {
 ...
 */
export const fetchAttendanceByDateRangeFirebase = async (
  startDate: string,
  endDate: string,
): Promise<AttendanceRecord[]> => {
  try {
 ...
 */
export const getAttendanceStatusFirebase = async (
  studentId: string,
  date: string,
  classId?: string,
): Promise<string> => {
  try {
    ...
    if (classId) {
      q = query(attendanceRef, where('fecha', '==', date), where('classId', '==', classId));
    } else {
      q = query(attendanceRef, where('fecha', '==', date));
    }
    ...
    for (const doc of querySnapshot.docs) {
      ...
      if (data && data.data) {
        ...
        if (data.data.tarde?.includes(studentId)) {
          ...
          const hasJustification = data.data.justificacion?.some(
            (j: JustificationData) => j.id === studentId,
          );
...
export async function addObservationToHistoryFirebase(
  ...
  try {
    ...
    const queryConditions = [where('classId', '==', classId), where('date', '==', date)];
    ...
    const existingQuery = query(collection(db, OBSERVATIONS_COLLECTION), ...queryConditions);
...
export async function fetchAttendanceRecords(startDate?: string, endDate?: string) {
  console.warn(
    '⚠️ fetchAttendanceRecords está obsoleta. Use fetchAttendanceByDateRangeFirebase en su lugar.',
  );
 ...
 */
export async function getTeacherAttendanceDocsFirebase(
  teacherId: string,
  fromDate: string,
  toDate: string,
...
export async function getStudentAttendanceByDate(
  ...
  try {
    const snapshot = await getDocs(
      query(attendanceCollection, where('studentId', '==', studentId), where('date', '==', date)),
    );
...
async function validateTeacherPermissions(
  ...
  try {
    ...
    const { canTeacherRecordAttendance, canTeacherAddObservations, canTeacherViewAttendanceHistory } =
      await import('../../../Classes/service/classes'); // Adjusted path
...
export const fetchAttendanceByDateRangeAndClassesFirebase = async (
  ...
  try {
    ...
    console.log(
      `🔍 Buscando asistencias para ${classIds.length} clases entre ${startDate} y ${endDate}`,
    );
    ...
    allSnapshot.docs.forEach((doc) => {
      ...
      if (documentClassId && classIds.includes(documentClassId)) {
        ...
        if (data.data) {
          ...
          if (attendanceData.presentes && Array.isArray(attendanceData.presentes)) {
            console.log(
              `👥 Presentes (${attendanceData.presentes.length}):`,
          ...
          if (attendanceData.justificacion && Array.isArray(attendanceData.justificacion)) {
            console.log(
              `📝 Justificados (${attendanceData.justificacion.length}):`,
              attendanceData.justificacion,
            );
