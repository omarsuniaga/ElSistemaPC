...
export async function getAttendanceDocumentFirebase(docId: string): Promise<AttendanceDocument | null> {
...
export async function addObservationToHistoryFirebase(observation: ClassObservation): Promise<string> {
...
export async function getAttendanceStatusFirebase(
  ...
  const querySnapshot = await getDocs(q);
  return querySnapshot.empty ? null : { id: querySnapshot.docs[0].id, ...querySnapshot.docs[0].data() } as AttendanceRecord;
}
