...
class OptimizedAttendanceQueries {
  ...
  ): Promise<{ documents: AttendanceDocument[]; lastDoc: QueryDocumentSnapshot<DocumentData> | null; hasMore: boolean }> {
    try {
      console.log('[OptimizedQueries] Getting attendance documents:', { startDate, endDate, pageSize });
  ...
  async getAttendanceStatsCached(
    ...
    try {
      ...
      const averageAttendance = totalPossibleSessions > 0 ? 
        (totalPresentSessions / totalPossibleSessions) * 100 : 0;
