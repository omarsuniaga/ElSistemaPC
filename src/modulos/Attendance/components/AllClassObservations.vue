<template>
  <div class="class-observations-container">
    ...
    <div v-else-if="error" class="p-6 bg-red-50 dark:bg-red-900/20 rounded-lg text-center mb-6">
      ...
      <button @click="fetchAllObservations" class="mt-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors">
        Reintentar
      </button>
    </div>
    ...
    <div v-else-if="observations.length === 0" class="text-center py-12">
      ...
      <h3 class="text-xl font-medium text-gray-700 dark:text-gray-300">No hay observaciones registradas</h3>
    ...
    <div v-else class="mb-6 flex flex-wrap gap-4">
      ...
      <div class="flex-grow">
        <label for="date-filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Filtrar por fecha</label>
