...
<template>
  ...
  <div
    ...
    class="my-2 p-2 bg-yellow-50 border border-yellow-200 rounded-md text-sm"
  >
    <p class="font-medium text-yellow-800 flex items-center justify-between">
      <span>
        <span class="w-2 h-2 bg-amber-500 rounded-full animate-pulse inline-block mr-1" />
  ...
  <div v-if="Object.keys(attendanceRecords).length > 0" class="mb-4 flex flex-wrap gap-2 text-sm">
    ...
    <div
      v-if="hasPendingChanges"
      class="bg-amber-100 text-amber-800 px-3 py-1 rounded-full flex items-center"
    >
      <span class="w-2 h-2 bg-amber-500 rounded-full animate-pulse mr-1" />
      Cambios pendientes: {{ pendingChangesCount }}
    </div>
  </div>
</template>
