<template>
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        <label for="instrumentFilter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
    ...
    <div v-else-if="filteredStudents.length === 0" class="text-center py-8 text-gray-500 dark:text-gray-400">
    ...
    <div v-if="selectedStudents.length > 0" class="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-md p-3">
...
const filteredStudents = computed(() => {
  ...
  if (filters.value.level) {
    students = students.filter(student => 
      student.nivel === filters.value.level
    )
  }
...
const availableInstruments = computed(() => {
  const instruments = [...new Set(props.availableStudents
    .map(student => student.instrumento)
    .filter(Boolean)
  )]
...
const availableLevels = computed(() => {
  const levels = [...new Set(props.availableStudents
