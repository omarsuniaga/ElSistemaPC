...
const pendingJustifications = ref<
  Map<string, {id?: string; reason: string; documentURL?: string; timestamp?: Date}>
>(new Map())
...
const selectedStudentForJustification = ref<{id: string; nombre: string; apellido: string} | null>(
  null
)
...
const verifyClassExists = async (classId: string) => {
  ...
  try {
    ...
    if (!classData) {
      console.log(
        `[ClassDebug] Clase no encontrada individualmente, refrescando todas las clases...`
      )
    ...
    if (!classData) {
      console.log(
        `[ClassDebug] Clase no encontrada en clases regulares, buscando en clases emergentes...`
      )
      try {
        ...
        if (classData) {
          ...
          if (classData.teacherId === currentTeacherId) {
            console.log(
              `[ClassDebug] ✅ Maestro ${currentTeacherId} es titular de la clase emergente ${classId}`
            )
            return true
          } else {
            console.log(
              `[ClassDebug] ❌ Maestro ${currentTeacherId} no es titular de la clase emergente (titular: ${classData.teacherId})`
            )
            return false
          }
        }
    ...
    if (!classData) {
      console.error(
        `[ClassDebug] Clase con ID=${classId} no encontrada ni en clases regulares ni emergentes`
      )
      return false
    }

    console.log(
      `[ClassDebug] Clase encontrada: "${classData.name}", Titular: ${classData.teacherId}`
    )
    ...
    if (classData.teachers && Array.isArray(classData.teachers)) {
      ...
      if (isAssistant) {
        console.log(
          `[ClassDebug] ✅ Maestro ${currentTeacherId} es ASISTENTE autorizado en clase ${classId}`
        )
        return true
      }
    }
    ...
    if (hasAccess) {
      console.log(
        `[ClassDebug] ✅ Maestro ${currentTeacherId} tiene acceso verificado por store a clase ${classId}`
      )
      return true
    }
    ...
    console.log(
      `[ClassDebug] - No es asistente (${classData.teachers ? classData.teachers.length + " maestros" : "sin maestros adicionales"})`
    )
    return false
  } catch (error) {
...
const hasObservations = computed(() => {
  ...
  const todayObservations = allObservations.filter(
    (obs) => obs.classId === currentSelectedClass.value && obs.date === currentSelectedDate.value
  )
...
const effectiveAttendanceRecords = computed(() => {
  ...
  if (pendingChangesCount.value > 0) {
    console.log(
      `[AttendanceDebug] PRIORIDAD 1: Usando registros locales (cambios pendientes: ${pendingChangesCount.value})`
    )
...
const fetchDataForComponent = async (dateParam: string, classIdParam: string) => {
  ...
  if (normalizedDate.length === 8 && !normalizedDate.includes("-")) {
    ...
    if (debugEnabled) {
      console.log(
        `[AttendanceDebug] fetchDataForComponent: Date normalized from ${dateParam} to ${normalizedDate}`
      )
    }
  }
  ...
  try {
    ...
    const hasCachedStudents =
      localStudents.value.length > 0 && localStudents.value[0]?.classId === classIdParam
    const hasCachedAttendance =
      Object.keys(localAttendanceRecords.value).length > 0 &&
    ...
    if (!classInfo) {
      console.log(
        `[AttendanceDebug] fetchDataForComponent: Clase no encontrada en clases regulares, verificando clases emergentes...`
      )
      ...
      if (normalizedEmergencyData) {
        ...
        console.log(
          `[AttendanceDebug] fetchDataForComponent: Clase emergente encontrada: "${normalizedEmergencyData.name}"`
        )
      } else {
        // Intentar normalizar el ID y buscar de nuevo (por si hay problemas de formato)
        const normalizedId = classIdParam.trim()
        const alternativeClass = classesStore.classes.find(
          (c) => c.id.includes(normalizedId) || normalizedId.includes(c.id)
        )

        if (alternativeClass) {
          console.log(
            `[AttendanceDebug] fetchDataForComponent: Se encontro una clase similar con ID=${alternativeClass.id}`
          )
    ...
    if (!hasCachedStudents) {
      ...
      console.log(
        `[AttendanceDebug] fetchDataForComponent: 📋 Total estudiantes obtenidos: ${studentsInClass.length}`
      )
    ...
    if (!hasCachedAttendance) {
      console.log(
        "[AttendanceDebug] fetchDataForComponent: 🔄 CARGANDO DOCUMENTO DE ASISTENCIA DE FIRESTORE"
      )
      ...
      console.log(
        "[AttendanceListDebug] fetchDataForComponent: attendanceStore.fetchAttendanceDocument returned:",
        attendanceDoc
      )
      ...
      if (attendanceDoc && Object.keys(attendanceStore.attendanceRecords).length > 0) {
        localAttendanceRecords.value = {...attendanceStore.attendanceRecords}
        console.log(
          `[AttendanceDebug] fetchDataForComponent: ✅ Cargados ${Object.keys(localAttendanceRecords.value).length} registros desde Firebase`
        )
      } else {
        ...
        if (studentsInClass.length > 0) {
          ...
          console.log(
            `[AttendanceDebug] fetchDataForComponent: ✅ Inicializados ${Object.keys(defaultRecords).length} registros con estado 'Ausente'`
          )
        } else {
          localAttendanceRecords.value = {} // No hay estudiantes, no hay registros
          console.log(
            "[AttendanceDebug] fetchDataForComponent: No hay estudiantes, no se inicializan registros"
          )
        }
      }
    } else {
    ...
    if (debugEnabled) {
      console.log(
        "[AttendanceDebug] fetchDataForComponent: Total de registros tras cargar:",
        Object.keys(localAttendanceRecords.value).length
      )
    }
...
onMounted(async () => {
  ...
  const rawDate = props.date || (route.params.date as string) || attendanceStore.selectedDate
  ...
  if (rawDate) {
    if (typeof rawDate === "string" && rawDate.length === 8 && !rawDate.includes("-")) {
      ...
      if (debugEnabled) {
        console.log(
          `[AttendanceDebug] onMounted: Invalid date "${rawDate}", using today: ${dateToUse}`
        )
      }
    }
  } else {
  ...
  try {
    ...
    if (!classInfo) {
      try {
        console.log(
          `[AttendanceDebug] Clase no encontrada en clases regulares, buscando en clases emergentes...`
        )
        ...
        if (normalizedEmergencyData) {
          ...
          isEmergencyClass = true
          console.log(
            `[AttendanceDebug] Clase emergente normalizada encontrada: "${normalizedEmergencyData.name}"`
          )
        }
      } catch (error) {
    ...
    if (!classInfo) {
      console.error(
        `[AttendanceDebug] No se encontro la clase con ID=${classIdToUse} ni en clases regulares ni emergentes`
      )
    ...
    console.log(
      ...
      isEmergencyClass ? "EMERGENTE" : "REGULAR",
      classInfo.name
    )
    ...
    console.log(
      `[AttendanceDebug] 📋 onMounted: Total estudiantes obtenidos: ${studentsInClass.length}`
    )

    if (studentsInClass.length === 0) {
      console.warn(
        `[AttendanceDebug] ⚠️ No se obtuvieron estudiantes para la clase ${classIdToUse}`
      )
    ...
    console.log(
      `[AttendanceDebug] Cargando datos de asistencia para fecha=${dateToUse}, clase=${classIdToUse}`
    )
    ...
    if (propsSize === 0 && storeSize === 0) {
      ...
      if (attendanceDoc) {
        console.log(
          "[AttendanceDebug] ✓ DOCUMENTO ENCONTRADO EN FIRESTORE (onMounted):",
          attendanceDoc
        )
        // Actualizar localAttendanceRecords con los datos frescos del store (que fetchAttendanceDocument actualiza)
        localAttendanceRecords.value = {
          ...(attendanceStore.attendanceRecords as Record<string, AttendanceStatus>),
        }
      } else if (Object.keys(localAttendanceRecords.value).length === 0) {
        console.log(
          "[AttendanceDebug] ⚠️ NO HAY DOCUMENTO DE ASISTENCIA (onMounted) - INICIALIZANDO ESTADOS PREDETERMINADOS SI ES NECESARIO"
        )
      ...
      console.log(
        "[AttendanceDebug] Usando datos de Props o Store, no se carga de Firebase en onMounted a menos que se fuerce."
      )
    }
    ...
    if (
      ...
      JSON.stringify(localAttendanceRecords.value) !==
        JSON.stringify(attendanceStore.attendanceRecords)
    ) {
      console.log(
        "[AttendanceDebug] Sincronizando localAttendanceRecords con attendanceStore.attendanceRecords en onMounted"
      )
    ...
    try {
      ...
      console.warn(
        "[AttendanceDebug] ⚠️ Error al cargar observaciones:",
    ...
    await nextTick()
    console.log(
      "[AttendanceDebug] Total de registros tras cargar (onMounted):",
      Object.keys(localAttendanceRecords.value).length
    )
    ...
    if (Object.keys(localAttendanceRecords.value).length > 0) {
      setTimeout(() => {
        ...
        if (recordCount === 0 && studentsInClass.length > 0) {
          console.warn(
            "[AttendanceDebug] ALERTA: No se cargaron registros de asistencia pero hay estudiantes en la clase."
          )
        }
      }, 100)
    }
...
watch(
  ...
  async (newValues) => {
    const debugEnabled = window.localStorage.getItem("attendance-debug") === "true"
    const [propDate, propClassId, routeParamDate, routeParamClassId, storeDate, storeClassId] =
      newValues
    ...
    if (debugEnabled) {
      console.log(
        `[AttendanceList Watcher] Valores efectivos: fecha='${finalDate}', claseId='${finalClassId}'`
      )
    }

    // Prevenir llamadas redundantes - comprobar si los datos realmente cambiaron
    if (
      finalDate === lastProcessedValues.value.date &&
      finalClassId === lastProcessedValues.value.classId
    ) {
      if (debugEnabled) {
        console.log(
          "[AttendanceDebug] Watcher: Valores identicos a los ultimos procesados, omitiendo actualizacion."
        )
      }
    ...
    if (isWatcherProcessing.value) {
      if (debugEnabled) {
        console.log(
          "[AttendanceDebug] Watcher: Ya hay una actualizacion en proceso, omitiendo llamada paralela."
        )
      }
      return
    }
    ...
    if (!finalDate || !finalClassId) {
      if (debugEnabled) {
        console.log(
          "[AttendanceDebug] Watcher: Fecha o ID de clase invalidos. Esperando parametros validos."
        )
      }
      return
    }
    ...
    try {
      ...
      if (classExists) {
        ...
        if (debugEnabled && result) {
          console.log(
            `[AttendanceDebug] Watcher: Datos cargados correctamente. Estudiantes: ${result.students?.length || 0}, Registros: ${Object.keys(result.attendanceRecords || {}).length}`
          )
        }
      } else {
...
watch(
  [() => props.attendanceRecords, () => attendanceStore.attendanceRecords],
  (newValues, oldValues) => {
    ...
    if (pendingChangesCount.value > 0) {
      console.log(
        `[AttendanceDebug] ⚠️ PROTECCION DE CAMBIOS: Ignorando cambios externos porque hay ${pendingChangesCount.value} cambios pendientes`
      )
      return
    }
...
const refreshAttendanceData = async () => {
  try {
    ...
    console.log(
      `[AttendanceDebug] Refrescando datos para fecha=${dateToUse}, clase=${classIdToUse}`
    )
...
watch(
  ...
  (newStudents) => {
    console.log(
      "[AttendanceDebug] Watch detecto cambio en props.students:",
    ...
    if (newStudents) {
      ...
      if (Object.keys(localAttendanceRecords.value).length > 0) {
        ...
        const studentsWithoutStatus = newStudents.filter(
          (student) => !localAttendanceRecords.value[student.id]
        )

        if (studentsWithoutStatus.length > 0) {
          console.log(
            `[AttendanceDebug] ${studentsWithoutStatus.length} estudiantes sin estado de asistencia`
          )
...
const validateAttendanceStates = () => {
  ...
  localStudents.value.forEach((student) => {
    ...
    if (status === "Justificado" && !pendingJustifications.value.has(student.id)) {
      console.warn(
        `[AttendanceDebug] Estudiante ${student.id} marcado como justificado pero sin justificacion`
      )
...
const handleUpdateStatus = async (studentId: string, status: string) => {
  if (studentId === "all" && status === "save") {
    ...
    if (!valid && invalidStudents.length > 0) {
      console.error(
        `[AttendanceDebug] Se encontraron ${invalidStudents.length} estudiantes con estados invalidos`
      )
      displayToast(
        `Error: Hay ${invalidStudents.length} estudiantes con estados invalidos`,
        "error"
      )
      return
    }
    ...
    if (!valid && missingStatusStudents.length > 0 && invalidStudents.length === 0) {
      ...
      console.warn(
        `[AttendanceDebug] Guardando a pesar de ${missingStatusStudents.length} estudiantes sin estado`
      )
    }
  ...
  if (pendingChangesCount.value > 0) {
    console.log(
      `[AttendanceList] Cambios pendientes: ${pendingChangesCount.value}. Esperando accion de guardar.`
    )
...
const handleOpenJustification = async (student: any) => {
  try {
    ...
    if (localAttendanceRecords.value) {
      ...
      console.log(
        `[Justificacion] Estado de asistencia actualizado localmente de ${previousStatus} a Justificado`
      )
    ...
    const existingJustification =
      studentJustifications && studentJustifications.length > 0 ? studentJustifications[0] : null

    // Si ya existe una justificacion, usarla como pendiente
    if (existingJustification) {
      console.log(
        `[Justificacion] Justificacion existente encontrada para ${student.nombre}:`,
...
const normalizeEmergencyClassData = async (emergencyClassId: string) => {
  try {
    ...
    try {
      ...
      console.warn(
        `[AttendanceDebug] No se pudo acceder a los datos de la clase emergente usando attendanceStore`
      )
      return null
    }
  } catch (error) {
...
const getClassStudents = async (classId: string, isEmergencyClass = false) => {
  try {
    console.log(
      `[AttendanceDebug] Obteniendo estudiantes para clase ${classId}, emergente: ${isEmergencyClass}`
    )
    ...
    const filteredStudents = allStudents.filter(
      (student) => studentIds.includes(student.id) && (student as any).estado !== "Inactivo"
    )
    ...
    if (filteredStudents.length > 0) {
      console.log(
        `[AttendanceDebug] Primeros estudiantes:`,
    ...
    if (filteredStudents.length === 0 && studentIds.length > 0) {
      console.warn(
        `[AttendanceDebug] PROBLEMA: Se encontraron ${studentIds.length} IDs pero 0 estudiantes filtrados`
      )
      ...
      console.warn(
        `[AttendanceDebug] ¿Existe el primer estudiante (${sampleId}) en el store?`,
        studentExists
      )

      if (!studentExists) {
        console.warn(
          `[AttendanceDebug] El estudiante ${sampleId} no existe en el store de estudiantes`
        )
        // Forzar recarga de estudiantes
        await studentsStore.fetchStudents()
        // Intentar filtrar de nuevo
        const refreshedStudents = studentsStore.students || []
        return refreshedStudents.filter(
          (student) => studentIds.includes(student.id) && (student as any).estado !== "Inactivo"
        )
      }
    }
...
const handleExportToPDF = async () => {
  try {
    ...
    const currentClassName =
      props.selectedClassName || classesStore.getClassById(currentClass)?.name || "Clase sin nombre"

    if (!currentDate || !currentClass || !currentClassName) {
      displayToast(
        "Faltan datos necesarios para generar el PDF. Seleccione una fecha y clase.",
        "error"
      )
      return
    }
    ...
    const attendanceRecords =
      effectiveAttendanceRecords.value || attendanceStore.attendanceRecords || {}
    ...
    if (classInfo?.schedule) {
      if (
        "slots" in classInfo.schedule &&
        Array.isArray(classInfo.schedule.slots) &&
        classInfo.schedule.slots.length > 0
      ) {
        classSchedule = classInfo.schedule.slots
          .map((slot) => `${slot.day}: ${slot.startTime} - ${slot.endTime}`)
          .join(", ")
      } else if (
        "day" in classInfo.schedule &&
        "startTime" in classInfo.schedule &&
        "endTime" in classInfo.schedule
      ) {
            ...
            .map((obs) => {
              ...
              const content =
                typeof obs.content === "string"
                  ? obs.content
                  : obs.content?.text || "Observacion sin contenido"
...
<template>
  ...
  <div class="attendance-list-container">
    ...
    <AttendanceObservation
      ...
      :is-visible="attendanceObservationModalOpen"
      :class-id="currentSelectedClass"
