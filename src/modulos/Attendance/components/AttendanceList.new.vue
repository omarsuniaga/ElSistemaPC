...
const fetchDataForComponent = async (dateParam: string, classIdParam: string) => {
  ...
  try {
    ...
    if (records && records.length > 0) {
      ...
      console.log(
        `Cargados ${records.length} registros de asistencia para la fecha ${dateParam} y clase ${classIdParam}`
      )
    } else {
      console.log(
        `No hay registros de asistencia para la fecha ${dateParam} y clase ${classIdParam}`
      )
      localAttendanceRecords.value = {}
    }
  } catch (error) {
...
watch(
  ...
  (newRecords) => {
    if (pendingChangesCount.value === 0 && newRecords) {
      ...
      console.log(
        `No se actualizarán los registros porque hay ${pendingChangesCount.value} cambios pendientes`
      )
    }
  },
...
const handleUpdateStatus = (studentId: string, status: string) => {
  if (studentId === "all" && status === "save") {
    ...
    const dateToSave =
      (route.params.date as string) || props.date || attendanceStore.selectedDate || ""
...
const handleOpenJustification = async (student: any) => {
  try {
    ...
    const classIdToUse =
      (route.params.classId as string) || props.initialClassId || attendanceStore.selectedClass
    ...
    if (localAttendanceRecords.value) {
      ...
      console.log(
        `[Justificación] Estado de asistencia actualizado localmente de ${previousStatus} a Justificado`
      )
    ...
    if (existingJustification) {
      console.log(
        `[Justificación] Justificación existente encontrada para ${student.nombre}:`,
...
const handleSaveJustification = (data: {
  studentId: string
  reason: string
  documentURL?: string
  file?: File
  ...
  displayToast(
    "Justificación guardada. Recuerde guardar los cambios generales de asistencia.",
    "info"
  )
}
...
const resetAllStatuses = async () => {
  ...
  if (
    confirm(
      "¿Estás seguro de que quieres reestablecer el estado de todos los estudiantes a su último estado guardado?"
    )
  ) {
    // Obtener la fecha y clase actual
    const dateToUse = (route.params.date as string) || props.date || attendanceStore.selectedDate
