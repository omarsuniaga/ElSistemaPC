<template>
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 md:p-6">
    ...
    <div v-if="!configStore.attendanceWebhookUrl && !configStore.isLoading" class="mb-4 p-3 bg-yellow-100 text-yellow-700 rounded-md">
      <p><i class="fas fa-exclamation-triangle mr-2"></i> No hay una URL de webhook configurada para la asistencia.</p>
    </div>

    <div v-if="!hasPermission" class="mb-4 p-3 bg-red-100 text-red-700 rounded-md">
      <p><i class="fas fa-lock mr-2"></i> No tienes permisos para modificar configuraciones. Contacta al administrador.</p>
    </div>

    <!-- Formulario de configuración -->
    <form @submit.prevent="saveConfig" class="space-y-4">
      <div class="form-group">
        <label for="webhook-url" class="block text-sm font-medium mb-1">URL del Webhook para Asistencia:</label>
        <div class="flex">
          <input 
            id="webhook-url" 
            v-model="webhookUrl" 
        ...
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          Esta URL es donde se enviarán los datos de asistencia para procesamiento externo (Make.com).
        </p>
      </div>

      <!-- Estado de la última prueba -->
      <div v-if="lastTestResult" :class="`p-3 rounded-md ${lastTestResult.success ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`">
        <p>
          <i :class="`mr-2 ${lastTestResult.success ? 'fas fa-check-circle' : 'fas fa-exclamation-circle'}`"></i>
        ...
        <table class="min-w-full table-auto">
          <thead>
            <tr class="bg-gray-100 dark:bg-gray-700">
              ...
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Cambio</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(update, index) in configStore.configs.updates" :key="index" class="border-b border-gray-200 dark:border-gray-700">
