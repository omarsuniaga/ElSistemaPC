  ...
  <div class="observations-history bg-gray-50 dark:bg-gray-900 p-4 sm:p-6 rounded-lg">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      <div class="col-span-1">
        <label for="class-filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Filtrar por clase</label>
        <select 
          id="class-filter"
      ...
      <div class="col-span-1">
        <label for="date-range" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Rango de fechas</label>
      ...
      <div class="col-span-1">
        <label for="search-filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Buscar en observaciones</label>
    ...
    <div v-else-if="error" class="p-6 bg-red-50 dark:bg-red-900/20 rounded-lg text-center mb-6">
      ...
      <button @click="refreshObservations" class="mt-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors">
        Reintentar
      </button>
    </div>

    <!-- Empty state -->
    <div v-else-if="filteredObservations.length === 0" class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
      <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-full inline-block mb-4">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
      </div>
      <h3 class="text-xl font-medium text-gray-700 dark:text-gray-300">No hay observaciones que mostrar</h3>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="observation in paginatedObservations" :key="observation.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  ...
                  <div class="ml-3">
                    <div class="text-sm font-medium text-gray-900 dark:text-gray-200">{{ getClassName(observation.classId) }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div>
            <p class="text-sm text-gray-700 dark:text-gray-300">
              Mostrando <span class="font-medium">{{ paginationStart + 1 }}</span> a <span class="font-medium">{{ paginationEnd }}</span> de <span class="font-medium">{{ filteredObservations.length }}</span> observaciones
            </p>
          </div>
          <div>
