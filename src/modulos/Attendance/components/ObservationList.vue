    ...
    <div v-else-if="observations.length > 0" class="space-y-4">
      ...
      >
        ...
        <div class="flex justify-between items-start mb-4">
          <div>
            <span
              ...
              :class="priorityColors[observation.priority]"
            >
              {{ typeLabels[observation.type] }}
            </span>
            <span
              v-if="observation.requiresFollowUp"
              class="ml-2 inline-block px-2 py-1 text-xs font-semibold rounded-full bg-purple-100 text-purple-800"
            >
        ...
        <div class="space-y-3">
          ...
          <!-- Viñetas -->
          <ul
            v-if="observation.content.bulletPoints?.length"
            class="list-disc list-inside space-y-1"
          >
            <li
              v-for="(point, index) in observation.content.bulletPoints"
              :key="index"
              class="text-gray-700"
            >
              {{ point }}
          ...
          <div v-if="observation.content.works?.length" class="space-y-2">
            ...
            <div
              v-for="(work, index) in observation.content.works"
              :key="index"
              class="pl-4 border-l-2 border-gray-200"
            >
              <p class="font-medium">{{ work.title }}</p>
              <p v-if="work.composer" class="text-sm text-gray-600">
          ...
          <div v-if="observation.content.classDynamics?.length" class="space-y-2">
            ...
            <div
              v-for="(dynamic, index) in observation.content.classDynamics"
              :key="index"
              class="pl-4 border-l-2 border-gray-200"
            >
        ...
        <div class="mt-4 pt-4 border-t border-gray-200">
          <p class="text-sm text-gray-500">Observación creada por {{ observation.author }}</p>
        </div>
      </div>
    </div>
