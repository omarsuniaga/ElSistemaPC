...
import type {AttendanceRecord, AttendanceDocument} from "../../types/attendance"
...
function _updateAttendanceRecordsFromDocument(
  ...
  if (!document || !document.data) {
    console.warn(
      "⚠️ Documento de asistencia vacío o sin datos para _updateAttendanceRecordsFromDocument"
    )
    return
  }
  ...
  if (Array.isArray(data.justificacion)) {
    ...
    console.warn(
      "⚠️ Justificaciones en formato de objeto no estándar detectado en _updateAttendanceRecordsFromDocument. Intentando convertir.",
      data.justificacion
    )
    justificationsToProcess = Object.entries(
      data.justificacion as Record<string, Omit<JustificationItemInDoc, "id">>
  ...
  justifiedStudentIds.forEach((studentId) => {
    if (this.attendanceRecords[studentId] !== "Justificado") {
      console.warn(
        `⚠️ Corrigiendo inconsistencia en _updateAttendanceRecordsFromDocument: Estudiante ${studentId} con justificación aprobada no marcado como Justificado. Forzando.`
      )
...
async function fetchAttendanceDocuments(this: AttendanceStoreState): Promise<AttendanceDocument[]> {
  // Renamed back from fetchAttendanceRecords
  this.isLoading = true
  this.error = null
  try {
...
async function fetchAttendanceDocument(
  ...
  if (
    ...
    this.lastFetch &&
    now.getTime() - this.lastFetch.getTime() < this.cacheExpiryMinutes * 60 * 1000
  ) {
    console.log(
      `[FetchActionsDebug] fetchAttendanceDocument: Returning cached document for ${fecha} - ${classId}`
    )
    this.isLoading = false
    // Ensure attendanceRecords are up-to-date with this cached doc
    if (this.currentAttendanceDoc.data) {
  ...
  try {
    console.log(
      `[FetchActionsDebug] fetchAttendanceDocument: Fetching from Firebase for ${fecha} - ${classId}`
    )
    const document = await getAttendanceDocumentFirebase(fecha, classId)
    console.log(
      `[FetchActionsDebug] fetchAttendanceDocument: getAttendanceDocumentFirebase returned:`,
      document ? JSON.parse(JSON.stringify(document)) : null
    )
    if (document && document.data) {
      // Ensure document and document.data exist
      ...
      const index = this.attendanceDocuments.findIndex(
        (doc) =>
          doc.id === document.id ||
          (doc.fecha === document.fecha && doc.classId === document.classId)
      )
      if (index !== -1) {
      ...
      console.warn(
        `[FetchActionsDebug] fetchAttendanceDocument: No document found or document.data is missing for ${fecha} - ${classId}. Clearing current doc and records.`
      )
...
async function fetchAttendanceByClassAndDate(
  ...
  // Return type is Record<string, string> (studentId -> status)
  this.isLoading = true
  this.error = null
  try {
    // This service function returns AttendanceRecord[]
    const fetchedFirebaseRecords: AttendanceRecord[] = await getAttendanceByDateAndClassFirebase(
      date,
      classId
    )
    ...
    const docInStore = this.attendanceDocuments.find(
      (d) => d.fecha === date && d.classId === classId
    )
    ...
    if (docInStore) {
      ...
      const currentJustifications: JustificationItemInDoc[] = Array.isArray(
        docInStore.data.justificacion
      )
      ...
      fetchedFirebaseRecords.forEach((record) => {
        ...
        if (status === "Presente") {
          ...
          const justRec = currentJustifications.find((j) => j.id === studentId)
          if (justRec) {
            justRec.approved = true
            justRec.status = "approved" // Ensure status reflects approval
            justRec.reason =
              record.justification?.reason || justRec.reason || "Justificado (actualizado)"
...
async function getStudentAttendanceStatus(
  this: any,
  studentId: string,
  date: string,
  classId?: string
  ...
  try {
    ...
    if (relevantDoc && relevantDoc.data) {
      ...
      if (Array.isArray(data.justificacion)) {
        ;(data.justificacion as JustificationItemInDoc[]).forEach((j) => {
          if (j.id && (j.approved || j.status === "approved")) {
            // Check for approval
      ...
      data.presentes?.forEach((id: string) => {
        if (!tempRecords[id]) tempRecords[id] = "Presente"
      })
      data.ausentes?.forEach((id: string) => {
        if (!tempRecords[id] && !justifiedStudentIds.has(id)) tempRecords[id] = "Ausente"
      })
      data.tarde?.forEach((id: string) => {
        if (!tempRecords[id] && !justifiedStudentIds.has(id)) tempRecords[id] = "Tardanza"
      })
    ...
    console.error(
      `Error getting student attendance status for ${studentId} on ${date} (clase ${classId || "N/A"}): ${error.message}`
    )
