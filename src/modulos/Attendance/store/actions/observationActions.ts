...
export const observationActions = {
  async updateObservations(
    ...
    try {
      ...
      const doc = this.attendanceDocuments.find(
        (d: any) => d.fecha === date && d.classId === classId,
      );
      if (doc) {
  ...
  async addObservationToHistory(
    this: any,
    observation: Omit<ClassObservation, 'id' | 'createdAt'>,
  ): Promise<void> {
    this.isLoading = true;
    this.error = null;
    try {
      ...
      console.log(
        `Adding observation to history: ${observation.content.taggedStudents && observation.content.taggedStudents.length > 0 ? 'student-specific' : 'class-level'}`,
      );
      ...
      await addObservationToHistoryFirebase(
        ...
        observation.content.text,
        observation.author,
        observation.content.taggedStudents && observation.content.taggedStudents.length > 0
          ? observation.content.taggedStudents[0]
          : undefined, // Pass the first studentId if it exists for student-specific observations
      );
  ...
  async updateObservationInHistory(
    this: any,
    ...
    try {
      ...
      const index = this.observationsHistory.findIndex(
        (obs: ClassObservation) => obs.id === observationId,
      );
      if (index !== -1) {
        // Update only the text, or more broadly if service supported more partial updates
        this.observationsHistory[index] = {
          ...this.observationsHistory[index],
          content: { ...this.observationsHistory[index].content, text: updates.content.text },
          updatedAt: new Date(),
        };
      }
    } catch (error: any) {
