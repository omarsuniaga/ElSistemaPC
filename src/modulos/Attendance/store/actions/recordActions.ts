...
import type { AttendanceDocument, JustificationData, AttendanceRecord as MainAttendanceRecord } from '../../types/attendance'; // Use alias for imported type
import {
...
interface RecordActionsThisContext {
  ...
  fetchAttendanceByClassAndDate?: (classId: string, date: string) => Promise<Record<string, string> | AttendanceDocument | null>; // Adjusted return type
...
export const recordActions = {
  ...
  _updateAttendanceDocumentsArray(this: RecordActionsThisContext, document: AttendanceDocument): void {
    if (!this.attendanceDocuments) {
      console.error('_updateAttendanceDocumentsArray called but attendanceDocuments is undefined on store state.');
      this.attendanceDocuments = []; 
    }
    const index = this.attendanceDocuments.findIndex((d: AttendanceDocument) => d.id === document.id);
    if (index !== -1) {
  ...
  async addRecord(this: RecordActionsThisContext, recordData: Omit<MainAttendanceRecord, 'id'>): Promise<MainAttendanceRecord | null> {
    this.isLoading = true;
    this.error = null;
    try {
      ...
      const newRecord = await addAttendanceRecordFirebase({ ...recordData, status: validStatus as MainAttendanceRecord['status'] });
      if (newRecord) {
  ...
  async updateRecord(this: RecordActionsThisContext, recordData: MainAttendanceRecord): Promise<MainAttendanceRecord | null> {
    this.isLoading = true;
    this.error = null;
    try {
      const validStatus = ['Presente', 'Ausente', 'Tardanza', 'Justificado'].includes(recordData.status) 
      ...
      const docId = await updateAttendanceRecordFirebase({ ...recordData, status: validStatus as MainAttendanceRecord['status'] });
      ...
      if (docId) {
        ...
        if (index !== -1) {
          ...
          this.records[index] = { ...this.records[index], ...recordData, status: validStatus as MainAttendanceRecord['status'] };
        }
