...
import type { AttendanceDocument, AttendanceRecord, AttendanceAnalytics } from '../../types/attendance';
...
export interface AttendanceStoreState { // Added export
  ...
  fetchAttendanceRecords(params: { classId?: string; startDate: string | Date; endDate?: string | Date }): Promise<Record<string, string>>;
}
...
export interface AttendanceReport { // Added export
  parameters: {
    startDate: string;
    endDate: string;
    classId: string;
...
export interface AbsentStudentInfo { // Added export
  studentId: string;
  absences: number;
  ...
  try {
    ...
    filteredDocs.forEach(doc => {
      ...
      const justificationsMap = new Map(
        (docData.justificacion || []).map(j => [j.id, j.reason])
      );
      ...
      const processStudentList = (studentIds: string[], status: string) => {
        studentIds.forEach(sId => {
          ...
          let justificationText = justificationsMap.get(sId);
...
async function calculateAbsentStudents(
  ...
  try {
    ...
    const filteredRecords = classId
      ? this.records.filter(record => record.classId === classId && record.Fecha >= startDate && record.Fecha <= endDate)
    ...
    filteredRecords.forEach(record => {
      if (!absencesByStudent[record.studentId]) {
        absencesByStudent[record.studentId] = { count: 0, lastDate: '', totalClasses: 0, attendedClasses: 0 };
      }
      absencesByStudent[record.studentId].totalClasses++;

      if (record.status === 'Ausente') { // Estrictamente 'Ausente', no 'Justificado'
        ...
        if (!absencesByStudent[record.studentId].lastDate || record.Fecha > absencesByStudent[record.studentId].lastDate) {
    ...
    const result = Object.entries(absencesByStudent)
      .map(([studentId, data]) => ({
        ...
        attendanceRate: data.totalClasses > 0 ? (data.attendedClasses / data.totalClasses) * 100 : 0,
      }))
...
async function generateProfessionalReport(
  this: AttendanceStoreState,
  params: { classId?: string; studentId?: string; startDate: string; endDate: string }
): Promise<any[]> { // El tipo de retorno coincide con el original, aunque es mejor una interfaz específica
  this.isLoading = true;
  this.error = null;
  try {
    const { classId, studentId, startDate, endDate } = params;
    ...
    const reportData = filteredRecords.map((record: AttendanceRecord) => {
      ...
      if (record.justification) {
        justificationText = typeof record.justification === 'string' // No debería ocurrir con el tipo JustificationData
...
async function updateAnalytics(this: AttendanceStoreState): Promise<void> {
  ...
  try {
    ...
    const recordsForAnalytics = this.records.filter(r => r.Fecha >= formattedStartDate && r.Fecha <= formattedEndDate);
    ...
    const dailyStats: Record<string, { present: number; absent: number; tardy: number; justified: number, total: number }> = {};
    const studentAttendance: Record<string, { present: number; absent: number; tardy: number; justified: number, totalClasses: number }> = {};
    ...
    recordsForAnalytics.forEach(record => {
      ...
      if (!studentAttendance[record.studentId]) {
        studentAttendance[record.studentId] = { present: 0, absent: 0, tardy: 0, justified: 0, totalClasses: 0 };
      }
    ...
    const averageAttendance = totalStudentsTrackedForAvg > 0 ? (totalPresentOrJustified / totalStudentsTrackedForAvg) * 100 : 0;
