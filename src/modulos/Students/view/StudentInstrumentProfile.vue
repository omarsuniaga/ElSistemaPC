  ...
  <div class="max-w-3xl mx-auto p-6 bg-white rounded shadow">
    ...
    <div v-if="student && instrument">
      ...
      <section class="mb-6">
        ...
        <div v-if="instrument.asignacion?.contrato">
          <b>Contrato:</b> <a :href="instrument.asignacion.contrato" target="_blank">Ver PDF</a>
        </div>
      </section>
      <section class="mb-6">
        ...
        <div v-if="instrument">
          <ul class="list-disc ml-6">
            ...
            <li v-if="instrument.isAssign && instrument.asignacion">
              ...
              <div v-if="instrument.asignacion.contrato">
                <b>Contrato:</b>
                <a :href="instrument.asignacion.contrato" target="_blank">Ver PDF</a>
              </div>
            ...
            <li v-for="(h, idx) in instrument.historial" :key="'hist-' + idx">
              ...
              <div>
                <b>Condiciones:</b> Entrega: {{ h.condiciones.entrega }}, Devolución:
                {{ h.condiciones.devolucion }}
              </div>
              ...
              <div v-if="h.cambiosAccesorios && h.cambiosAccesorios.length">
                ...
                <ul>
                  <li v-for="(c, i) in h.cambiosAccesorios" :key="i">
                    {{ c.accesorio }} - {{ c.detalle }} ({{ c.fecha }})
                  </li>
                </ul>
              </div>
            </li>
          ...
          <div
            v-if="!instrument.isAssign && (!instrument.historial || !instrument.historial.length)"
            class="text-gray-500"
          >
            Sin historial de instrumento.
          </div>
        </div>
      </section>
      <div class="flex gap-2">
        <button class="px-4 py-2 bg-blue-600 text-white rounded" @click="goToEdit">
          Reasignar Instrumento
        </button>
