...
<template>
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4">
    ...
    <div class="flex justify-between items-center mb-4">
      <button class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700" @click="previousWeek">
        <i class="fas fa-chevron-left" />
      </button>
      <h3 class="text-lg font-medium">
        {{ format(currentWeek, "MMMM yyyy", {locale: es}) }}
      </h3>
      <button class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700" @click="nextWeek">
    ...
    <div class="overflow-x-auto">
      <div class="inline-block min-w-full">
        <div class="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700">
          <!-- Columna de horas -->
          <div class="bg-gray-50 dark:bg-gray-800 p-2 text-sm font-medium">Hora</div>
          <!-- Cabeceras de días -->
          <div
          ...
          <template v-for="time in timeSlots" :key="time">
            <!-- Hora -->
            <div
              class="bg-white dark:bg-gray-900 p-2 text-sm border-t border-gray-200 dark:border-gray-700"
            >
              {{ time }}
            </div>
            <!-- Clases por día -->
              ...
              <div
                ...
                :class="{
                  ...
                  'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200':
                    class_.teacherId && class_.studentIds?.length,
  ...
  props.classes.forEach((class_: any) => {
    ...
    const schedule_ =
      typeof class_.schedule === 'string'
        ? {
          days: [class_.schedule.split(' ')[0]],
          startTime: class_.schedule.split(' ')[1],
          endTime: class_.schedule.split(' ')[3],
        }
        : class_.schedule;
