...
export async function migrateLegacySchedules(
  ...
  try {
    ...
    const migratedSchedules: Schedule[] = legacySchedules.map((legacy) =>
      convertLegacyToNewFormat(legacy)
    )
...
export async function updateSchedule(
  ...
  try {
    ...
    // Convert Date objects to Firestore Timestamps
    const processedUpdates = Object.entries(cleanUpdates).reduce(
      (acc, [key, value]) => {
...
function convertLegacyToNewFormat(legacy: LegacySchedule): ScheduleDay {
  return {
    ...
    timeSlot: {
      ...
      duration: calculateDuration(legacy.startTime || "08:00", legacy.endTime || "09:30"),
    },
