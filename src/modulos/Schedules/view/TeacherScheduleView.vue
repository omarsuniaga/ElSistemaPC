...
const loadData = async (uid) => {
  try {
    ...
    teacher.value = teachersStore.getTeacherById(uid)
    if (!teacher.value) {
      throw new Error('Maestro no encontrado')
    }
    schedule.value = await teachersStore.getTeacherSchedule(uid)
  } catch (err: any) {
...
onMounted(async () => {
  ...
  const user = computed(() => {
    ...
    return teacherId.value = user ? user : null
  })
 
console.log("onMounted: ",user.value)
  
  await loadData(user.value?.uid)
})
      ...
      <div class="mb-8">
        ...
        <div class="bg-gray-50 p-4 rounded-b-lg shadow-sm">
          ...
          <div class="grid grid-cols-2 gap-3 text-sm mt-4">
            <div>
              <span class="text-gray-500 dark:text-gray-400">Disponibilidad:</span>
          ...
          <table class="w-full">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-2">{{ class_.studentIds.length }}</td>
              </tr>
