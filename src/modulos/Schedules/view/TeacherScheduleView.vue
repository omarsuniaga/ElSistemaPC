<template>
  <div class="p-4 max-w-5xl mx-auto">
    ...
    <!-- Vista del horario semanal del maestro -->
    <div class="mb-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4">
      <TeacherWeeklySchedule 
        v-if="!isLoading && teacher && teacherClasses.length > 0" 
      ...
      <div v-else class="text-center py-12 text-gray-500 dark:text-gray-400">
        No hay clases asignadas para este maestro
      </div>
    </div>
...
// Computed para obtener las clases del maestro
const teacherClasses = computed(() => {
  if (!teacher.value?.id) return []
  return classesStore.classes.filter(class_ => class_.teacherId === teacher.value.id)
})
...
const loadData = async (uid: string) => {
  try {
    ...
    if (foundTeacher) {
      ...
      console.log('Clases del maestro:', teacherClasses.value)
    } else {
      await teachersStore.fetchTeachers()
