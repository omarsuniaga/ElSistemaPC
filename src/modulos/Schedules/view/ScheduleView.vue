...
<template>
  <div class="p-4 md:p-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4 mb-6">
      ...
      <!-- Error -->
      <div v-else-if="error" class="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-md p-4 mb-4">
      ...
      <div v-else-if="showDiagnostic" class="mt-4">
        <div class="bg-gray-50 dark:bg-gray-900 rounded-md p-4 font-mono text-sm whitespace-pre-wrap">
          {{ diagnosticInfo }}
        </div>
        <button 
      ...
      <!-- Sin datos -->
      <div v-else-if="!hasData" class="bg-yellow-50 dark:bg-yellow-900/10 p-4 rounded-md text-center">
        <p class="text-yellow-700 dark:text-yellow-400 mb-2">No hay clases registradas en el sistema.</p>
        <button 
          @click="openNewScheduleForm" 
        ...
        <div v-if="viewMode === 'list'">
          ...
          <div class="mt-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
              <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-md">
                <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ classStats.total }}</div>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-800">
                  <tr>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Clase</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Horario</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Profesor</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Estudiantes</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Nivel</th>
                  ...
                  >
                    <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                      <div class="flex flex-col">
                        ...
                        <span class="text-xs text-gray-500">{{ class_.instrument || 'Sin instrumento' }}</span>
                      </div>
                    </td>
                      ...
                      <div class="flex flex-col items-center">
                        ...
                        <span 
                          :class="{
                            ...
                            'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300': class_.schedule,
                            'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300': !class_.schedule
                          }"
                        >
                          {{ 
          ...
          <div class="mt-6 md:hidden">
            ...
            <div class="grid grid-cols-1 gap-4">
              ...
              <!-- Mensaje si no hay clases -->
              <div v-if="enhancedClasses.length === 0" class="text-center py-4 text-gray-500 dark:text-gray-400">
                No hay clases disponibles
              </div>
            </div>
          </div>
    ...
    <div v-if="showDetailsModal && selectedClassDetails" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        ...
        <div class="p-6">
          ...
          <div class="space-y-6">
            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ selectedClassDetails.name }}</h3>
              
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p class="text-sm text-gray-500 dark:text-gray-400">Instrumento</p>
                  <p class="font-medium">{{ selectedClassDetails.instrument || 'No especificado' }}</p>
                </div>
            ...
            <div>
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Estudiantes inscritos</h3>
              
              <div v-if="selectedClassDetails.studentDetails && selectedClassDetails.studentDetails.length > 0" 
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-for="student in selectedClassDetails.studentDetails" :key="student.id">
                      <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ student.id }}</td>
                      <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ student.name }}</td>
                      <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ student.level }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              
              <div v-else class="text-center py-8 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 rounded-lg">
    ...
    <div v-if="showConfirmDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6">
        ...
        <p class="text-gray-600 dark:text-gray-400 mb-6">¿Estás seguro de que deseas eliminar esta clase? Esta acción no se puede deshacer.</p>
        <div class="flex justify-end space-x-3">
          <button
    ...
    <div v-if="showStudentsModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-auto">
        <div class="p-6">
          ...
          <div v-if="selectedStudentsList.length > 0" class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                  <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">ID</th>
