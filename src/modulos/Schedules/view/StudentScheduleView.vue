...
const fetchInitialData = async () => {
  try {
    ...
    console.log(`Datos cargados: ${studentsStore.students.length} estudiantes, ${classesStore.classes.length} clases, ${teachersStore.teachers.length} profesores`);
    return true;
  } catch (err) {
...
const loadData = async () => {
  try {
    ...
    const classes = classesStore.classes.filter(
      class_ => class_.studentIds && Array.isArray(class_.studentIds) && class_.studentIds.includes(selectedStudent.value)
    )
    ...
    studentClasses.value = await Promise.all(classes.map(async (class_) => {
...
<template>
  <div class="student-schedule-view max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-else-if="!hasClasses" class="text-center py-8">
      <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md p-4 mb-4 inline-block">
    ...
    <div v-else class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="p-4 space-y-3">            
            ...
            <div class="flex items-start">
              <i class="fas fa-user mt-1 text-gray-500 dark:text-gray-400 w-5"></i>
              <div class="ml-2">
            ...
            <div class="flex items-start">
              <i class="fas fa-graduation-cap mt-1 text-gray-500 dark:text-gray-400 w-5"></i>
              <div class="ml-2">
                <p class="text-sm font-medium text-gray-900 dark:text-white">Nivel</p>
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ class_.level || 'No definido' }}</p>
              </div>
            </div>
            
            <div class="flex items-start">
              <i class="fas fa-building mt-1 text-gray-500 dark:text-gray-400 w-5"></i>
              <div class="ml-2">
                <p class="text-sm font-medium text-gray-900 dark:text-white">Aula</p>
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ class_.classroom || 'No asignada' }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="studentClasses.length === 0" class="text-center py-8 text-gray-500 dark:text-gray-400">
        No se encontraron clases asignadas para este estudiante
      </div>
    </div>
  </div>
