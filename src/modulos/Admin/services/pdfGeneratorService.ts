...
export class PDFGeneratorService {
  ...
  constructor(orientation: 'portrait' | 'landscape' = 'portrait', pageSize: 'letter' | 'a4' | 'legal' | 'tabloid' = 'letter') {
    this.doc = new jsPDF({
      orientation,
      unit: 'mm',
      format: pageSize
  ...
  public async addHeader(
    ...
    if (includeDate) {
      ...
      this.doc.text(`Generado el: ${formattedDate} a las ${formattedTime}`, pageWidth / 2, yPosition, { align: 'center' })
      yPosition += 6
  ...
  private hasActiveFilters(filters: any): boolean {
    return filters.classId || filters.teacherId || filters.instrumentId || 
           filters.ageRange.min > 0 || filters.ageRange.max < 100 ||
           filters.enrollmentDate.start || filters.enrollmentDate.end ||
           filters.status !== 'all'
  }
  ...
  private addDataTable(data: any[], fields: string[], reportType: string, options: any) {
    if (data.length === 0) {
      ...
      this.doc.text('No hay datos para mostrar', this.pageWidth / 2, this.currentY, { align: 'center' })
      return
    }
    ...
    const tableData = data.map(item => 
      fields.map(field => this.getFieldValue(item, field))
    )
    ...
    data.forEach(scheduleItem => {
      scheduleItem.students.forEach((student: any) => {
        if (!studentMap.has(student.name)) {
          studentMap.set(student.name, { student: student, schedules: [] });
        }
