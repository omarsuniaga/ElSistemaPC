...
export class AdvancedTeachersService {
   ...
   */  async getTeacherMetrics(): Promise<TeacherMetrics> {
    try {
      ...
      const teacherStudentCounts = teachers.map(teacher => {
        ...
        const totalStudents = teacherClasses.reduce((sum: number, c: any) => sum + (c.alumnos?.length || 0), 0)
        return totalStudents
      })

      const averageStudentsPerTeacher = teacherStudentCounts.length > 0 
        ? teacherStudentCounts.reduce((sum, count) => sum + count, 0) / teacherStudentCounts.length 
        : 0

      // Calculate hours per week (from class schedules)
      const teacherHours = await Promise.all(
        teachers.map(t => this.calculateWeeklyHours(t.id))
      )
      const averageHoursPerWeek = teacherHours.length > 0 
      ...
      const totalHoursThisMonth = payrollRecords.reduce((sum, record) => sum + record.hoursWorked, 0)      // Teachers at risk (low performance or issues)
      const riskAnalysis = await this.getTeacherRiskAnalysis()
      const teachersAtRisk = riskAnalysis.filter(r => r.performanceLevel === 'needs_improvement').length
    ...
    try {
      ...
      const performanceAnalysis = await Promise.all(
        teachers.map(async (teacher) => {
          ...
          if (evaluationScore >= 4.5 && averageAttendance >= 0.85 && studentRetention >= 0.8) {
          ...
          } else if (evaluationScore >= 4.0 && averageAttendance >= 0.75 && studentRetention >= 0.7) {
            ...
            evaluationScore, averageAttendance, studentRetention, hoursPerWeek
          )
          ...
          return {
            ...
            teacherName: `${teacher.nombre || teacher.name || ''} ${teacher.apellido || teacher.lastname || ''}`.trim(),
            studentsManaged,
            averageAttendance,
            studentRetention,
            evaluationScore,
    ...
    try {
      ...
      const optimizations = await Promise.all(
        teachers.map(async (teacher) => {
          ...
          const suggestions = this.generateScheduleSuggestions(teacher.id, currentHours, optimalHours)
  ...
  private async getTeacherStudentRetention(teacherId: string): Promise<number> {
    try {
      ...
      const estimatedRetention = Math.min(currentStudents.size / Math.max(currentStudents.size * 1.2, 1), 1)
  ...
  private generateScheduleSuggestions(_teacherId: string, currentHours: number, optimalHours: number): string[] {
    const suggestions = []
      ...
      for (let i = 0; i < classes.length; i++) {
        for (let j = i + 1; j < classes.length; j++) {
          if (this.hasScheduleOverlap(classes[i], classes[j])) {
            ...
            conflicts.push(`Conflicto entre ${class1.nombre || 'Clase'} y ${class2.nombre || 'Clase'}`)
          }
        }
      }
    } catch (error) {
