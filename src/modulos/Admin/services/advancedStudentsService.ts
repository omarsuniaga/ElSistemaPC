...
export class AdvancedStudentsService {
    ...
    try {
      ...
      for (const doc of querySnapshot.docs) {
        ...
        students.push({
          ...
          'Fecha Nacimiento': data.fechaNacimiento ? new Date(data.fechaNacimiento.seconds * 1000).toLocaleDateString() : '',
          ...
          'Fecha Inscripción': data.createdAt ? new Date(data.createdAt.seconds * 1000).toLocaleDateString() : '',
      ...
      // Auto-size columns
      const maxWidth = students.reduce((acc, row) => {
  ...
  async sendBulkWhatsApp(studentIds: string[], message: WhatsAppMessage): Promise<void> {
    try {      const students = await Promise.all(
        studentIds.map(id => getDoc(doc(this.studentsCollection, id)))
      )
   ...
   */  async getStudentMetrics(): Promise<StudentMetrics> {
    try {
      ...
      const newThisMonth = students.filter(s => {
        ...
        const createdAt = s.createdAt instanceof Timestamp ? s.createdAt.toDate() : new Date(s.createdAt)
      ...
      const retentionRate = oldStudents.length > 0 
      ...
      const averageAttendance = attendanceRates.reduce((sum, rate) => sum + rate, 0) / attendanceRates.length * 100

      // Calculate risk students
      const dropoutRisks = await this.getDropoutRiskAnalysis()
      const riskStudents = dropoutRisks.filter(r => r.riskLevel === 'high' || r.riskLevel === 'critical').length
    ...
    try {
      ...
      return attendanceSnapshot.docs.map(doc => {
        ...
        return {
          ...
          present: studentAttendance?.status === 'present' || studentAttendance?.status === 'delayed',
  ...
  private analyzeAttendancePattern(attendanceHistory: any[]): string {
    const attendanceRate = attendanceHistory.filter(a => a.present).length / attendanceHistory.length
  ...
  private generateRiskRecommendations(riskScore: number, factors: any): string[] {
    ...
    if (factors.attendanceRate < 0.7) {
      recommendations.push(isUrgent ? 'URGENTE: Contactar inmediatamente para discutir las ausencias' : 'Contactar para discutir las ausencias')
      recommendations.push('Ofrecer horarios alternativos o clases de recuperación')
    }
    
    if (factors.paymentDelays > 1) {
      recommendations.push(isHigh ? 'Revisar urgentemente opciones de pago flexibles' : 'Revisar opciones de pago flexibles')
