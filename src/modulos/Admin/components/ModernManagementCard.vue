<template>
  <div class="group relative">
    ...
    <div :class="[
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <div class="flex items-center text-sm">
            <component 
              :is="trendIcon" 
              :class="['w-4 h-4 mr-1', trendColorClass]" 
            />
            <span :class="trendColorClass">
              {{ trendDirection }}{{ recent }}
            </span>
      ...
      <!-- Extra Actions Slot -->
      <div v-if="$slots['extra-actions']" class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
        <slot name="extra-actions"></slot>
      </div>
    </div>
    
    <!-- Hover Effect Overlay -->
    <div class="absolute inset-0 rounded-xl bg-gradient-to-r opacity-0 group-hover:opacity-5 transition-opacity duration-300 pointer-events-none"
         :class="gradientClass">
    </div>
  </div>
</template>
