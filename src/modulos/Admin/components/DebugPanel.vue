<template>
  <div class="fixed bottom-4 right-4 bg-white border-2 border-blue-500 rounded-lg p-4 shadow-lg max-w-md z-50">
    ...
    <div class="space-y-2 text-xs">
      ...
      <div v-if="students.length > 0" class="mt-2">
        <details class="text-xs">
          ...
          <pre class="mt-1 p-2 bg-gray-100 rounded text-xs overflow-auto max-h-32">{{ JSON.stringify(students[0], null, 2) }}</pre>
        </details>
      </div>
...
const logDebugInfo = () => {
  ...
  console.log('LocalStorage Students Cache:', 
