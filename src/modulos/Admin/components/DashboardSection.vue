<template>
  <section class="dashboard-section bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
    <!-- Header -->
    ...
    <div v-show="!expandable || isExpanded" class="section-content">
      ...
      <div v-else-if="empty" class="flex items-center justify-center py-12">
        <div class="flex flex-col items-center space-y-3 text-center">
          ...
          <div>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ emptyTitle || 'No hay datos' }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ emptyDescription || 'No se encontraron elementos para mostrar' }}</p>
          </div>
    ...
    <!-- Footer -->
    <div v-if="$slots.footer" class="border-t border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-700/50">
      <slot name="footer" />
    </div>
  </section>
</template>
