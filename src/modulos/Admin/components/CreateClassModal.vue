<template>
  <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" @click="handleBackdropClick">
    <div class="relative top-10 mx-auto p-6 border max-w-4xl shadow-lg rounded-md bg-white dark:bg-gray-800" @click.stop>
      <!-- Header -->
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="min-h-[400px]">
          ...
          <div v-if="currentStep === 0" class="space-y-4">
            <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Información Básica</h4>
          ...
          <div v-if="currentStep === 1" class="space-y-4">
            <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Configuración de Horario</h4>
              ...
              <div class="grid grid-cols-3 gap-3">
                ...
                >
                  ...
                  <label
                    ...
                    :class="formData.classType === type.value 
                  ...
                  <div v-for="day in weekDays" :key="day.value" class="text-center">
                    <label class="block">
                      ...
                      <div 
                        ...
                        :class="formData.weekDays.includes(day.value) 
                          ? 'bg-blue-500 text-white' 
                          : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'"
                      >
                        {{ day.short }}
                      </div>
                      <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ day.label }}</div>
                    </label>
                  </div>
                </div>
              </div>
          ...
          <div v-if="currentStep === 2" class="space-y-4">
            <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Recursos y Requisitos</h4>
            ...
            <div>
              ...
              <div class="space-y-2">
                <div v-for="(material, index) in formData.materials" :key="index" class="flex items-center space-x-2">
                  <input
          ...
          <div v-if="currentStep === 3" class="space-y-6">
            <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Revisar y Confirmar</h4>
              ...
              <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                ...
                <dl class="space-y-2 text-sm">
                  ...
                  <div class="flex justify-between">
                    ...
                    <dd class="text-gray-900 dark:text-white font-medium">{{ getCategoryLabel(formData.category) }}</dd>
                  </div>
                  <div class="flex justify-between">
                    <dt class="text-gray-500 dark:text-gray-400">Nivel:</dt>
                    <dd class="text-gray-900 dark:text-white font-medium">{{ getLevelLabel(formData.level) }}</dd>
                  </div>
                  <div class="flex justify-between">
                    <dt class="text-gray-500 dark:text-gray-400">Capacidad:</dt>
                    <dd class="text-gray-900 dark:text-white font-medium">{{ formData.maxStudents }}</dd>
                  </div>
                  <div class="flex justify-between">
                    <dt class="text-gray-500 dark:text-gray-400">Duración:</dt>
                    <dd class="text-gray-900 dark:text-white font-medium">{{ formData.duration }} min</dd>
                  </div>
                </dl>
              </div>
              ...
              <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                ...
                <dl class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    ...
                    <dd class="text-gray-900 dark:text-white font-medium">{{ getClassTypeLabel(formData.classType) }}</dd>
                  </div>
                  <div v-if="formData.classType === 'recurring'">
                    <div class="flex justify-between">
                      <dt class="text-gray-500 dark:text-gray-400">Días:</dt>
                      <dd class="text-gray-900 dark:text-white font-medium">{{ getWeekDaysLabel(formData.weekDays) }}</dd>
                    </div>
                    <div class="flex justify-between">
                      <dt class="text-gray-500 dark:text-gray-400">Hora:</dt>
                      <dd class="text-gray-900 dark:text-white font-medium">{{ formData.startTime }}</dd>
                    </div>
                    <div class="flex justify-between">
                      <dt class="text-gray-500 dark:text-gray-400">Inicio:</dt>
                      <dd class="text-gray-900 dark:text-white font-medium">{{ formatDate(formData.startDate) }}</dd>
                    </div>
                  </div>
                  <div v-if="formData.classType === 'workshop'">
                    <div class="flex justify-between">
                      <dt class="text-gray-500 dark:text-gray-400">Fecha:</dt>
                      <dd class="text-gray-900 dark:text-white font-medium">{{ formatDate(formData.workshopDate) }}</dd>
                    </div>
                    <div class="flex justify-between">
                      <dt class="text-gray-500 dark:text-gray-400">Hora:</dt>
                      <dd class="text-gray-900 dark:text-white font-medium">{{ formData.workshopTime }}</dd>
                    </div>
                  </div>
                </dl>
              </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div v-if="formData.roomId">
                  ...
                  <span class="ml-2 text-gray-900 dark:text-white font-medium">{{ getRoomName(formData.roomId) }}</span>
                </div>
                <div v-if="formData.cost">
                  <span class="text-gray-500 dark:text-gray-400">Costo:</span>
                  <span class="ml-2 text-gray-900 dark:text-white font-medium">${{ formData.cost }}</span>
                </div>
                <div v-if="formData.materials.length > 0">
                  <span class="text-gray-500 dark:text-gray-400">Materiales:</span>
                  <span class="ml-2 text-gray-900 dark:text-white font-medium">{{ formData.materials.length }} elementos</span>
                </div>
              </div>
            </div>
          </div>
  ...
  switch (currentStep.value) {
    ...
    case 1:
      if (formData.value.classType === 'recurring') {
        return formData.value.weekDays.length > 0 && formData.value.startTime && formData.value.startDate
