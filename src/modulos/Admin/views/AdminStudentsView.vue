    ...
    <main class="p-6">
      ...
      <div v-else-if="filteredStudents.length === 0" class="text-center py-12">
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          {{ hasActiveFilters ? 'No se encontraron estudiantes con los filtros aplicados.' : 'Comienza creando un nuevo estudiante.' }}
        </p>
        <div class="mt-6">
          <button
...
const filteredStudents = computed(() => {
  ...
  if (instrumentFilter.value) {
    filtered = filtered.filter(student => 
      student.instruments?.includes(instrumentFilter.value)
    )
  }
