...
export function useSuperAdminFilters() {
  ...
  const statusOptions = computed(() => [
    {value: "all", label: "Todos los estados", count: studentsStore.students.length},
    {
      value: "Activo",
      label: "Activos",
      count: studentsStore.students.filter((s) => s.estado === "Activo").length,
    },
    {
      value: "Inactivo",
      label: "Inactivos",
      count: studentsStore.students.filter((s) => s.estado === "Inactivo").length,
    },
    {
      value: "Suspendido",
      label: "Suspendidos",
      count: studentsStore.students.filter((s) => s.estado === "Suspendido").length,
    },
  ])
  ...
  const filteredStudents = computed(() => {
    ...
    if (advancedFilters.value.ageRange.min > 0 || advancedFilters.value.ageRange.max < 100) {
      students = students.filter((student) => {
        ...
        return (
          age >= advancedFilters.value.ageRange.min && age <= advancedFilters.value.ageRange.max
        )
      })
    }
    ...
    if (advancedFilters.value.paymentStatus !== "all") {
      students = students.filter(
        (student) => student.estadoPago === advancedFilters.value.paymentStatus
      )
    }
