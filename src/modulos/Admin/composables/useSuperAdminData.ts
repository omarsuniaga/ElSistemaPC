...
export function useSuperAdminData() {
  ...
  const students = computed(() => {
    return studentsStore.students.map(student => ({
      ...
      ageCalculated: student.fechaNacimiento 
        ? Math.floor((Date.now() - new Date(student.fechaNacimiento).getTime()) / (365.25 * 24 * 60 * 60 * 1000))
        : student.edad,
  ...
  const systemStatus = computed(() => {
    ...
    const activeUsers = students.value.filter(s => s.status === 'Activo').length +
  ...
  function generateAttendanceAnalysis() {
    return {
      ...
      trends: Array.from({ length: 7 }, (_, i) => ({
        day: new Date(Date.now() - i * 24 * 60 * 60 * 1000).toLocaleDateString('es-ES', { weekday: 'short' }),
  ...
  function generateFinancialMetrics() {
    ...
    return {
      ...
      trends: Array.from({ length: 6 }, (_, i) => ({
        month: new Date(Date.now() - i * 30 * 24 * 60 * 60 * 1000).toLocaleDateString('es-ES', { month: 'short' }),
