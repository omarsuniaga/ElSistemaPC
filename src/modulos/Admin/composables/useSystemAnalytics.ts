...
export function useSystemAnalytics() {
  ...
  const recordOperation = (module: string) => {
    ...
    lastOperationsTimestamp.value[module] = lastOperationsTimestamp.value[module].filter(t => t >= cutoff)
    
    // Update operations count
    const count = lastOperationsTimestamp.value[module].length
    switch (module) {
      case 'auth': operationCounts.value.authOperations = count; break
      case 'classes': operationCounts.value.classesOperations = count; break
      case 'reports': operationCounts.value.reportsOperations = count; break
      case 'notifications': operationCounts.value.notificationsOperations = count; break
      case 'database': operationCounts.value.dbOperations = count; break
      case 'storage': operationCounts.value.storageOperations = count; break
    }
    
    // Simulate measuring response time
    const responseTime = measureResponseTime(module)
    switch (module) {
      case 'auth': performanceMetrics.value.authResponseTime = responseTime; break
      case 'classes': performanceMetrics.value.classesResponseTime = responseTime; break
      case 'reports': performanceMetrics.value.reportsResponseTime = responseTime; break
      case 'notifications': performanceMetrics.value.notificationsResponseTime = responseTime; break
      case 'database': performanceMetrics.value.dbResponseTime = responseTime; break
      case 'storage': performanceMetrics.value.storageResponseTime = responseTime; break
    }
  }
  ...
  const calculateSystemHealth = computed(() => {
    ...
    const authScore = performanceMetrics.value.authResponseTime < responseTimeThresholds.excellent ? 100 :
                     performanceMetrics.value.authResponseTime < responseTimeThresholds.good ? 90 :
                     performanceMetrics.value.authResponseTime < responseTimeThresholds.fair ? 75 : 60
    
    const classesScore = performanceMetrics.value.classesResponseTime < responseTimeThresholds.excellent ? 100 :
                        performanceMetrics.value.classesResponseTime < responseTimeThresholds.good ? 90 :
                        performanceMetrics.value.classesResponseTime < responseTimeThresholds.fair ? 75 : 60
    
    const reportsScore = performanceMetrics.value.reportsResponseTime < responseTimeThresholds.excellent ? 100 :
                        performanceMetrics.value.reportsResponseTime < responseTimeThresholds.good ? 90 :
                        performanceMetrics.value.reportsResponseTime < responseTimeThresholds.fair ? 75 : 60
    
    const notificationsScore = performanceMetrics.value.notificationsResponseTime < responseTimeThresholds.excellent ? 100 :
                              performanceMetrics.value.notificationsResponseTime < responseTimeThresholds.good ? 90 :
                              performanceMetrics.value.notificationsResponseTime < responseTimeThresholds.fair ? 75 : 60
    
    const dbScore = performanceMetrics.value.dbResponseTime < responseTimeThresholds.excellent ? 100 :
                   performanceMetrics.value.dbResponseTime < responseTimeThresholds.good ? 90 :
                   performanceMetrics.value.dbResponseTime < responseTimeThresholds.fair ? 75 : 60
    
    const storageScore = performanceMetrics.value.storageResponseTime < responseTimeThresholds.excellent ? 100 :
                        performanceMetrics.value.storageResponseTime < responseTimeThresholds.good ? 90 :
                        performanceMetrics.value.storageResponseTime < responseTimeThresholds.fair ? 75 : 60
  ...
  const getModuleStatus = computed(() => {
    return {
      auth: performanceMetrics.value.authResponseTime < 100 ? 'healthy' : 
            performanceMetrics.value.authResponseTime < 200 ? 'warning' : 'critical',
            
      classes: performanceMetrics.value.classesResponseTime < 100 ? 'healthy' : 
              performanceMetrics.value.classesResponseTime < 200 ? 'warning' : 'critical',
              
      reports: performanceMetrics.value.reportsResponseTime < 100 ? 'healthy' : 
              performanceMetrics.value.reportsResponseTime < 200 ? 'warning' : 'critical',
              
      notifications: performanceMetrics.value.notificationsResponseTime < 100 ? 'healthy' : 
                    performanceMetrics.value.notificationsResponseTime < 200 ? 'warning' : 'critical',
                    
      database: performanceMetrics.value.dbResponseTime < 100 ? 'healthy' : 
               performanceMetrics.value.dbResponseTime < 200 ? 'warning' : 'critical',
               
      storage: performanceMetrics.value.storageResponseTime < 100 ? 'healthy' : 
              performanceMetrics.value.storageResponseTime < 200 ? 'warning' : 'critical'
    }
  })
