...
export function usePDFFields() {
  ...
  const getFilterSummary = (filters: any, reportType: string) => {
    ...
    if (filters.enrollmentDate.start || filters.enrollmentDate.end) {
      const start = filters.enrollmentDate.start ? new Date(filters.enrollmentDate.start).toLocaleDateString('es-ES') : 'Inicio'
      const end = filters.enrollmentDate.end ? new Date(filters.enrollmentDate.end).toLocaleDateString('es-ES') : 'Hoy'
    ...
    if (filters.status !== 'all') {
      ...
      summary.push(`Estado: ${statusLabels[filters.status as keyof typeof statusLabels] || filters.status}`)
    }
