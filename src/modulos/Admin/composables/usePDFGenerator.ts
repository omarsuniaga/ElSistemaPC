...
export function usePDFGenerator() {
  ...
  const generateFileName = (): string => {
    ...
    const sanitizedTitle = reportType?.title.toLowerCase()
      .replace(/\s+/g, '_')
  ...
  const sortStudents = (students: any[], sortBy: string) => {
    return students.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'enrollment':
          return new Date(a.fecInscripcion || 0).getTime() - new Date(b.fecInscripcion || 0).getTime()
        default:
          return 0
      }
    })
  ...
  const getDayName = (day: string | number): string => {
    ...
    const dayIndex = dayNamesShort.findIndex(d => d.toLowerCase() === dayLower) || 
  ...
  const getScheduleData = async () => {
    ...
    for (const classItem of classesStore.classes) {
      ...
      const studentsInClass = studentsStore.students.filter(s => classItem.studentIds?.includes(s.id))
  ...
  const addHeader = async (doc: jsPDF, yPosition: number, reportTitle: string): Promise<number> => {
    ...
    if (pdfOptions.value.includeDate) {
      ...
      doc.text(`Generado el: ${formattedDate} a las ${formattedTime}`, pageWidth / 2, yPosition, { align: 'center' })
      yPosition += 6
    }
  ...
  const generateScheduleByTeacherPDF = async (doc: jsPDF) => {
    ...
    if (scheduleData.length === 0) {
      ...
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text('No se encontraron horarios programados', doc.internal.pageSize.width / 2, yPos, { align: 'center' });
      yPos += 10;
      
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text('Verifique que las clases tengan maestros asignados y horarios configurados.', doc.internal.pageSize.width / 2, yPos, { align: 'center' });
      
      return;
    }
    
    const teachersWithSchedules = groupBy(scheduleData, 'teacherName');
    const teacherNames = Object.keys(teachersWithSchedules).filter(name => name !== 'Sin maestro' && name !== 'Sin especificar');
    ...
    if (teacherNames.length === 0) {
      ...
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text('No se encontraron maestros con horarios asignados', doc.internal.pageSize.width / 2, yPos, { align: 'center' });
      yPos += 10;
      
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text('Asigne maestros a las clases y configure sus horarios.', doc.internal.pageSize.width / 2, yPos, { align: 'center' });
    ...
    for (let i = 0; i < teacherNames.length; i++) {
      ...
      const totalStudents = new Set(teacherSchedule.flatMap((s: any) => s.students?.map((st: any) => st.id) || [])).size;
      ...
      doc.text(`Horas por semana: ${weeklyHours} horas | Cantidad de alumnos: ${totalStudents}`, 20, yPos);
      yPos += 10;
      ...
      if (sortedDays.length === 0) {
        ...
        const daySchedule = scheduleByDay[day].sort((a: any, b: any) => (a.startTime || '').localeCompare(b.startTime || ''));
        ...
        const tableBody = daySchedule.map((slot: any) => [
          slot.startTime && slot.endTime ? formatTimeRange(slot.startTime, slot.endTime) : 'Sin horario',
  ...
  const generateScheduleByStudentPDF = async (doc: jsPDF) => {
        ...
        slot.students.forEach((student: any) => {
          if (!studentSchedules.has(student.id)) {
            studentSchedules.set(student.id, {
              studentName: student.name || `${student.nombre} ${student.apellido}` || 'Estudiante sin nombre',
              classes: []
            });
          }
    ...
    const studentsWithSchedules = Array.from(studentSchedules.entries()).filter(([_, data]) => data.classes.length > 0);
    ...
    if (studentsWithSchedules.length === 0) {
      ...
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text('No se encontraron estudiantes con horarios asignados', doc.internal.pageSize.width / 2, yPos, { align: 'center' });
      return;
    }
    ...
    for (let i = 0; i < studentsWithSchedules.length; i++) {
      ...
      const weeklyHours = studentData.classes.reduce((sum: number, cls: any) => {
      ...
      doc.text(`Total de clases: ${totalClasses} | Horas por semana: ${weeklyHours.toFixed(1)} horas`, 20, yPos);
      yPos += 10;
      ...
      for (const day of sortedDays) {
        const dayClasses = classesByDay[day].sort((a: any, b: any) => (a.startTime || '').localeCompare(b.startTime || ''));
        ...
        const tableBody = dayClasses.map((cls: any) => [
          cls.startTime && cls.endTime ? formatTimeRange(cls.startTime, cls.endTime) : 'Sin horario',
  ...
  const generateScheduleByDayPDF = async (doc: jsPDF) => {
    ...
    if (sortedDays.length === 0) {
      ...
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text('No se encontraron días con horarios asignados', doc.internal.pageSize.width / 2, yPos, { align: 'center' });
      return;
    }
    ...
    for (let i = 0; i < sortedDays.length; i++) {
      ...
      const totalStudents = daySchedule.reduce((sum: number, slot: any) => sum + (slot.studentCount || 0), 0);
      const uniqueTeachers = [...new Set(daySchedule.map((slot: any) => slot.teacherName))].filter(name => name !== 'Sin maestro').length;

      doc.text(`Total de clases: ${totalClasses} | Total de alumnos: ${totalStudents} | Maestros activos: ${uniqueTeachers}`, 20, yPos);
      yPos += 10;
  ...
  const generateScheduleByClassPDF = async (doc: jsPDF) => {
    ...
    if (classNames.length === 0) {
      ...
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text('No se encontraron clases con horarios asignados', doc.internal.pageSize.width / 2, yPos, { align: 'center' });
      return;
    }
    ...
    for (let i = 0; i < classNames.length; i++) {
      ...
      const totalStudents = new Set(classSchedule.flatMap((slot: any) => slot.students?.map((st: any) => st.id) || [])).size;
      ...
      doc.text(`Horas por semana: ${weeklyHours} horas | Total de alumnos: ${totalStudents}`, 20, yPos);
      yPos += 10;
      ...
      for (const day of sortedDays) {
        const dayClasses = classByDay[day].sort((a: any, b: any) => (a.startTime || '').localeCompare(b.startTime || ''));
