...
export class PermissionsService {
   ...
   */
  async getPermissionConfig(resource: ResourceType): Promise<PermissionServiceResult<FirestorePermissionConfig>> {
    try {
      const permissionDoc = doc(db, PERMISSIONS_COLLECTION, resource);
   ...
   */
  async grantUserPermission(    userId: string,
    resource: ResourceType,
    action: PermissionAction,
    grantedBy: string,
    reason?: string
   ...
   */
  async getUserPermissionOverrides(userId: string): Promise<PermissionServiceResult<UserPermissionOverride[]>> {
    try {
      const q = query(
   ...
   * Get all permissions
   */
  async getAllPermissions(): Promise<PermissionServiceResult<Record<string, FirestorePermissionConfig>>> {
    try {
