...
describe('ObraDetailView', () => {
  ...
  describe('Carga de datos', () => {
    ...
    it('debe manejar error cuando la obra no existe', async () => {
      ...
      expect(alertSpy).toHaveBeenCalledWith('La obra no existe o no se pudo cargar. Será redirigido a la lista de obras.')
  ...
  describe('Navegación entre tabs', () => {
    it('debe cambiar de tab al hacer clic', async () => {
      const frasesTab = wrapper.findAll('.py-4.px-1.border-b-2').find(tab => 
        tab.text().includes('Frases')
      )
  ...
  describe('Interacciones de usuario', () => {
    ...
    it('debe navegar hacia atrás al hacer clic en Volver', async () => {
      ...
      const backButton = wrapper.find('button').filter(btn => 
