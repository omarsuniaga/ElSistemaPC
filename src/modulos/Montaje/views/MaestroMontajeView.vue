  ...
  <div class="maestro-montaje-container min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="header-section bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700 p-6">
      <div class="flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Panel de Maestro - Montaje</h1>
          <p class="text-gray-600 dark:text-gray-300 mt-1">Gestión de obras y seguimiento de progreso</p>
        </div>
        <div class="flex items-center space-x-4">
          ...
          <div class="flex items-center space-x-2" v-if="directorReviewed">
          ...
          <div class="text-right">
            ...
            <div class="text-lg font-bold text-green-600 dark:text-green-400">{{ progresoSemanalPorcentaje }}%</div>
          </div>
        </div>
      </div>
    </div>    <!-- Sección de Plan de Acción Semanal -->
    <div class="plan-section bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 dark:border-blue-400 p-6 mx-6 mt-6 rounded-r-lg">
      <h2 class="text-lg font-semibold text-blue-900 dark:text-blue-200 mb-3">📋 Plan de Acción - Semana {{ currentWeek }}</h2>
      <div v-if="planSemanal" class="space-y-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="bg-white dark:bg-gray-800 p-4 rounded-lg">
            ...
            <ul class="space-y-1">
              <li v-for="obra in planSemanal.obrasAsignadas" :key="obra.id" class="text-sm text-gray-700 dark:text-gray-300">
                • {{ obra.titulo }}
              </li>
            </ul>
          </div>
          <div class="bg-white dark:bg-gray-800 p-4 rounded-lg">
            ...
            <ul class="space-y-1">
              <li v-for="objetivo in planSemanal.objetivos" :key="objetivo" class="text-sm text-gray-700 dark:text-gray-300">
                • {{ objetivo }}
              </li>
            </ul>
          </div>
    ...
    <div class="main-content mx-6 mt-6 grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          >
            <div class="p-6">
              <div class="flex justify-between items-start mb-4">
                <div>
                  <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ obra.titulo }}</h3>
                  <p class="text-gray-600 dark:text-gray-300">{{ obra.compositor }}</p>
                  <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ obra.totalCompases }} compases • {{ obra.duracionEstimada }} min</p>
                </div>
                <div class="text-right">
                  <div class="text-2xl font-bold" :class="getProgresoColorClass(obra.metadatos.progresoPorcentaje)">
              ...
              <div class="mb-2 flex items-center justify-between">
                <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300">Mapa de Calor - Compases</h4>
                <select
              ...
              <div class="mb-4">
                <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Instrumentos</h4>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">🎯 Observaciones del Director</h3>
          <div v-if="observacionesDirector.length > 0" class="space-y-3">
            ...
            >
              <div class="flex justify-between items-start mb-2">
                <h4 class="font-medium text-amber-900 dark:text-amber-200">{{ observacion.obra.titulo }}</h4>
                <span class="text-xs text-amber-700 dark:text-amber-300">{{ formatDate(observacion.fecha) }}</span>
              </div>
              <p class="text-amber-800 dark:text-amber-300 text-sm">{{ observacion.contenido }}</p>
              <div v-if="observacion.tipo" class="mt-2">
                <span class="px-2 py-1 bg-amber-200 dark:bg-amber-800/50 text-amber-800 dark:text-amber-200 text-xs rounded-full">
                  {{ observacion.tipo }}
                </span>
              </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">📊 Resumen de Actividad</h3>
          <div class="space-y-4">
            <div class="flex justify-between items-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <span class="text-blue-900 dark:text-blue-200 font-medium">Obras Trabajadas Hoy</span>
              <span class="text-blue-700 dark:text-blue-300 font-bold">{{ actividadHoy.obrasTrabajas }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
              <span class="text-green-900 dark:text-green-200 font-medium">Compases Completados</span>
              <span class="text-green-700 dark:text-green-300 font-bold">{{ actividadHoy.compasesCompletados }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
              ...
              <span class="text-purple-700 dark:text-purple-300 font-bold">{{ actividadHoy.tiempoDedicado }}h</span>
            </div>
          </div>
        </div>
        ...
        <!-- Próximas Tareas -->
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">⏭️ Próximas Tareas</h3>
          <div v-if="proximasTareas.length > 0" class="space-y-2">
            ...
            >
              <div class="flex justify-between items-start">
                ...
                <span class="text-xs text-gray-500 dark:text-gray-400">{{ formatDate(tarea.fechaLimite) }}</span>
              </div>
            </div>
          </div>
          <div v-else class="text-gray-500 dark:text-gray-400 text-center py-4">
...
const planSemanal = ref({
  descripcion: 'Esta semana enfócate en perfeccionar la articulación en los pasajes rápidos y trabajar la sincronización entre vientos y cuerdas.',
  obrasAsignadas: [
...
const observacionesDirector = ref([
  {
    ...
    contenido: 'Excelente trabajo en la sección de cuerdas. Continúa enfocándote en los crescendos del movimiento III.',
    tipo: 'Felicitación',
    fecha: new Date()
  },
  {
  ...
  if (filtroEstado.value) {
    obras = obras.filter(obra => {
      ...
      switch (filtroEstado.value) {
        case 'sin_trabajar': return progreso === 0
        case 'en_progreso': return progreso > 0 && progreso < 100
        case 'completado': return progreso === 100
        default: return true
      }
    })
  }
  ...
  montajeStore.obras = [
    {
      ...
      instrumentosRequeridos: [
        { instrumentoId: '1', nombre: 'Violín I', cantidad: 8, nivel: 'avanzado' as const, esObligatorio: true },
        { instrumentoId: '2', nombre: 'Violín II', cantidad: 6, nivel: 'intermedio' as const, esObligatorio: true },
        { instrumentoId: '3', nombre: 'Viola', cantidad: 4, nivel: 'intermedio' as const, esObligatorio: true },
        { instrumentoId: '4', nombre: 'Cello', cantidad: 4, nivel: 'avanzado' as const, esObligatorio: true }
      ],
      repertorioId: 'rep1',
