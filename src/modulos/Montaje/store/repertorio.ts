...
import type { Repertorio, ParticipanteRepertorio, FiltrosMontaje, MetricasProgreso } from '../types';
...
export const useRepertorioStore = defineStore('repertorio-montaje', () => {
  ...
  const repertoriosProximos = computed(() =>
    repertorios.value
      ...
      .sort(
        (a, b) =>
          (a.fechaConcierto?.toDate().getTime() || 0) - (b.fechaConcierto?.toDate().getTime() || 0),
      ),
  );
  ...
  const estadisticasGenerales = computed(() => {
    ...
    const progresoPromedio =
      repertorios.value.reduce((sum, r) => sum + r.metadatos.progresoPorcentaje, 0) / total || 0;
   ...
   */
  const createRepertoireItem = async (
    datos: Omit<Repertorio, 'id' | 'fechaCreacion' | 'auditoria'>,
  ) => {
    isLoading.value = true;
    error.value = null;
    ...
    try {
      ...
      const nuevoRepertorio: Omit<Repertorio, 'id'> = {
        ...
        estado: EstadoRepertorio.PLANIFICANDO,
        metadatos: {
  ...
  const updateParticipant = async (
    ...
    cambios: Partial<ParticipanteRepertorio>,
  ) => {
    const repertorio = repertorios.value.find((r) => r.id === repertorioId);
    if (!repertorio) {
      throw new Error('Repertorio no encontrado');
  ...
  const removeParticipant = async (repertorioId: string, userId: string) => {
    ...
    const participantesActualizados = repertorio.participantes.filter(
      (p) => p.estudianteId !== userId,
    );
    await updateRepertoireItem(repertorioId, { participantes: participantesActualizados });
  };
