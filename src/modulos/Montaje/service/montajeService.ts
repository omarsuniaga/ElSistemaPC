...
class MontajeService {
  ...
  async obtenerObras(repertorioId: string): Promise<Obra[]> {
    try {
      ...
      obras = obras.filter(obra => {
        ...
        const matchesRepertorio = !repertorioId || repertorioId === 'default-repertorio' || obra.repertorioId === repertorioId;
  ...
  async crearNotificacion(datos: Omit<NotificacionMontaje, 'id'>): Promise<string> {
    try {
      ...
      const docRef = await addDoc(collection(db, this.notificacionesCollection), notificacionCompleta);
      return docRef.id;
    } catch (error) {
   ...
   */
  async obtenerNotificaciones(destinatarioId: string, limit: number = 20): Promise<NotificacionMontaje[]> {
    try {
      const q = query(
   ...
   */
  async obtenerEvaluaciones(obraId: string, tipo: 'continua' | 'final' = 'continua'): Promise<EvaluacionContinua[] | EvaluacionFinal[]> {
    try {
      const collectionName = tipo === 'continua' ? this.evaluacionesCollection : this.evaluacionesFinalesCollection;
  ...
  async buscarObras(filtros: FiltrosMontaje): Promise<Obra[]> {
    try {
      ...
      // Solo obras activas
      q = query(q, where('auditoria.activo', '==', true));      const snapshot = await getDocs(q);
      let obras = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })) as Obra[];
      ...
      if (filtros.dificultad) {
        obras = obras.filter(obra => 
          obra.metadatos?.complejidadGeneral === filtros.dificultad
        );
      }
  ...
  async obtenerEstadisticasObra(obraId: string): Promise<any> {
    try {
      ...
      const evaluacionesContinuas = await this.obtenerEvaluaciones(obraId, 'continua') as EvaluacionContinua[];
      const evaluacionesFinales = await this.obtenerEvaluaciones(obraId, 'final') as EvaluacionFinal[];

      // Calcular estadísticas
      const totalEvaluaciones = evaluacionesContinuas.length + evaluacionesFinales.length;
      const promedioEvaluacionesContinuas = evaluacionesContinuas.reduce((sum, e) => 
        sum + (e.metadatos?.porcentajeCumplimiento || 0), 0) / (evaluacionesContinuas.length || 1);
  ...
  async obtenerPlanAccion(obraId: string): Promise<PlanAccion | null> {
    try {
      ...
      const snapshot = await getDocs(q);
      const plan = snapshot.empty ? null : { id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as PlanAccion;
   ...
   */
  async cambiarEstadoCompass(compassNumber: number, nuevoEstado: any, cambio: CambioEstadoCompass): Promise<void> {
    try {
