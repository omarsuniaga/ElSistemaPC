...
vi.mock('firebase/firestore', async (importOriginal) => {
  ...
  return {
    ...
    where: vi.fn(),
    orderBy: vi.fn(),
    limit: vi.fn(), // Added limit
    limitToFirst: vi.fn(), // Added limitToFirst
    increment: vi.fn((value) => value), // Added increment
    onSnapshot: vi.fn(),
    Timestamp: {
      now: () => ({ toDate: () => new Date() }),
      fromDate: (date: Date) => ({ toDate: () => date }),
    },
    serverTimestamp: vi.fn(() => new Date()), // Ensure serverTimestamp is mocked
  };
});
