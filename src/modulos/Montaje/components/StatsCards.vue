...
const stats = computed(() => {
  ...
  const activeWorks = props.works.filter(w => 
    w.status === 'en_montaje' || w.status === 'en_estudio' || w.status === 'en_pulimiento'
  ).length
  const completedWorks = props.works.filter(w => 
    w.status === 'presentada' || w.status === 'lista'
  ).length
