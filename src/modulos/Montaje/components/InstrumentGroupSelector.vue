    ...
    <div v-if="selectedInstruments.length > 0" class="mt-4 p-3 bg-blue-50 rounded-md">
      <div class="flex justify-between items-center">
        <span class="text-sm font-medium text-blue-800">
          {{ selectedInstruments.length }} {{ selectedInstruments.length === 1 ? 'instrumento' : 'instrumentos' }} seleccionado(s)
        </span>
        <button 
...
const filteredInstruments = computed(() => {
  ...
  return allInstruments.value.filter(instr => 
    familyInstruments.includes(instr.id)
  );
});
...
watch(() => props.classId, async (newId) => {
