...
export function useMontajeAnalytics() {
  ...
  const calculateTimeMetrics = (obras: Obra[], planes: PlanAccion[]) => {
    ...
    const obrasThisMonth = obras.filter(obra => {
      ...
      return obraDate.getMonth() === now.getMonth() && 
             obraDate.getFullYear() === now.getFullYear()
    })
    ...
    const averageCompletionTime = obrasCompletadas.length > 0 
      ? obrasCompletadas.reduce((sum, obra) => {
        ...
        }, 0) / obrasCompletadas.length / (1000 * 60 * 60 * 24) // días
      : 0
  ...
  const calculateEvaluationMetrics = (evaluaciones: EvaluacionContinua[]) => {
    ...
    const averageScore = evaluaciones.reduce((sum, evaluacion) => sum + evaluacion.metadatos.puntuacionTotal, 0) / evaluaciones.length

    // Distribución de puntuaciones
    const scoreDistribution = evaluaciones.reduce((acc, evaluacion) => {
    ...
    const sortedEvaluaciones = [...evaluaciones].sort((a, b) => 
      new Date(b.fecha.toDate()).getTime() - new Date(a.fecha.toDate()).getTime()
    )
    ...
    if (recent.length > 0 && previous.length > 0) {
      const recentAvg = recent.reduce((sum, evaluation) => sum + evaluation.metadatos.puntuacionTotal, 0) / recent.length
      const previousAvg = previous.reduce((sum, evaluation) => sum + evaluation.metadatos.puntuacionTotal, 0) / previous.length
  ...
  const calculateProgressMetrics = (obras: Obra[]) => {
    const progressDistribution = obras.reduce((acc, obra) => {
    ...
    const inProgress = obras.filter(obra => 
      obra.metadatos.progresoPorcentaje > 0 && obra.metadatos.progresoPorcentaje < 100
    ).length

    const completed = obras.filter(obra => 
      obra.metadatos.progresoPorcentaje === 100
    ).length

    const notStarted = obras.filter(obra => 
    ...
    return {
      ...
      completed,
      notStarted,
      averageProgress: obras.length > 0 
  ...
  const calculateInstitutionalKPIs = (
    ...
    const completionEfficiency = timeMetrics.totalObras > 0 
      ? (timeMetrics.obrasCompletadas / timeMetrics.totalObras) * 100 
      : 0
  ...
  const calculatePeriodComparison = (
    ...
    const currentAvgScore = currentEvaluaciones.length > 0
      ? currentEvaluaciones.reduce((sum, evaluation) => sum + evaluation.metadatos.puntuacionTotal, 0) / currentEvaluaciones.length
      : 0

    const previousAvgScore = previousEvaluaciones.length > 0
      ? previousEvaluaciones.reduce((sum, evaluation) => sum + evaluation.metadatos.puntuacionTotal, 0) / previousEvaluaciones.length
      : 0
  ...
  const generateTimelineChart = (obras: Obra[], evaluaciones: EvaluacionContinua[]) => {
    ...
    obras.forEach(obra => {
      ...
      const monthIndex = last12Months.findIndex(month => {
        ...
        return monthDate.getMonth() === obraDate.getMonth() && 
    ...
    evaluaciones.forEach(evaluacion => {
      ...
      const monthIndex = last12Months.findIndex(month => {
        const [monthName, year] = month.month.split(' ')
        const monthDate = new Date(`${monthName} 1, ${year}`)
        return monthDate.getMonth() === evalDate.getMonth() && 
  ...
  const generateDistributionChart = (evaluaciones: EvaluacionContinua[]) => {
    ...
    return scoreRanges.map(range => {
      ...
      const count = evaluaciones.filter(evaluacion => 
  ...
  const generateComparisonChart = (obras: Obra[]) => {
    const progressRanges = {
      ...
      'En progreso': obras.filter(obra => obra.metadatos.progresoPorcentaje > 0 && obra.metadatos.progresoPorcentaje < 100).length,
