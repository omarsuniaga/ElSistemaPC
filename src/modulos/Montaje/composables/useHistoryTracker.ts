...
import type {HistorialCambio, TipoCambio, Obra, Plan, Frase, ControlVersion} from "../types"
...
export function useHistoryTracker() {
  ...
  const filteredHistory = computed(() => {
    return historyRecords.value.filter((record) => {
      ...
      const matchesDateFrom =
        !filterDateFrom.value || new Date(record.fechaCambio) >= new Date(filterDateFrom.value)
      const matchesDateTo =
        !filterDateTo.value || new Date(record.fechaCambio) <= new Date(filterDateTo.value)
      const matchesUser =
        !filterUser.value || record.usuario.toLowerCase().includes(filterUser.value.toLowerCase())
      const matchesSearch =
        !searchQuery.value ||
  ...
  const loadHistory = async (entidadId?: string, entidadTipo?: string, limit = 100) => {
    loading.value = true
    error.value = null

    try {
      historyRecords.value = await montajeService.getHistorialCambios(entidadId, entidadTipo, limit)
    } catch (err) {
