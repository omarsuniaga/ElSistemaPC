...
export function useHeatmapTracking(obraId: string | Ref<string>) {
  ...
  const finalizarDragSelection = (compases: { numero: number; instrumento: TipoInstrumento; x: number; y: number }[]) => {
    ...
    const compasesEnArea = compases.filter(c => 
    ...
    try {
      ...
      const compasesActualizados = compasesSeleccionados.value.map(compas => {
        const compasActualizado = {
          ...compas,
          estado,
          observacion: estado === EstadoCompass.CON_DIFICULTAD ? (observacion || compas.observacion) : '',
  ...
  const registrarAccion = (tipo: 'seleccion' | 'cambio-estado', compases: CompasSeleccionado[], estado?: EstadoCompass) => {
