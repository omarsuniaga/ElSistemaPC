...
describe('InstrumentGroupSelector', () => {
  ...
  describe('Renderizado básico', () => {
    ...
    it('debe mostrar todas las familias de instrumentos', () => {
      ...
      const familyButtons = wrapper.findAll('button').filter(btn => 
        ['Todos', 'Cuerdas', 'Vientos Madera', 'Vientos Metal', 'Percusión', 'Teclados', 'Otros']
          .includes(btn.text())
      );
  ...
  describe('Selección de familias', () => {
    ...
    it('debe filtrar instrumentos según la familia seleccionada', async () => {
      ...
      const instrumentLabels = wrapper.findAll('.text-sm').filter(el => 
        el.text().includes('Violín') || el.text().includes('Viola') || el.text().includes('Cello')
      );
  ...
  describe('Funcionalidad de limpiar selección', () => {
    it('debe mostrar el botón de limpiar cuando hay instrumentos seleccionados', () => {
      ...
      const clearButton = wrapper.find('button').filter(btn => 
        btn.text().includes('Limpiar selección')
      );
