...
describe('Analytics Components - Data Real Only', () => {
  it('StudentAnalytics does not use mock data', () => {
    ...
    expect(code).not.toMatch(/mock|generateMock|Gabriel Mendez|Sara Linares|Miguel Torres|Valentina Ruiz/)
  })
  ...
  it('StudentAnalytics renders with real store data', async () => {
    const wrapper = mount(StudentAnalytics, {
      global: {
        mocks: {
          analyticsStore: { loading: false, error: '', attendanceMetrics: {}, studentMetrics: { atRiskStudents: [] } },
