<template>
  <div class="attendance-analytics">
    ...
    <div v-else class="space-y-8">
      ...
      <div v-else-if="selectedView === 'table'" class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        <div class="p-4">
        ...
        <div class="grid grid-cols-7 gap-2">
          <div v-for="(day, idx) in calendarData" :key="idx" 
            :class="[
              ...
              day.isCurrentMonth ? 'bg-gray-50 dark:bg-gray-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-400',
              day.isToday ? 'ring-2 ring-indigo-500' : ''
            ]">
            <span class="text-sm block mb-1">{{ day.day }}</span>
            <div v-if="day.data" class="flex justify-center space-x-1">
              <span class="w-2 h-2 rounded-full" :style="{ backgroundColor: getRateColor(day.data.rate) }"></span>
              <span v-if="day.data.absent > 0" class="w-2 h-2 rounded-full bg-red-500"></span>
            </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="h-60">
            <Bar 
              :data="weekdayAttendanceChartData" 
              :options="chartOptions" 
            />
          </div>
          <div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        ...
        <div class="space-y-4">
          <div v-for="(classItem, index) in sortedClassAttendanceData" :key="index" class="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              ...
              <div class="text-right">
                <p class="text-sm">
                  <span :class="{
                    ...
                    'text-yellow-600': classItem.attendanceRate >= 75 && classItem.attendanceRate < 90,
...
import { format, parseISO, isToday, isSameMonth, startOfMonth, endOfMonth, eachDayOfInterval, getDay } from 'date-fns'
...
async function loadAttendanceMetrics() {
  ...
  try {
    ...
    if (selectedPeriod.value === 'day') {
      ...
      metrics.value.justificationRate = totalAbsent > 0 ? Math.round((totalJustified / totalAbsent) * 100) : 0
    ...
    for (let i = 0; i < 10; i++) {
      ...
      classAttendance.push({
        id: `class-${i}`,
        name: ['Piano', 'Guitarra', 'Violín', 'Flauta', 'Teoría Musical', 'Canto', 'Batería'][i % 7] + ' - ' + ['Nivel 1', 'Nivel 2', 'Nivel 3'][Math.floor(i / 3) % 3],
