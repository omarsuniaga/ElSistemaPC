    ...
    <div v-else class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
          ...
          <div class="h-72">
            <Doughnut :data="instrumentDistributionChart" :options="doughnutOptions" />
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
          ...
          <div class="h-72">
            <Bar :data="levelDistributionChart" :options="barChartOptions" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        ...
        <div class="h-60">
          <Line :data="attendanceTrendChart" :options="lineChartOptions" />
        </div>
      </div>
              ...
              <tr v-for="student in filteredAtRiskStudents" :key="student.id">
                  ...
                  <span
                    ...
                    :class="{
                      'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200':
                        student.riskType === 'attendance',
                      'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200':
                        student.riskType === 'performance',
                      'bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-200':
                        student.riskType === 'payments',
                    }"
                  >
                ...
                <td class="px-4 py-2 whitespace-nowrap">
                  {{ student.currentValue
                    ...
                    student.riskType === "attendance" || student.riskType === "performance"
                      ? "%"
                      : ""
                  }}
                </td>
                <td class="px-4 py-2">
                  <button class="text-blue-600 hover:text-blue-800 text-sm mr-2">Contactar</button>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="h-60">
            <Bar :data="activityByMonthChart" :options="barChartOptions" />
          </div>
          ...
          <div class="space-y-4">
            <div class="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              ...
              <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                Los estudiantes son más activos los días
                <span class="font-bold">Miércoles y Viernes</span>, especialmente entre las
                <span class="font-bold">16:00 y 19:00</span> horas.
              </p>
            </div>
            ...
            <div class="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              ...
              <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                El <span class="font-bold">75%</span> de los estudiantes participan activamente en
                al menos 2 actividades extracurriculares.
              </p>
            </div>
    ...
    if (selectedStudent.value) {
      ...
      metrics.value = {
        ...
        atRiskPercentage: Math.round(
          (identifyAtRiskStudents().length / studentsStore.students.length) * 100,
        ),
...
function calculateStudentGrowth(): number {
  ...
  if (enrollmentTrends && enrollmentTrends.length >= 2) {
    ...
    if (previousEntry.totalStudents > 0) {
      const growthRate =
        ((latestEntry.totalStudents - previousEntry.totalStudents) / previousEntry.totalStudents) *
        100;
  ...
  if (previousPeriodStudents > 0) {
    return Number(
      (((currentStudentCount - previousPeriodStudents) / previousPeriodStudents) * 100).toFixed(1),
    );
  }
...
function calculateAveragePerformance(): number {
  ...
  try {
    ...
    if (qualificationStore?.qualifications?.length > 0) {
      ...
      const sum = qualificationStore.qualifications.reduce(
        (acc, qual) => acc + (qual.score || 0),
        0,
      );
  ...
  if (studentsStore.students && studentsStore.students.length > 0) {
    ...
    const studentsWithPerformance = studentsStore.students.filter(
      (student) => student.performance || student.averageScore || student.qualifications,
    );
...
function calculateMonthlyActivity(): any[] {
  ...
  // Convert to array format for chart
  return Object.values(activityByMonth).sort((a: any, b: any) => {
