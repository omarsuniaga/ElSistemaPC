...
async function loadAttendanceData() {
  try {
    ...
    const records = attendanceStore.records.filter(record => {
      ...
      return (
        record.studentId === props.studentId &&
    ...
    const justified = records.filter(r => r.status === 'justificado' || r.status === 'JUSTIFIED').length
...
async function loadQualificationsData() {
  try {
    ...
    const evaluations = qualificationStore.qualifications.filter(qual => {
      ...
      return (
        qual.studentId === props.studentId &&
        evalDate >= startDate &&
        evalDate <= endDate
      )
    })
    ...
    const sum = evaluations.reduce((acc, eval_) => acc + (eval_.calificacion || eval_.grade || 0), 0)
    ...
    <div v-else>
        ...
        <div class="flex justify-between items-start">
          <div>
            ...
            <div class="flex flex-wrap gap-2 mt-1">
              ...
              <span v-if="performanceData" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" :class="statusClass">
          ...
          <div v-if="!props.compact">
            <select v-model="$props.period" class="mt-1 block w-full pl-3 pr-10 py-1 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
      ...
      <div v-if="performanceData" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            ...
            <div class="mt-1 flex items-center">
              ...
              <span v-if="performanceData.trend.attendance != 0" class="ml-2 flex items-center text-xs font-medium">
                <svg v-if="performanceData.trend.attendance > 0" class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span :class="performanceData.trend.attendance > 0 ? 'text-green-500' : 'text-red-500'">
            ...
            <div class="mt-1 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div 
                ...
                :class="{
                  ...
                  'bg-yellow-500': performanceData.attendanceRate >= 75 && performanceData.attendanceRate < 90,
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            ...
            <div class="mt-1 flex items-center">
              ...
              <span v-if="performanceData.trend.grade != 0" class="ml-2 flex items-center text-xs font-medium">
                <svg v-if="performanceData.trend.grade > 0" class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"></path>
                </svg>
                <svg v-else class="w-4 h-4 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 14l-7 7m0 0l-7-7m7 7V3"></path>
            ...
            <div class="mt-1 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div 
                ...
                :class="{
                  ...
                  'bg-emerald-500': performanceData.averageGrade >= 8 && performanceData.averageGrade < 9,
                  'bg-blue-500': performanceData.averageGrade >= 7 && performanceData.averageGrade < 8,
                  'bg-yellow-500': performanceData.averageGrade >= 6 && performanceData.averageGrade < 7,
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            ...
            <div class="mt-1 flex items-center">
              ...
              <span class="ml-1 text-gray-500 dark:text-gray-400">/{{ attendanceData?.total || 0 }}</span>
            </div>
            <div class="mt-1 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div v-if="qualificationsData?.evaluations?.length" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        ...
        <div v-if="!props.compact && qualificationsData?.evaluations?.length > 0" class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
              ...
              <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                <tr v-for="(evaluation, index) in qualificationsData.evaluations" :key="index">
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                    {{ format(new Date(evaluation.fecha || evaluation.date), 'dd/MM/yyyy', { locale: es }) }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="px-2 py-1 text-xs inline-flex leading-5 font-semibold rounded"
                      :class="{
                        'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400': evaluation.calificacion >= 9 || evaluation.grade >= 9,
                        'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400': (evaluation.calificacion >= 7 && evaluation.calificacion < 9) || (evaluation.grade >= 7 && evaluation.grade < 9),
                        'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400': (evaluation.calificacion >= 6 && evaluation.calificacion < 7) || (evaluation.grade >= 6 && evaluation.grade < 7),
                        'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400': (evaluation.calificacion < 6) || (evaluation.grade < 6),
                      }"
                    >
