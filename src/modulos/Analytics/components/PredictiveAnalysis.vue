...
const generatePredictions = async () => {
  ...
  try {
    ...
    predictiveData.value.dropoutRiskStudents = [
      { id: '1', name: 'Luis Ramírez', instrument: 'Violín', riskScore: 78, factors: ['Asistencia baja', 'Rendimiento decreciente'] },
      { id: '2', name: 'Ana Flores', instrument: 'Piano', riskScore: 65, factors: ['Falta de práctica', 'Ausencias frecuentes'] },
      { id: '3', name: 'Miguel Torres', instrument: 'Flauta', riskScore: 82, factors: ['Conflicto de horarios', 'Dificultad con técnica'] }
    ];
    
    // Opportunities for improvement
    predictiveData.value.improvementOpportunities = [
      { area: 'Asistencia en clases de cuerdas', impact: 'Alto', suggestion: 'Ajustar horarios a tarde-noche' },
      { area: 'Rendimiento en teoría musical', impact: 'Medio', suggestion: 'Introducir material interactivo' },
      { area: 'Participación en ensamble', impact: 'Alto', suggestion: 'Programar presentaciones mensuales' }
    ];
...
<template>
  <div class="predictive-analysis p-4 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      <ChartContainer title="Predicción de Asistencia" icon="ChartBarIcon" :isLoading="isLoading">
        <Line v-if="!isLoading" :data="attendanceTrendChart" :options="lineChartOptions" height="250" />
      </ChartContainer>
      
      <ChartContainer title="Predicción de Rendimiento" icon="ChartBarIcon" :isLoading="isLoading">
        <Line v-if="!isLoading" :data="performanceTrendChart" :options="lineChartOptions" height="250" />
      </ChartContainer>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Factores</th>
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Acciones Recomendadas</th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="student in predictiveData.dropoutRiskStudents" :key="student.id">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ student.name }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ student.instrument }}</td>
              ...
              <td class="px-6 py-4 text-sm text-gray-900">
                <button class="text-indigo-600 hover:text-indigo-900 font-medium">Plan de intervención</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      <!-- Teacher comparison -->
      <ChartContainer title="Comparativa de Profesores" icon="UsersIcon" :isLoading="isLoading">
      ...
      <!-- Seasonal patterns -->
      <ChartContainer title="Patrones Estacionales" icon="CalendarIcon" :isLoading="isLoading">
        <div v-if="!isLoading" class="h-72">
          <Bar 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
      ...
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <div v-else class="space-y-4">
          <div v-for="(opportunity, idx) in predictiveData.improvementOpportunities" :key="idx" class="bg-gray-50 p-4 rounded-md">
            ...
            <div class="flex">
              ...
              <div class="ml-3">
                <p class="text-sm text-green-700">
                  Los alumnos de piano muestran mejora constante en rendimiento (+5% en comparación con el trimestre anterior)
                </p>
              </div>
            </div>
          </div>
            ...
            <div class="flex">
              ...
              <div class="ml-3">
                <p class="text-sm text-yellow-700">
                  Las clases de teoría musical tienen una tasa de asistencia 15% menor que las clases prácticas
                </p>
              </div>
            </div>
          </div>
