...
import { optimizedAttendanceQueries } from '../modulos/Attendance/services/optimizedQueries';
...
export function useOptimizedAttendance() {
   ...
   */
  const loadMore = async (startDate: string, endDate: string, pageSize: number = 50) => {
   ...
   */
  const getStats = async (startDate: string, endDate: string, classId?: string) => {
    try {
      loading.value = true;
      error.value = null;

      return await optimizedAttendanceQueries.getAttendanceStatsCached(startDate, endDate, classId);
    } catch (err: any) {
  ...
  const getFilteredRecords = computed(() => {
    return (filters: {classId?: string; studentId?: string; status?: string}) => {
