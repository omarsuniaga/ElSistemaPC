...
import { useObservationsStore, type ObservationFilters, type ObservationData } from '../stores/observations';
...
export function useTeacherObservations() {
  ...
  const teacherFilters = computed((): ObservationFilters => ({
  ...
  const myObservationStats = computed(() => {
    ...
    return {
      ...
      byType: myObservations.reduce((acc, obs) => {
...
export function useAdminObservations() {
  ...
  const fetchObservationsByTeacher = async (teacherId: string, additionalFilters: Partial<ObservationFilters> = {}) => {
    checkAdminPermissions();
    return await fetchAllObservations({ ...additionalFilters, authorId: teacherId });
  };

  // Obtener observaciones por clase
  const fetchObservationsByClass = async (classId: string, additionalFilters: Partial<ObservationFilters> = {}) => {
    checkAdminPermissions();
    return await fetchAllObservations({ ...additionalFilters, classId });
  };

  // Obtener observaciones por rango de fechas
  const fetchObservationsByDateRange = async (dateFrom: string, dateTo: string, additionalFilters: Partial<ObservationFilters> = {}) => {
    checkAdminPermissions();
    return await fetchAllObservations({ ...additionalFilters, dateFrom, dateTo });
  };

  // Obtener observaciones que requieren seguimiento
  const fetchObservationsRequiringFollowUp = async (additionalFilters: Partial<ObservationFilters> = {}) => {
    checkAdminPermissions();
  ...
  const generateAdvancedAnalysis = async (filters: ObservationFilters = {}) => {
    try {
      ...
      const monthlyTrends = observations.reduce((acc, obs) => {
      ...
      const teacherAnalysis = observations.reduce((acc, obs) => {
      ...
      // Análisis por clase
      const classAnalysis = observations.reduce((acc, obs) => {
      ...
      const kpis = {
        ...
        followUpRate: (observations.filter(obs => obs.requiresFollowUp).length / observations.length) * 100,
        averageObservationsPerTeacher: observations.length / new Set(observations.map(obs => obs.authorId)).size,
        averageObservationsPerClass: observations.length / new Set(observations.map(obs => obs.classId)).size,
  ...
  const exportForAIAnalysis = async (filters: ObservationFilters = {}, format: 'json' | 'csv' = 'json') => {
    checkAdminPermissions();
    const observations = await fetchAllObservations(filters);
    ...
    const aiData = observations.map(obs => ({
      ...
      urgency: obs.priority === 'critica' ? 1 : obs.priority === 'alta' ? 0.7 : obs.priority === 'media' ? 0.4 : 0.1
    }));
    ...
    if (format === 'csv') {
      ...
      const csv = [
        headers.join(','),
        ...aiData.map(row => headers.map(h => `"${row[h as keyof typeof row] || ''}"`).join(','))
      ].join('\n');
      return csv;
    }
...
export function useObservationAnalytics() {
  ...
  const generateTeacherActivityReport = async (limit: number = 10) => {
    try {
      ...
      const teacherActivity = observations.reduce((acc, obs) => {
        const teacher = obs.authorName || obs.author;
