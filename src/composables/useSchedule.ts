...
export function useSchedule() {
  ...
  const classStats = computed(() => ({
    ...
    withoutStudents: classesStore.classes.filter((c) => !c.studentIds || c.studentIds.length === 0)
      .length,
  }))
  ...
  const validateScheduleConflicts = (schedule: any, classId?: string, teacherId?: string) => {
    ...
    if (typeof schedule === "object" && schedule.classroom) {
      const roomClasses = currentClasses.filter((c) => c.classroom === schedule.classroom)
    ...
    if (schedule.studentIds?.length > 0) {
      currentClasses.forEach((existingClass) => {
        const hasCommonStudents = existingClass.studentIds?.some((id: string) =>
          schedule.studentIds.includes(id)
        )
