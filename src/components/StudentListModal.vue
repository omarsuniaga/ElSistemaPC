    ...
    <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[80vh] flex flex-col">
      ...
      <div class="p-4 overflow-y-auto flex-grow">
        ...
        <div v-else-if="error" class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4">
          <p>{{ error }}</p>
          <button @click="loadStudents" class="mt-2 px-3 py-1 bg-red-200 hover:bg-red-300 rounded">
            Reintentar
          </button>
        </div>
        ...
        <div v-else>
          ...
          <div v-else>
            ...
            <ul class="divide-y divide-gray-200">
              <li v-for="student in filteredStudents" :key="student.id" class="py-2 flex items-center">
                <div class="flex-1">
                  <p class="font-medium">{{ student.nombre }} {{ student.apellido }}</p>
                  <p class="text-sm text-gray-500">{{ student.instrumento || 'Sin instrumento asignado' }}</p>
                </div>
                <div>
                  <span 
                    :class="{
...
const loadStudents = async () => {
  ...
  try {
    ...
    const studentsList = studentIds.map(studentId => {
      const studentData = studentsStore.students.find(s => s.id === studentId) || { nombre: 'Estudiante', apellido: 'Desconocido' }
