      ...
      <div class="flex h-[calc(95vh-150px)]">
            ...
            <div class="space-y-2">
              ...
              >
                <div class="flex items-center justify-between mb-1">
                  ...
                  <span class="text-xs px-2 py-1 rounded-full" :class="getCategoryClass(template.category)">
        ...
        <div class="flex-1 flex flex-col">
          <div class="p-6 overflow-y-auto">
            <div v-if="!selectedTemplate && !isCreating" class="text-center py-12 text-gray-500 dark:text-gray-400">
...
import { useWhatsAppPresets, type WhatsAppPreset, type MessageData } from '../composables/useWhatsAppPresets';
...
const { presets, loading, error, loadPresets, addPreset, updatePreset, deletePreset, processTemplate } = useWhatsAppPresets();
...
const availableVariables = [
  { key: '{studentName}', label: 'Nombre del estudiante', description: 'Nombre completo del estudiante' },
  { key: '{representanteName}', label: 'Nombre del representante', description: 'Nombre del padre/madre/representante' },
  { key: '{representantePhone}', label: 'Teléfono', description: 'Número de teléfono del representante' },
  ...
  { key: '{attendanceRate}', label: 'Porcentaje de asistencia', description: 'Porcentaje de asistencia del estudiante' },
  { key: '{absenceDetails}', label: 'Detalles de ausencias', description: 'Detalles específicos de las ausencias' }
];
...
const saveTemplate = async () => {
  ...
  try {
    ...
    if (isCreating.value) {
      ...
      emit('template-created', { ...templateData, id: '', createdAt: new Date(), updatedAt: new Date() });
    } else if (selectedTemplate.value && !selectedTemplate.value.isSystem) {
      await updatePreset(selectedTemplate.value.id, templateData);
      emit('template-updated', { ...templateData, id: selectedTemplate.value.id, createdAt: selectedTemplate.value.createdAt, updatedAt: new Date() });
    }
