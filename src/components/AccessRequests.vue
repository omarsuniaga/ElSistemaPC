...
<template>
  <div class="space-y-6">
    ...
    <!-- Sin solicitudes -->
    <div
      v-else-if="safeArrayLength(pendingUsers) === 0"
          ...
          <div class="space-y-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
              ...
              <div class="flex items-center gap-1 text-gray-600 dark:text-gray-400 sm:col-span-2">
                ...
                <span
                  >Registro:
                  {{ formatDate(safeGet(user, "createdAt", new Date().toISOString())) }}</span
                >
              </div>
              <div class="flex items-center gap-1 text-gray-600 dark:text-gray-400 sm:col-span-2">
                <CalendarIcon class="h-4 w-4 flex-shrink-0" />
                <span>
                  ...
                  {{
                    safeGet(user, "availability.type") === "complete"
                      ? "Tiempo Completo"
                      : "Tiempo Parcial"
                  }}
                </span>
              ...
              <div
                v-if="safeGet(user, 'availability.type') === 'partial'"
                class="sm:col-span-2 mt-1 p-2 bg-gray-50 dark:bg-gray-700 rounded-md"
              >
                <div class="text-xs text-gray-600 dark:text-gray-400">
                  <div class="font-medium mb-1">Horario disponible:</div>
                  <div
                    v-for="day in safeGet(user, 'availability.schedule', [])"
                    :key="day.day"
                    class="flex gap-1"
                  >
                    <template v-if="safeGet(day, 'enabled', false)">
                      ...
                      <span
                        >{{ safeGet(day, "startTime", "") }} -
                        {{ safeGet(day, "endTime", "") }}</span
                      >
                    </template>
                  </div>
                  <div
                    v-if="
                      !safeGet(user, 'availability.schedule', []).some((d) =>
                        safeGet(d, 'enabled', false)
                      )
                    "
                    class="italic"
                  >
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                    ...
                    <span class="font-semibold">{{
                      safeGet(selectedUser, "name", "Usuario")
                    }}</span>
                  ...
                  <div
                    v-if="error"
                    class="mb-4 p-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-sm rounded"
                  >
                    {{ error }}
                  </div>
                  ...
                  <div class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-md">
                    <p class="text-xs text-blue-700 dark:text-blue-300">
                      <span class="font-medium">Nota:</span> Los permisos del usuario serán
                      establecidos según el rol asignado.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">
                    ¿Estás seguro de que deseas rechazar el acceso de
                    <span class="font-semibold">{{ safeGet(selectedUser, "name", "Usuario") }}</span
                    >?
                  </p>
                  <div>
                    <label
