<template>
  <div class="p-4 mb-16 space-y-6 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg shadow">
    ...
    <div v-else-if="classReports.length > 0">
        ...
        <div class="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-800">
          <div class="flex items-center">
            ...
            <div class="ml-3">
              ...
              <p class="text-2xl font-bold text-green-900 dark:text-green-100">{{ totalPresentes }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800">
          <div class="flex items-center">
            ...
            <div class="ml-3">
              ...
              <p class="text-2xl font-bold text-yellow-900 dark:text-yellow-100">{{ totalTardes }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
          <div class="flex items-center">
            ...
            <div class="ml-3">
              ...
              <p class="text-2xl font-bold text-blue-900 dark:text-blue-100">{{ totalJustificados }}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            <div class="flex items-center justify-between mb-3">
              <h4 class="font-semibold text-gray-800 dark:text-gray-200">{{ classData.className }}</h4>
      ...
      <div v-if="selectedClassData" class="mt-8">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold">
            {{ selectedClassData.className }} - Estudiantes
          </h3>
          <button 
          ...
          >
            ...
            <div class="flex items-center justify-between">
              <div class="flex flex-col w-full">
                <div v-for="date in selectedClassData.relevantDates" :key="date" class="flex items-center gap-2 mb-1">
                  <span class="text-xs text-gray-500 dark:text-gray-400 min-w-[90px]">{{ formatDate(date) }}</span>
                  ...
                  <template v-else>
                    <span :class="getStatusClass(student.attendance[date])" class="text-xs font-semibold">
                      {{ getStatusSymbol(student.attendance[date]) === '✓' ? 'Presente' : (getStatusSymbol(student.attendance[date]) === '✗' ? 'Ausente' : getStatusSymbol(student.attendance[date])) }}
                    </span>
                  </template>
                </div>
              </div>
      ...
      <div v-if="selectedStudent" class="mt-8">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold">
            {{ selectedStudent.name }} - Detalle de Asistencias
          </h3>
          <button 
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
              ...
              <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-600">
                <tr v-for="date in selectedClassData?.relevantDates || []" :key="date" 
                  ...
                  <td class="px-4 py-3 text-sm" :class="{'text-blue-700 dark:text-blue-300': getJustificationText(selectedStudent.id, date) && getJustificationText(selectedStudent.id, date) !== 'Sin justificacion'}">
    ...
    <div v-else-if="!loading" class="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
      ...
      <p class="text-lg text-gray-500 dark:text-gray-400 mb-2">No hay datos para mostrar en el rango seleccionado</p>
...
const getStatusClass = (status: string) => {
  switch (status) {
    ...
    case 'T': return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300'
    case 'J': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300'
...
const getJustificationText = (studentId: string, date: string) => {
  ...
  const attendanceDoc = attendanceStore.attendanceDocuments.find(doc => 
    doc.fecha === date && doc.classId === selectedClassData.value?.classId
  )
...
const getObservationText = (classId: string, date: string) => {
  ...
  const attendanceDoc = attendanceStore.attendanceDocuments.find(doc => 
    doc.fecha === date && doc.classId === classId
  )
...
const fetchReport = async () => {
  try {
    ...
    await Promise.all([
      ...
      attendanceStore.fetchAttendanceDocumentsByTeacher(currentTeacherId.value, from.value, to.value)
    ])
