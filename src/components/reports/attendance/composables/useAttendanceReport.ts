...
export function useAttendanceReport(props: {teacherId: string}) {
  ...
  const debugStores = () => {
    ...
    console.log(
      "📊 Attendance By Class:",
      Object.keys(attendanceByClass.value).map((classId) => ({
        classId,
        documentos: attendanceByClass.value[classId].length,
        fechas: Array.from(new Set(attendanceByClass.value[classId].map((doc: any) => doc.fecha)))
          .length,
      }))
    )
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      console.log(
        `📆 Generando informe de ${start} a ${end} para profesor ${currentTeacherId.value}`
      )
      ...
      classReports.value = await Promise.all(
        classes.map(async (classObj: any, index: any) => {
          console.log(
            `📝 Procesando clase ${index + 1}/${classes.length}: ${classObj.nombre || "Sin nombre"}`
          )

          const classAttendanceDocs = attendanceByClass.value[classObj.id] || []
          console.log(
            `📊 Documentos de asistencia para clase ${classObj.nombre}: ${classAttendanceDocs.length}`
          )
          ...
          if (studentIds && Array.isArray(studentIds) && studentIds.length > 0) {
            console.log(
              `👥 Procesando ${studentIds.length} estudiantes de la clase ${classObj.nombre}`
            )
          ...
          const relevantDates = Array.from(
            new Set(classAttendanceDocs.map((doc: any) => doc.fecha || ""))
          )
            .filter(Boolean)
            .sort()
          ...
          console.log(
            `✅ Clase ${classObj.nombre} procesada: ${students.length} estudiantes, ${relevantDates.length} fechas, ${observations.length} observaciones`
          )
