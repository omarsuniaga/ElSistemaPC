...
export function useAttendanceData() {
  ...
  const fetchAttendanceData = async (teacherId: string, startDate: string, endDate: string) => {
    try {
      ...
      const attendanceDocs = await attendanceStore.fetchAttendanceDocumentsByTeacher(
        teacherId,
        startDate,
        endDate,
      );
      ...
      if (attendanceDocs.length > 0) {
        console.log(
          ...
          attendanceDocs.slice(0, 3).map((doc) => ({
            id: doc.id,
            fecha: doc.fecha,
            classId: doc.classId,
      ...
      // Organizar por clase
      const docsByClass = attendanceDocs.reduce(
        (acc, doc) => {
      ...
      console.log(
        '📋 Documentos organizados por clase:',
        Object.keys(docsByClass).map((classId) => ({
          classId,
  ...
  const loadTeacherClasses = async (teacherId: string) => {
    try {
      ...
      console.log(
        '📊 Primeras 3 clases en store:',
        classesStore.classes?.slice(0, 3).map((c: any) => ({
          id: c.id,
          nombre: c.nombre,
      ...
      const primaryClasses = (classesStore.classes || []).filter(
  ...
  const debugAttendanceStore = () => {
    ...
    if (attendanceStore.attendanceDocuments.length > 0) {
      ...
      const docsByTeacher = attendanceStore.attendanceDocuments.reduce(
        (acc, doc) => {
  ...
  const debugAllStores = () => {
    ...
    if (studentsStore.students?.length > 0) {
      console.log(
        ...
        studentsStore.students.slice(0, 3).map((s) => ({
          id: s.id,
          nombre: s.nombre,
          email: s.email,
        })),
    ...
    if (teachersStore.teachers?.length > 0) {
      console.log(
        '  - Primeros 3 maestros:',
        teachersStore.teachers.slice(0, 3).map((t) => ({
          id: t.id,
          nombre: t.nombre,
          email: t.email,
        })),
