...
export const calculateStudentAttendancePercentage = (student: any, relevantDates?: string[]): number => {
...
export const getDayAttendancePercentage = (classData: any, date: string): number => {
  const present = countStatusForDate(classData, date, 'P') + countStatusForDate(classData, date, 'J')
  const total = classData.students.filter((s: any) => s.attendance[date] && s.attendance[date] !== '-').length
