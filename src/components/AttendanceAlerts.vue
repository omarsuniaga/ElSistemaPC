        ...
        <div v-for="alert in getHighPriorityAlerts" :key="alert.id" class="alert alert-danger">
          ...
          <div class="alert-body">
            ...
            <div v-if="alert.details" class="alert-details">
              <template v-if="alert.type === 'ConsecutiveAbsences'">
                <p>Ausencias consecutivas: {{ alert.details.absences }}</p>
              </template>
              <template v-else-if="alert.type === 'LowAttendance'">
                <p>Tasa de asistencia: {{ (alert.details.attendanceRate * 100).toFixed(1) }}%</p>
              </template>
              <template v-else-if="alert.type === 'LatePattern'">
