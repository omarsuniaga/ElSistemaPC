...
const students = computed(() => {
  ...
  return studentsStore.students.filter(s => 
    currentClass.value?.studentIds?.includes(s.id)
  );
});
...
const nextLevelClass = computed(() => {
  ...
  return classesStore.classes.find(c => 
    c.instrument === currentClass.value?.instrument && 
    c.level === nextLevel
  );
});
...
const progressStudents = async () => {
  ...
  try {
    ...
    const updatedCurrentStudents = (currentClass.value.studentIds || [])
      .filter(id => !selectedStudents.value.includes(id));
      ...
      <div class="space-y-2">
        <div v-for="student in students" :key="student.id"
          ...
          <div class="flex items-center">
            ...
            >
              <CheckIcon v-if="selectedStudents.includes(student.id)" 
