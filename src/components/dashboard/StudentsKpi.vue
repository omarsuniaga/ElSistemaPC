...
const trend = computed(() => {
  if (props.previousAttendancePercentage === 0 || props.attendancePercentage === props.previousAttendancePercentage) {
    return 'neutral';
  }
...
<template>
  ...
  >
    ...
    <div v-if="showLowAttendanceWarning" class="text-xs px-2 py-1 bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 rounded mb-3">
    ...
    <div v-if="previousAttendancePercentage > 0" class="text-xs mt-1">
      <span :class="{
        ...
        <span v-else>→</span>
        {{ Math.abs(attendancePercentage - previousAttendancePercentage).toFixed(1) }}% respecto a ayer
      </span>
    </div>
  </KpiCard>
</template>
