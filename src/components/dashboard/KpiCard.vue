...
<template>
  <div class="kpi-card rounded-lg shadow-lg transition-all duration-300 dark:bg-gray-800 bg-white overflow-hidden" 
    ...
    >
      <div class="flex items-center">
        <div :class="`text-${color}-600 dark:text-${color}-400 bg-${color}-100 dark:bg-${color}-900 rounded-full p-2 shadow-sm mr-3`">
        ...
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
          {{ title }}
          <span v-if="isMinimized" class="ml-2 text-sm font-normal text-gray-500 dark:text-gray-400">
            ({{ value }})
          </span>
        </h3>
      </div>
      ...
      <div class="text-gray-400 dark:text-gray-500">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" v-if="!isMinimized" />
