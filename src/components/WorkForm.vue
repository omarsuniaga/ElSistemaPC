...
const availableStudentsByInstrument = (instrumentName: string) => {
  return studentsStore.students.filter(student => 
    student.instrumento.toLowerCase() === instrumentName.toLowerCase()
  )
}
...
const workSchema = yup.object().shape({
  ...
  duration: yup.string().matches(/^\d{2}:\d{2}$/, 'La duración debe estar en el formato MM:SS').required('La duración es obligatoria'),
  difficulty: yup.string().oneOf(['beginner', 'intermediate', 'advanced'], 'Dificultad inválida').required('La dificultad es obligatoria'),
...
<template>
  <Dialog :open="true" @close="emit('cancel')" class="relative z-50">
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  class="input"
                  @change="addInstrument($event.target.value); $event.target.value = ''"
                >
            ...
            <div class="space-y-4">
              <div
                v-for="instrument in formData.instruments"
                :key="instrument.id"
                class="card"
              >
                ...
                <div class="mt-4">
                  ...
                  <select
                    v-model="instrument.studentProgress"
                    multiple
                    class="input"
                  >
                    <option
                ...
                <!-- Compases -->
                <div class="mt-4">
                  <label class="block text-sm font-medium mb-1">Compases ({{ instrument.measures.length }})</label>
