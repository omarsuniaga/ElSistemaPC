<template>
  ...
  >
    ...
    <div
      ...
      class="bg-yellow-50 border-l-4 border-yellow-500 p-4 dark:bg-yellow-900/20 dark:border-yellow-600 mb-4"
    >
      <div class="flex items-center">
        <div class="flex-shrink-0">
        ...
        <div class="ml-3">
          <p class="text-sm text-yellow-700 dark:text-yellow-300 font-bold">DEBUG INFO</p>
    ...
    <div class="flex flex-wrap gap-2 items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
      <div class="flex items-center">
        ...
        <input
          v-model="from"
          type="date"
          class="border rounded px-2 py-1 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-600 focus:ring-primary-500 focus:border-primary-500"
        />
      </div>
      <div class="flex items-center">
      ...
      <div class="flex gap-2 ml-auto">
        <button
          class="px-2 py-1 text-xs rounded bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-gray-200"
        ...
        <button
          class="px-3 py-1 text-xs font-medium rounded bg-blue-600 hover:bg-blue-700 text-white dark:bg-blue-500 dark:hover:bg-blue-600"
        ...
        <button
          class="px-3 py-1 text-xs font-medium rounded bg-primary-600 hover:bg-primary-700 text-white dark:bg-primary-500 dark:hover:bg-primary-600"
          @click="fetchReport"
        >
          Generar
        </button>
    ...
    <div class="flex flex-wrap gap-2 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
      ...
      <button
        class="flex items-center gap-1 px-3 py-1.5 text-sm rounded bg-red-100 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-800/40"
        @click="downloadPDF"
      >
        <svg
          class="w-4 h-4"
      ...
      <button
        class="flex items-center gap-1 px-3 py-1.5 text-sm rounded bg-green-100 text-green-700 hover:bg-green-200 dark:bg-green-900/30 dark:text-green-400 dark:hover:bg-green-800/40"
        @click="exportCSV"
      >
        <svg
          class="w-4 h-4"
      ...
      <button
        class="flex items-center gap-1 px-3 py-1.5 text-sm rounded bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:hover:bg-blue-800/40"
        @click="exportXls"
      >
        <svg
          class="w-4 h-4"
      ...
      >
        ...
        <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div
              class="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 p-3 rounded-lg text-center"
            >
              <p class="text-lg font-bold">{{ totalPresentes }}</p>
              <p class="text-sm">Presentes</p>
            </div>
            <div
              class="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 p-3 rounded-lg text-center"
            >
              <p class="text-lg font-bold">{{ totalAusentes }}</p>
              <p class="text-sm">Ausentes</p>
            </div>
            <div
              class="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 p-3 rounded-lg text-center"
            >
              <p class="text-lg font-bold">{{ totalTardes }}</p>
              <p class="text-sm">Tardanzas</p>
            </div>
            <div
              class="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 p-3 rounded-lg text-center"
            >
          ...
          <div class="mt-3">
            ...
            <p>
              <strong>Porcentaje promedio de asistencia:</strong> {{ averageAttendancePercentage }}%
            </p>
          </div>
        </div>
        ...
        <div v-for="(classData, index) in classReports" :key="index" class="mb-10">
          <h3 class="text-xl font-bold mb-3 text-primary-600 dark:text-primary-400">
            {{ classData.className }}
          </h3>
          ...
          <div class="overflow-x-auto">
            <table class="w-full table-auto border-collapse">
              <thead class="bg-gray-100 dark:bg-gray-700">
                <tr>
                  <th
                    class="p-2 border border-gray-300 dark:border-gray-600 text-left sticky left-0 bg-gray-100 dark:bg-gray-700 z-10"
                  >
                    Nombre
                  </th>
                  ...
                  <th
                    ...
                    :key="date"
                    class="p-2 border border-gray-300 dark:border-gray-600 text-center min-w-[60px]"
                  >
              ...
              <tbody>
                ...
                >
                  <td
                    class="p-2 border border-gray-300 dark:border-gray-600 sticky left-0 bg-white dark:bg-gray-800 z-10"
                  >
                    {{ student.name }}
                  </td>
                  ...
                  <td class="p-2 border border-gray-300 dark:border-gray-600">
                    ...
                    <template v-else>
                      ...
                      <template v-for="date in classData.relevantDates" :key="date">
                        <template
                          v-if="
                            student.attendance[date] === 'J' &&
                            getStudentJustification(classData.classId, date, student.id)
                          "
                        >
                ...
                <tr class="bg-gray-100 dark:bg-gray-700 font-medium">
                  <td
                    class="p-2 border border-gray-300 dark:border-gray-600 sticky left-0 bg-gray-100 dark:bg-gray-700 z-10"
                  >
                  ...
                  >
                    <span class="text-green-600 dark:text-green-400 font-bold">{{
                      countStatusForDate(classData, date, "P")
                    }}</span>
                    /
                    <span class="text-red-600 dark:text-red-400 font-bold">{{
                      countStatusForDate(classData, date, "A")
                    }}</span>
                    /
                    <span class="text-yellow-600 dark:text-yellow-400 font-bold">{{
                      countStatusForDate(classData, date, "T")
                    }}</span>
                    /
                    <span class="text-blue-600 dark:text-blue-400 font-bold">{{
                      countStatusForDate(classData, date, "J")
                    }}</span>
                  </td>
                </tr>
              </tbody>
          ...
          <div class="mt-4">
            ...
            >
              <div
                v-for="obs in sortedObservations(classData.observations)"
                :key="obs.date"
            ...
            <div
              v-else
              class="p-4 text-center text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 rounded-lg"
            >
              <InformationCircleIcon class="h-6 w-6 mx-auto mb-2 text-gray-400" />
              <p class="text-sm">
                No hay observaciones registradas para esta clase en el período seleccionado.
              </p>
            </div>
            ...
            <div class="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div class="flex flex-wrap justify-between">
                <div class="mr-4">
                  ...
                  <p>
                    <strong>% Asistencia:</strong> {{ calculateAttendancePercentage(classData) }}%
                  </p>
                </div>
                <div class="flex flex-wrap gap-3">
                  <div
                    class="px-2 py-1 bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 rounded"
                  >
                  ...
                  <div
                    class="px-2 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded"
                  >
...
const fetchReport = async () => {
  try {
    ...
    const attendanceDocs = await attendanceStore.fetchAttendanceByDateRange(
      start,
      end,
    ...
    classReports.value = await Promise.all(
      teacherClasses.value.map(async (classObj) => {
        ...
        const relevantDates = Array.from(
          new Set(classAttendance.map((doc) => doc.fecha || doc.date || '')),
        ).filter((date) => date);
...
function prepareChartData() {
  ...
  const avgByWeekday = attendanceByWeekday
    .map((count, dayIndex) => ({
      day: dayIndex,
...
async function downloadPDF() {
  ...
  if (!reportElement) {
    error.value =
      'No se pudo encontrar el elemento del informe. Asegúrese de que el informe está generado.';
...
function getStudentJustification(classId: string, date: string, studentId: string): string | null {
  ...
  const document = attendanceStore.attendanceDocuments.find(
    (doc) =>
      doc.classId === classId && (doc.fecha === date || doc.Fecha === date || doc.date === date),
  );
