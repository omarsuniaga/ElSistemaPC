<template>
  <div class="card bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
    ...
    <div v-else-if="absenceReport.weeklyAbsences.length === 0 && absenceReport.monthlyAbsences.length === 0" class="p-8 text-center">
    ...
    <div v-else class="p-4">
      <div class="mb-2 border-b pb-2">
        <div class="flex justify-between items-center">
          <h4 class="font-medium">Alumnos con más de 2 inasistencias por {{ activeFilter === 'week' ? 'semana' : 'mes' }}</h4>
          <div class="flex space-x-2">
            <button 
              @click="activeFilter = 'week'" 
              :class="['px-3 py-1 text-mn rounded-full', activeFilter === 'week' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200']"
            >
              Semana
            </button>
            <button 
              @click="activeFilter = 'month'" 
              :class="['px-3 py-1 text-mn rounded-full', activeFilter === 'month' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200']"
            >
              Mes
            </button>
          </div>
            ...
            >
              <td class="px-4 py-3 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-8 w-8 rounded-full bg-gray-200 overflow-hidden flex items-center justify-center">
                    <UserIcon v-if="!student.photoURL" class="h-4 w-4 text-gray-400" />
                    <img v-else :src="student.photoURL" alt="" class="h-8 w-8 rounded-full object-cover">
                  </div>
                  <div class="ml-3">
...
function notifyDirector(student: Student): void {
  ...
  alert(`Se ha notificado al director sobre las inasistencias de ${student.nombre} ${student.apellido}`);
...
function createWarning(student: Student): void {
  ...
  doc.setFontSize(11);
  doc.text('El estudiante ha superado el límite de inasistencias permitidas, lo que afecta', 20, 100);
