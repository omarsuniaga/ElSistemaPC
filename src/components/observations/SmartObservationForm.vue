...
• :: Obras revisadas"
          ...
          <div
            ...
            :style="{ minHeight: textareaHeight, fontSize: '14px', lineHeight: '1.5', fontFamily: 'inherit' }"
          ...
          <div v-if="currentAutocompleteType === 'student' && filteredStudents.length > 0">
            ...
            >
              <div class="flex items-center space-x-2">
                ...
                <div>
                  <div class="font-medium text-gray-900">{{ student.nombre }} {{ student.apellido }}</div>
    ...
    <!-- Footer con acciones -->
    <div class="px-4 py-3 bg-gray-50 border-t border-gray-200 rounded-b-lg flex justify-between items-center">
...
const highlightedText = computed(() => {
  ...
  highlighted = highlighted.replace(/@(\w+)/g, '<span class="bg-blue-200 text-blue-800 px-1 rounded">@$1</span>');
  
  // Destacar hashtags (#)
  highlighted = highlighted.replace(/#(\w+)/g, '<span class="bg-green-200 text-green-800 px-1 rounded">#$1</span>');
  
  // Destacar viñetas (::)
  highlighted = highlighted.replace(/^:: (.+)$/gm, '<span class="bg-yellow-200 text-yellow-800 px-1 rounded">• $1</span>');
...
const handleKeyDown = (event: KeyboardEvent): void => {
  ...
  // Manejar viñetas con ::
  if (event.key === 'Enter' && target.value.slice(target.selectionStart - 2, target.selectionStart) === '::') {
