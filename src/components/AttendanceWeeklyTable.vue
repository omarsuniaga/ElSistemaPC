<template>
  <div class="attendance-panel bg-white dark:bg-gray-900 rounded-xl shadow-lg p-4 md:p-6 max-w-full w-full mx-auto transition-colors duration-300">
    <!-- Header -->
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
      ...
      <div class="flex flex-wrap gap-2 items-center">
        <button @click="showDateRangeModal = true" class="flex items-center gap-1 px-2 py-1 text-xs rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
        ...
        <div class="group relative">
          <button class="flex items-center gap-1 px-2 py-1 text-xs rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <!-- Dropdown menu -->
          <div class="absolute right-0 mt-1 w-56 rounded-md shadow-lg bg-white dark:bg-gray-800 z-10 hidden group-hover:block">
            <div class="py-1" role="menu" aria-orientation="vertical">
              <div class="px-3 py-2">
                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300">Clase</label>
                <select v-model="selectedClass" @change="handleClassChange" class="mt-1 w-full px-2 py-1 rounded border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-xs text-gray-700 dark:text-gray-200">
                  <option value="">Todas las clases</option>
                  <option v-for="classItem in classes" :key="classItem.id" :value="classItem.id">{{ classItem.name }}</option>
                </select>
              </div>
              <div class="px-3 py-2">
                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300">Estado</label>
                <div class="mt-1 space-y-1">
                  <div v-for="(label, status) in statusLabels" :key="status" class="flex items-center">
                    <input :id="`status-${status}`" type="checkbox" v-model="selectedStatuses" :value="status" class="w-3 h-3 text-blue-600 rounded">
                    <label :for="`status-${status}`" class="ml-2 text-xs text-gray-700 dark:text-gray-300">{{ label }}</label>
                  </div>
                </div>
              </div>
              <div class="px-3 py-2 border-t border-gray-200 dark:border-gray-700">
                <button @click="applyFilters" class="w-full text-center py-1 text-xs text-white bg-blue-600 hover:bg-blue-700 rounded">
                  Aplicar filtros
                </button>
              </div>
            </div>
        ...
        <div class="relative">
          <input type="text" v-model="searchQuery" placeholder="Buscar estudiante..." class="pl-8 pr-2 py-1 text-xs rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200">
        ...
        <select v-model="viewMode" @change="changeViewMode" class="px-2 py-1 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-gray-700 dark:text-gray-200">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-5">
      <div class="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
        ...
        <div class="mt-1 flex items-baseline justify-between">
          <span class="text-xl font-bold text-green-600 dark:text-green-400">{{ attendanceStats.present }}%</span>
          <span class="text-xs text-green-600 dark:text-green-400">{{ attendanceStats.presentCount }} estudiantes</span>
        </div>
        <div class="mt-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
          <div class="bg-green-500 h-1.5 rounded-full" :style="`width: ${attendanceStats.present}%`"></div>
        </div>
      </div>
      ...
      <div class="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
        ...
        <div class="mt-1 flex items-baseline justify-between">
          <span class="text-xl font-bold text-red-600 dark:text-red-400">{{ attendanceStats.absent }}%</span>
          <span class="text-xs text-red-600 dark:text-red-400">{{ attendanceStats.absentCount }} estudiantes</span>
        </div>
        <div class="mt-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
          <div class="bg-red-500 h-1.5 rounded-full" :style="`width: ${attendanceStats.absent}%`"></div>
        </div>
      </div>
      ...
      <div class="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
        ...
        <div class="mt-1 flex items-baseline justify-between">
          <span class="text-xl font-bold text-purple-600 dark:text-purple-400">{{ attendanceStats.late }}%</span>
          <span class="text-xs text-purple-600 dark:text-purple-400">{{ attendanceStats.lateCount }} estudiantes</span>
        </div>
        <div class="mt-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
          <div class="bg-purple-500 h-1.5 rounded-full" :style="`width: ${attendanceStats.late}%`"></div>
        </div>
      </div>
      ...
      <div class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        ...
        <div class="mt-1 flex items-baseline justify-between">
          <span class="text-xl font-bold text-blue-600 dark:text-blue-400">{{ attendanceStats.justified }}%</span>
          <span class="text-xs text-blue-600 dark:text-blue-400">{{ attendanceStats.justifiedCount }} estudiantes</span>
        </div>
        <div class="mt-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
          <div class="bg-blue-500 h-1.5 rounded-full" :style="`width: ${attendanceStats.justified}%`"></div>
        </div>
      </div>
      ...
      <div class="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
        ...
        <div class="mt-1 flex items-baseline justify-between">
          <span class="text-xl font-bold text-gray-600 dark:text-gray-400">{{ studentsWithoutClassesStats.percentage }}%</span>
          <span class="text-xs text-gray-600 dark:text-gray-400">{{ studentsWithoutClassesStats.count }} estudiantes</span>
        </div>
        <div class="mt-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
          <div class="bg-gray-500 h-1.5 rounded-full" :style="`width: ${studentsWithoutClassesStats.percentage}%`"></div>
        </div>
      </div>
    </div>

    <!-- Date navigation -->
    <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2 mb-3">
      <button @click="navigatePrevious" class="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors" title="Anterior">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600 dark:text-gray-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
      </button>
      
      <div class="flex items-center gap-2 text-gray-700 dark:text-gray-300">
        <button @click="goToToday" class="text-xs px-2 py-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
          Hoy
        </button>
      ...
      <button @click="navigateNext" class="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors" title="Siguiente">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600 dark:text-gray-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" /></svg>
      </button>
    </div>
    ...
    <div v-else-if="error" class="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-lg p-3 text-sm flex items-center gap-2">
    ...
    <div v-else-if="filteredStudents.length === 0" class="flex flex-col items-center justify-center py-10 text-center text-gray-500 dark:text-gray-400">
      ...
      <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
        <thead class="bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400">
          <tr>
            ...
            <th @click="sortBy('name')" class="sticky left-0 z-10 bg-gray-50 dark:bg-gray-800 p-3 text-left text-xs font-medium uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700">
              <div class="flex items-center">
                <span>Estudiante</span>
                <svg v-if="sortColumn === 'name'" xmlns="http://www.w3.org/2000/svg" class="ml-1 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path v-if="sortDirection === 'asc'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                  <path v-else stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
              </div>
            </th>
            <th @click="sortBy('attendance')" class="p-3 text-center text-xs font-medium uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700">
              <div class="flex items-center justify-center">
                <span>Asistencia</span>
                <svg v-if="sortColumn === 'attendance'" xmlns="http://www.w3.org/2000/svg" class="ml-1 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path v-if="sortDirection === 'asc'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                  <path v-else stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
              </div>
            </th>
            <th v-for="day in visibleDays" :key="day.getTime()" class="p-3 text-center text-xs font-medium whitespace-nowrap">
        ...
        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
          <tr v-for="student in sortedStudents" :key="student.id" class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            <td class="sticky left-0 z-10 bg-white dark:bg-gray-900 p-3 whitespace-nowrap hover:bg-gray-50 dark:hover:bg-gray-800">
              <div class="flex items-center">
                ...
                <div class="ml-3">
                  ...
                  <div class="text-xs text-gray-500 dark:text-gray-400">{{ student.classInfo || 'Sin clase' }}</div>
                </div>
              </div>
            </td>
            ...
            <td v-for="day in visibleDays" :key="day.getTime()" class="p-3 text-center">
              <div :class="getAttendanceClass(student.id, day) + ' inline-flex items-center justify-center h-7 w-7 rounded-full font-semibold'"
    ...
    <div v-if="totalPages > 1" class="mt-4 flex items-center justify-between">
      <div class="text-xs text-gray-500 dark:text-gray-400">
        Mostrando {{ (currentPage - 1) * pageSize + 1 }} a {{ Math.min(currentPage * pageSize, filteredStudents.length) }} de {{ filteredStudents.length }} estudiantes
      </div>
      <div class="flex items-center space-x-1">
        <button @click="changePage(1)" :disabled="currentPage === 1" class="px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 disabled:opacity-50 text-xs">
          &laquo;
        </button>
        <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1" class="px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 disabled:opacity-50 text-xs">
          &lsaquo;
        </button>
        
        <div v-for="page in paginationRange" :key="page" class="hidden sm:block">
          <button v-if="page !== '...'" @click="changePage(page)" 
                  :class="[currentPage === page ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300']"
        ...
        <button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages" class="px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 disabled:opacity-50 text-xs">
          &rsaquo;
        </button>
        <button @click="changePage(totalPages)" :disabled="currentPage === totalPages" class="px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 disabled:opacity-50 text-xs">
          &raquo;
        </button>
      </div>
    </div>
    ...
    <!-- Date Range Modal -->
    <div v-if="showDateRangeModal" class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true" role="dialog">
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-black bg-opacity-50 transition-opacity" aria-hidden="true" @click="showDateRangeModal = false"></div>

        <!-- Modal panel -->
        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-white mb-4">Seleccionar rango de fechas</h3>
            
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fecha de inicio</label>
                <input type="date" v-model="customDateRange.start" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fecha de fin</label>
                <input type="date" v-model="customDateRange.end" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
              </div>
            </div>
            
            <div class="mt-4">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Presets</label>
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <button @click="setPresetRange('week')" class="px-2 py-1 text-xs border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">Esta semana</button>
                <button @click="setPresetRange('month')" class="px-2 py-1 text-xs border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">Este mes</button>
                <button @click="setPresetRange('lastWeek')" class="px-2 py-1 text-xs border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">Semana pasada</button>
                <button @click="setPresetRange('lastMonth')" class="px-2 py-1 text-xs border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">Mes pasado</button>
              </div>
            </div>
          </div>
          
          <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 flex flex-col sm:flex-row-reverse gap-2">
            <button type="button" @click="applyCustomDateRange" class="w-full sm:w-auto px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 focus:outline-none">
              Aplicar
            </button>
            <button type="button" @click="showDateRangeModal = false" class="w-full sm:w-auto px-4 py-2 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-md hover:bg-gray-300 dark:hover:bg-gray-500 focus:outline-none">
              Cancelar
            </button>
          </div>
        </div>
...
import { 
  ...
  parseISO, isValid, subDays, isWithinInterval
} from 'date-fns';
...
const searchFilteredStudents = computed(() => {
  ...
  return filteredStudents.value.filter(student => 
    ...
    student.classInfo?.toLowerCase().includes(query)
  );
});
...
function getAttendanceTooltip(studentId: string, date: Date): string {
  ...
  switch (status) {
    case 'P': return `Presente el ${dateFormatted}`;
    case 'A': return `Ausente el ${dateFormatted}`;
    case 'T': return `Tarde el ${dateFormatted}`;
    case 'J': return `Justificado el ${dateFormatted}`;
    case '-': return `Sin registro el ${dateFormatted}`;
    default: return `Sin clase el ${dateFormatted}`;
  }
}
...
const attendanceStats = computed(() => {
  ...
  attendance.value.forEach(record => {
    ...
    const isDateInRange = visibleDays.value.some(day => 
      format(day, 'yyyy-MM-dd') === record.date
    );
