...
const { user } = useAuth();
const { hasPermission, hasModuleAccess, canAccessComponent, userRoles, loading: rbacLoading } = useRBAC();
...
// Reactividad
watch([() => user.value, () => props.permissions, () => props.roles, () => props.moduleId, () => props.componentId], () => {
  checkAccess();
});
