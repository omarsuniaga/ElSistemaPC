<template>
  <div v-if="isVisible" class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div
      v-if="showTagModal && showTagModal"
      class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 rounded-lg shadow-xl z-60 p-4 max-w-md w-full max-h-96 overflow-y-auto"
    >
    ...
    <!-- Contenido del modal -->
    <div
      class="relative bg-white dark:bg-gray-800 reounded-lg shadow-xl max-w-lg w-full max-h-[80vh] flex flex-col"
    >
      ...
      <div class="mb-4">
        ...
        <div v-if="isLoading" class="flex justify-center items-center py-4">
          <div
            class="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-700 dark:border-blue-500"
          />
        </div>
        ...
        <div
          v-else-if="error"
          class="bg-red-100 dark:bg-red-900/30 border border-red-400 dark:border-red-800 text-red-700 dark:text-red-400 px-4 py-3 rounded"
        >
          <p>{{ error }}</p>
        </div>
        ...
        <div
          v-else-if="
            classObservations.length === 0 && justifications.length === 0 && !legacyObservations
          "
          class="py-4"
        >
          <p class="text-gray-500 dark:text-gray-400 text-center">
            No hay observaciones para esta clase
          </p>
          <div class="flex justify-center mt-4">
            <button
              class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded flex items-center"
              @click="showAddObservationForm"
            >
        ...
        <div
          v-if="isFormVisible"
          class="border border-gray-300 dark:border-gray-700 p-4 rounded-lg mb-6 dark:bg-gray-750"
        >
          <h4 class="font-medium mb-3 text-gray-800 dark:text-gray-200">
            {{ isEditMode ? "Editar observación" : "Nueva observación" }}
          </h4>
          <div class="text-xs text-gray-500 dark:text-gray-400 mb-2">
            <span class="inline-block mr-2"
              >Usar <kbd class="bg-gray-200 dark:bg-gray-700 px-1 py-0.5 rounded">#</kbd> para
              etiquetar estudiantes</span
            >
            <span class="inline-block"
              >Presiona <kbd class="bg-gray-200 dark:bg-gray-700 px-1 py-0.5 rounded">Tab</kbd> para
              aceptar sugerencias de texto</span
            >
          </div>

          <div class="mb-3">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
              >Tipo de observación</label
            >
            <select
              v-model="formData.type"
              class="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded p-2 text-gray-800 dark:text-gray-200"
            >
          ...
          <div class="mb-3">
            ...
            <div class="relative">
              ...
              <!-- Sugerencia de texto -->
              <div
                v-if="smartDictionary.suggestionActive.value"
                class="absolute right-2 top-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-1 text-xs rounded"
              >
                <span class="opacity-70">Tab para completar:</span>
                {{ smartDictionary.currentSuggestion.value }}
              </div>

              <!-- Estudiantes etiquetados -->
              <div
                v-if="taggedStudents && taggedStudents.length > 0"
                class="mt-2 flex flex-wrap gap-2"
              >
                <div
                  v-for="(student, index) in taggedStudents"
                  :key="index"
          ...
          <div class="mb-3">
            <label class="inline-flex items-center">
              <input
                v-model="formData.requiresFollowUp"
                type="checkbox"
          ...
          <div class="flex justify-end space-x-2">
            <button
              class="border border-gray-300 dark:border-gray-600 px-4 py-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300"
              @click="cancelForm"
            >
              Cancelar
            </button>
          ...
          <div v-if="classObservations.length > 0" class="space-y-4 mb-4">
            <div class="flex justify-between items-center">
              <div class="font-medium text-gray-800 dark:text-gray-200">
                Observaciones ({{ classObservations.length }})
              </div>
              <button
                class="bg-blue-500 dark:bg-blue-600 hover:bg-blue-600 dark:hover:bg-blue-700 text-white py-1 px-3 rounded text-sm flex items-center"
                @click="showAddObservationForm"
              >
            ...
            >
              <div class="flex justify-between items-start mb-2">
                <div>
                  ...
                  <span
                    ...
                    class="ml-2 bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-400 text-xs px-2 py-0.5 rounded"
                  >
                    Seguimiento
                  </span>
                </div>
              ...
              <p class="text-lg font-medium mb-1 text-gray-800 dark:text-gray-200">
                {{
              ...
              <ul
                v-if="
                  typeof observation.content === 'object' &&
                  observation.content.bulletPoints &&
                  observation.content.bulletPoints.length > 0
                "
                class="list-disc pl-5 mt-2 text-gray-700 dark:text-gray-300"
              >
              ...
              <div class="mt-3 pt-2 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="text-xs text-gray-600 dark:text-gray-400 mt-1">
                  ...
                  <div v-if="observation.lastModified" class="flex items-center mt-1">
                    ...
                    <span
                      >Modificado: {{ formatDate(observation.lastModified) }} por
                      {{
                        observation.modifiedByName || getAuthorName(observation.modifiedBy)
                      }}</span
                    >
                  </div>
                </div>
...
const loadObservations = async () => {
  ...
  try {
    ...
    if (!attendanceDoc) {
      ...
      if (
        attendanceDoc.data?.observations &&
        Array.isArray(attendanceDoc.data.observations) &&
        attendanceDoc.data.observations.length > 0
      ) {
        try {
