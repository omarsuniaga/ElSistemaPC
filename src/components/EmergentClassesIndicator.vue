<template>
  <div class="emergent-classes-indicator">
    ...
    <div v-else class="flex items-center">
      <!-- Indicador SÍ -->
      <div v-if="hasApprovedEmergentClasses" 
        ...
        <span class="font-medium">{{ emergentClassesCount }} {{ emergentClassesCount === 1 ? 'clase emergente' : 'clases emergentes' }} hoy</span>
        <button 
          @click="openModal" 
    ...
    <div v-if="showModal" 
         ...
         aria-modal="true">
      <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg m-4 max-h-[80vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
          <h3 id="modal-title" class="text-lg font-medium text-gray-900">Clases Emergentes Aprobadas Hoy</h3>
          <button 
            @click="closeModal" 
        ...
        <div v-if="emergentClasses.length > 0" class="space-y-4">
          <div v-for="(clase, index) in emergentClasses" :key="index" class="border-b pb-4 last:border-b-0">
        ...
        <div class="mt-6 flex justify-end space-x-3">
          ...
          >
            <svg v-if="isNotifying" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white inline-block" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
              <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
...
const getEstadoClase = (clase) => {
  ...
  // Para clases en curso
    ...
    return { 
      estado: 'En proceso', 
      color: 'green',
      progreso: Math.floor(((ahora.getTime() - horaInicio.getTime()) / (horaFin.getTime() - horaInicio.getTime())) * 100)
    };
  } 
