    ...
    <template v-for="toast in toasts" :key="toast.id">
      ...
      >
        <div class="flex-1">
          ...
          <p v-if="toast.description" class="text-sm text-gray-600 dark:text-gray-400">{{ toast.description }}</p>
        </div>
        <button @click="dismiss(toast.id)" class="text-gray-500 hover:text-gray-700">
