  ...
  >
    ...
    <div v-if="isLoading" class="flex justify-center items-center p-6">
      <div class="animate-pulse flex flex-col items-center">
        <div class="rounded-full bg-gray-200 dark:bg-gray-700 h-10 w-10 mb-2" />
    ...
    <div v-else>
      ...
      >
        <li
          ...
          :key="classItem.id"
          class="p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2"
        >
          <div>
          ...
          <div class="flex flex-col items-end">
            ...
            <span
              v-if="classItem.status === 'next'"
              class="text-xs text-indigo-600 dark:text-indigo-300 mt-1"
            >
              Próxima clase
            </span>
          </div>
...
watch(
  ...
  () => {
    ...
    const mapped = allClasses.map((classItem) => {
      ...
      else if (isWithinInterval(now.value, { start: startDateTime, end: endDateTime }))
        status = 'active';
      ...
      const attendanceDoc = attendanceDocs.find(
        (doc) => doc.classId === classItem.id && doc.fecha === formattedDate,
      );
    ...
    mapped.sort((a, b) => {
      ...
      if (order[a.status as StatusType] !== order[b.status as StatusType])
        return order[a.status as StatusType] - order[b.status as StatusType];
