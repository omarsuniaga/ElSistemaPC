  ...
  if (props.maxSize) {
    ...
    for (let i = 0; i < files.length; i++) {
      if (files[i].size > maxSizeBytes) {
        emit('error', `El archivo ${files[i].name} excede el tamaño máximo permitido (${props.maxSize}MB)`)
        return
      }
    }
  }
        ...
        <template v-else>
          ...
          <p v-else class="text-sm text-gray-600 dark:text-gray-400 text-center">
            <span class="font-medium text-blue-600 dark:text-blue-400">
              Haz clic o arrastra
            </span> para subir archivo
            <span v-if="maxSize" class="block text-xs mt-1">
              (Máximo {{ maxSize }}MB)
            </span>
          </p>
        </template>
