...
export async function rbacGuard(
  ...
  try {
    ...
    console.log(`👤 RBAC Guard: Verificando acceso para usuario con rol '${userRole}' a ruta '${to.path}'`);      // Verificación especial para maestros en rutas de teacher
    ...
    if (to.meta.allowedRoles && Array.isArray(to.meta.allowedRoles)) {
      ...
      console.log(`🔐 RBAC Guard: Roles permitidos: [${to.meta.allowedRoles.join(', ')}], Usuario tiene: ${userRole || 'N/A'}, Acceso: ${hasAccess}`);
    ...
    if (to.meta.moduleKey && to.meta.permission) {
      ...
      const hasAccess = checkRolePermissionAccess(userRole, to.meta.moduleKey as string, to.meta.permission as string, to);
    ...
    console.warn(`❌ RBAC Guard: Acceso denegado para usuario ${user.uid} con rol ${userRole} a la ruta ${to.path}`);
 ...
 */
function checkRolePermissionAccess(role: string | undefined, moduleKey: string, permission: string, to?: RouteLocationNormalized): boolean {
