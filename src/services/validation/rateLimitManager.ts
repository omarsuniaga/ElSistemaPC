...
class RateLimitManager {
  ...
  recordAttempt(phoneNumber: string, success: boolean, messageType: string): void {
    ...
    console.log(`📊 Rate Limit - Intento registrado: ${phoneNumber}, éxito: ${success}, tipo: ${messageType}`)
  }
  ...
  calculateBatchDelay(batchSize: number): number {
    ...
    const remainingPerMinute = rateLimitStatus.currentLimits.perMinute.max - rateLimitStatus.currentLimits.perMinute.current
...
export const sendBatchWithRateLimit = async (
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (result.rateLimitInfo && !result.rateLimitInfo.canSend && result.rateLimitInfo.nextAvailableTime) {
  ...
  console.log(`✅ Rate Limit - Lote completado: ${successful} éxitos, ${failed} fallos, ${rateLimited} rate limited`)
