...
class CacheService {
  ...
  async setItem(storeName: 'students' | 'teachers' | 'classes' | 'attendance' | 'contents', key: string, value: any) {
    const db = await this.initDB()
    await db.put(storeName, value, key)
  }

  async getItem(storeName: 'students' | 'teachers' | 'classes' | 'attendance' | 'contents', key: string) {
    const db = await this.initDB()
  ...
  async removeItem(storeName: 'students' | 'teachers' | 'classes' | 'attendance' | 'contents', key: string) {
    const db = await this.initDB()
  ...
  async syncWithFirebase(storeName: 'students' | 'teachers' | 'classes' | 'attendance' | 'contents', data: any[]) {
    const db = await this.initDB()
  ...
  async hasData(storeName: 'students' | 'teachers' | 'classes' | 'attendance' | 'contents'): Promise<boolean> {
    const db = await this.initDB()
