...
export class AnalyticsEngine {
  ...
  private detectAbsencePatterns(): void {
    ...
    Object.entries(studentAbsences).forEach(([studentId, records]) => {
      ...
      if (absenceRate > 0.3) { // Más del 30% de ausencias
  ...
  private detectLatenessPatterns(): void {
    ...
    Object.entries(studentData).forEach(([studentId, records]) => {
      const attendedClasses = records.filter(r => r.status === "presente" || r.status === "tardanza")
      const lateArrivals = records.filter(r => r.status === "tardanza")
      const latenessRate = lateArrivals.length / attendedClasses.length
      
      if (latenessRate > 0.2 && attendedClasses.length > 5) { // Más del 20% llega tarde
    ...
    Object.entries(classData).forEach(([className, records]) => {
      ...
      if (absenceRate > 0.25 || latenessRate > 0.3) {
        this.patterns.push({
          ...
          recommendations: this.generateClassRecommendations(absenceRate, latenessRate, totalStudents),
          confidence: 0.8
        })
      }
    })
  ...
  private analyzeDayPattern(records: AnalyticsData[]): Record<string, number> {
    const dayCount = records.reduce((acc, record) => {
  ...
  private analyzeDayOfWeekPatterns(): any[] {
    const dayData = this.data.reduce((acc, record) => {
      const day = format(record.date, "EEEE", { locale: es })
      if (!acc[day]) {
  ...
  private calculateTrendDirection(data: number[]): { direction: "improving" | "stable" | "declining"; projectedValue: number; confidence: number } {
  ...
  private simpleLinearRegression(data: number[]): { slope: number; intercept: number; rSquared: number; predict: (x: number) => number } {
    const n = data.length
  ...
  private calculateRecentTrend(records: AnalyticsData[]): "improving" | "stable" | "declining" {
    ...
    const attendanceData = records.map(r => r.status === "presente" || r.status === "tardanza" ? 1 : 0)
   ...
   */
  private generateAbsenceRecommendations(absenceRate: number, consecutiveAbsences: number): string[] {
  ...
  private generateLatenessRecommendations(dayPattern: Record<string, number>): string[] {
    ...
    recommendations.push(`Revisar horarios de transporte, especialmente para los ${mostProblematicDay[0]}`)
  ...
  private generateClassRecommendations(absenceRate: number, latenessRate: number, totalStudents: number): string[] {
    const recommendations = []
  ...
  generateBenchmarks(
    ...
    benchmarks.push({
      ...
      improvement: ((currentAttendanceRate - benchmarkAttendanceRate) / benchmarkAttendanceRate) * 100
    })
