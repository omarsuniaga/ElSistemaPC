...
export class ExcelExporter {
   ...
   */
  async exportRiskStudentsReport(
    riskStudents: any[],
    ...
    const formattedData = riskStudents.map(student => ({
      ...
      'Última Asistencia': student.lastAttendance ? format(new Date(student.lastAttendance), 'dd/MM/yyyy') : 'N/A',
  ...
  private addTrendsSheet(data: any[]): void {
    ...
    const formattedData = Object.entries(dailyStats).map(([date, records]) => {
      const total = records.length
      const present = records.filter((r: any) => r.status === 'presente').length
      const absent = records.filter((r: any) => r.status === 'ausente').length
      const late = records.filter((r: any) => r.status === 'tardanza').length
  ...
  private addDashboardSheet(attendanceData: any[], statisticsData: any[]): void {
    ...
    const dashboardData = [
      ...
      { 'Métrica': 'Tasa de Asistencia Global (%)', 'Valor': Math.round(((presentCount + lateCount) / totalRecords) * 10000) / 100, 'Tipo': 'Asistencia' },
      { 'Métrica': 'Tasa de Puntualidad Global (%)', 'Valor': Math.round((presentCount / (presentCount + lateCount || 1)) * 10000) / 100, 'Tipo': 'Puntualidad' },
  ...
  private addRiskSummarySheet(riskStudents: any[]): void {
    const summary = {
      ...
      'Promedio Asistencia (%)': Math.round(riskStudents.reduce((acc, s) => acc + s.attendanceRate, 0) / riskStudents.length * 100) / 100,
