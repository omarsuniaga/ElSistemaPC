...
export class ReportGenerator {
  ...
  async generateRiskStudentsReport(
    ...
    const riskStudents = students.filter(s => 
      s.riskLevel === 'alto' || s.riskLevel === 'crítico'
    ).sort((a, b) => {
  ...
  private async addHeader(metadata: ReportData['metadata'], title: string): Promise<void> {
    ...
    this.doc.text(metadata.academyName, metadata.academyLogo ? this.margin + 40 : this.margin, this.currentY + 35)
    
    // Fecha de generación
    const dateStr = format(metadata.generatedAt, "dd 'de' MMMM 'de' yyyy 'a las' HH:mm", { locale: es })
    this.doc.setFontSize(10)
    this.doc.text(`Generado el ${dateStr} por ${metadata.generatedBy}`, 
                  this.pageWidth - this.margin - 100, this.currentY + 15)
  ...
  private addExecutiveSummary(data: AttendanceReportData[]): void {
    ...
    this.addSummaryBox([
      ...
      { label: 'Tasa de Asistencia', value: `${attendanceRate.toFixed(1)}%`, color: attendanceRate >= 90 ? '#059669' : attendanceRate >= 80 ? '#d97706' : this.accentColor },
      { label: 'Tasa de Puntualidad', value: `${punctualityRate.toFixed(1)}%`, color: punctualityRate >= 95 ? '#059669' : punctualityRate >= 85 ? '#d97706' : this.accentColor },
      { label: 'Ausencias Sin Justificar', value: absentCount.toString(), color: absentCount === 0 ? '#059669' : this.accentColor }
    ])
  }
  ...
  private addDetailedData(data: AttendanceReportData[], groupBy: string): void {
    ...
    switch (groupBy) {
      case 'student':
        headers = ['Estudiante', 'Clase', 'Profesor', 'Fecha', 'Estado', 'Hora Llegada', 'Notificaciones']
      ...
      case 'class':
        ...
        headers = ['Clase', 'Profesor', 'Total Estudiantes', 'Presentes', 'Ausentes', 'Tardanzas', '% Asistencia']
  ...
  private addAnalysisAndRecommendations(data: AttendanceReportData[]): void {
    ...
    patterns.forEach(pattern => {
      ...
      this.doc.setFont('helvetica', 'normal')
      this.doc.setFontSize(10)
      const lines = this.doc.splitTextToSize(pattern.description, this.pageWidth - this.margin * 2 - 10)
  ...
  private addRiskSummary(students: StudentStatistics[]): void {
    ...
    this.addSummaryBox([
      { label: 'Estudiantes en Riesgo Crítico', value: criticalCount.toString(), color: this.accentColor },
      ...
      { label: 'Requiere Acción Inmediata', value: criticalCount.toString(), color: this.accentColor }
    ])
  }
  ...
  private identifyPatterns(data: AttendanceReportData[]): Array<{ title: string; description: string }> {
    const patterns = []
  ...
  private async addTrendCharts(data: AttendanceReportData[]): Promise<void> {
    ...
    this.doc.setFontSize(11)
    this.doc.setTextColor(this.secondaryColor)
    this.doc.text('[Los gráficos de tendencias se implementarán en la siguiente versión]', this.margin, this.currentY)
    this.currentY += 20
  }
}
