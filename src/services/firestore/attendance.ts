...
const validateJustificationData = (
  justification: Partial<JustificationData>,
): JustificationData => {
  ...
  try {
    ...
    if (file) {
      const storageRef = ref(
        storage,
        `justifications/${fecha}_${validatedJustification.studentId}_${classId}_${file.name}`,
      );
    ...
    if (docSnap.exists()) {
      ...
      const justIndex = data.data.justificacion.findIndex(
        (j) => j.studentId === validatedJustification.studentId,
      );

      if (justIndex !== -1) {
        // Actualizar la justificación existente
        data.data.justificacion[justIndex] = {
          ...data.data.justificacion[justIndex],
          ...validatedJustification,
        };
      } else {
      ...
      const newDoc: Omit<
        AttendanceDocument,
        'id' | 'teacherId' | 'uid' | 'createdAt' | 'updatedAt'
      > = {
        fecha,
        classId,
        data: {
  ...
  try {
    ...
    if (attendanceData.status === 'Justificado' && attendanceData.justification) {
      const justIndex = document.data.justificacion.findIndex(
        (j) => j.studentId === attendanceData.studentId,
      );

      const justificationReason =
        typeof attendanceData.justification === 'string'
...
export const getObservationsHistoryFirebase = async (
  ...
  try {
    ...
    // Caso 2: Filtrar solo por clase
      ...
      q = query(collection(db, 'OBSERVACIONES'), where('classId', '==', classId));
    }
    // Caso 3: Filtrar solo por fecha
    else if (date) {
      q = query(collection(db, 'OBSERVACIONES'), where('date', '==', date));
    }
    // Caso 4: Obtener todas las observaciones
    else {
      q = query(collection(db, 'OBSERVACIONES'));
    }
