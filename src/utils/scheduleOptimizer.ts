...
export function optimizeSchedule(classes: any[]): OptimizationResult {
  ...
  const classesByDay = classes.reduce(
    (acc, class_) => {
      ...
      const schedule =
        typeof class_.schedule === 'string' ? parseStringSchedule(class_.schedule) : class_.schedule;
  ...
  Object.entries(classesByDay).forEach(([day, dayClasses]) => {
    ...
    dayClasses.sort((a, b) => a.parsedSchedule.startTime.localeCompare(b.parsedSchedule.startTime));
    ...
    for (let i = 0; i < dayClasses.length; i++) {
      ...
      const teacherClasses = dayClasses.filter((c) => c.teacherId === currentClass.teacherId);
      ...
      const classroomClasses = dayClasses.filter((c) => c.classroom === currentClass.classroom);
...
function findNextAvailableSlot(
  ...
  while (proposedStart.getHours() < 21) {
    // Límite a las 21:00
