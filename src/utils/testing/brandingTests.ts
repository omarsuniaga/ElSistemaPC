  ...
  private async testStoreInitialization(): Promise<void> {
    ...
    try {
      ...
      if (this.brandingStore.appTitle) {
        this.addResult('Computed properties funcionando', 'PASS', `Title: ${this.brandingStore.appTitle}`)
      } else {
  ...
  private testComposableProperties(): void {
    ...
    try {
      ...
      if (appDescription.value) {
        this.addResult('Composable appDescription', 'PASS', `Length: ${appDescription.value.length} chars`)
      } else {
  ...
  private testCSSVariables(): void {
    ...
    try {
      ...
      if (Object.keys(storeVariables).length > 0) {
        this.addResult('CSS Variables (Store)', 'PASS', `${Object.keys(storeVariables).length} variables`)
      } else {
      ...
      if (Object.keys(composableVariables).length > 0) {
        this.addResult('CSS Variables (Composable)', 'PASS', `${Object.keys(composableVariables).length} variables`)
      } else {
      ...
      if (dynamicVariables['--brand-primary']) {
        this.addResult('Dynamic CSS Variables', 'PASS', `Primary: ${dynamicVariables['--brand-primary']}`)
      } else {
  ...
  private testDOMApplication(): void {
    ...
    try {
      ...
      if (metaDescription && metaDescription.content) {
        this.addResult('Meta description', 'PASS', `Content: ${metaDescription.content.substring(0, 50)}...`)
      } else {
