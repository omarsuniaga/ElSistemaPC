...
class PerformanceTestSuite {
  ...
  async runFullSuite(): Promise<PerformanceBenchmark> {
    ...
    logger.info('PERFORMANCE_TEST', `✅ Suite completada en ${totalDuration.toFixed(2)}ms`, benchmark.summary)
    return benchmark
  }
  ...
  private async testComponentLoadTimes(): Promise<void> {
    ...
    try {
      ...
      const avgLoadTime = componentTests.reduce((sum, t) => sum + t.loadTime, 0) / componentTests.length
