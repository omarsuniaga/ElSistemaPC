...
function checkIfStudentHasInstruments(classIds: string[], classesStore: any): boolean {
  ...
  for (const className of classNames) {
    ...
    if (
      instrumentClasses.some(
        (instrClass) =>
          className.includes(instrClass) || className.toLowerCase() === instrClass.toLowerCase(),
      )
    ) {
      return true;
    }
  ...
  const onlyNoInstrumentClasses = classNames.every((className) =>
    noInstrumentClasses.some(
      (noInstrClass) =>
        className.includes(noInstrClass) || className.toLowerCase() === noInstrClass.toLowerCase(),
    ),
  );
