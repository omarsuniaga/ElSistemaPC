...
export const useWhatsAppLogsStore = defineStore('whatsappLogs', () => {
  ...
  async function fetchLogsByStudentIdAndDateRange(studentId: string, dateFrom?: string, dateTo?: string) {
    ...
    try {
      ...
      let result: WhatsAppLog[] = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as WhatsAppLog));
      // Filtro por rango de fechas si aplica
      if (dateFrom || dateTo) {
        result = result.filter(log => {
          const sentAt = log.sentAt?.seconds ? new Date(log.sentAt.seconds * 1000) : new Date(log.sentAt);
