...
import type { Repertoire, MusicalWork, PracticeNote, PracticeReminder, Measure, InstrumentSection } from '../types/repertoire'
        ...
        for (const repertoire of state.repertoires) {
          ...
          if (work) {
            ...
            if (instrument) {
              ...
              return instrument.measures.reduce((sum, m) => sum + m.progress, 0) / instrument.measures.length
            }
          }
        }
        return 0
            ...
            if (sectionInstruments.length > 0) {
              return Math.round(
                sectionInstruments.reduce((sum, inst) => {
                  return sum + inst.measures.reduce((mSum, m) => mSum + m.progress, 0) / inst.measures.length
