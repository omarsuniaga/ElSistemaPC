    ...
    for (const classId of sampleClasses) {
      ...
      if (Math.random() < 0.7) {
        ...
        try {
          ...
          await fetch(
            `https://firestore.googleapis.com/v1/projects/YOUR_PROJECT_ID/databases/(default)/documents/ASISTENCIAS/${docId}`,
            {
              method: 'PATCH',
              headers: {
          ...
          documentsCreated++;
          console.log(
            `✅ Documento creado: ${docId} - ${presentes.length}P, ${ausentes.length}A, ${tarde.length}T`,
          );
        } catch (error) {
