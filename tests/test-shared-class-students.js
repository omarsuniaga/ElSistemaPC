...
async function testSharedClassStudents(classId) {
  ...
  try {
    ...
    if (currentStudents.length !== firestoreStudents.length) {
      ...
      console.log(`   Diferencia: ${firestoreStudents.length - currentStudents.length} estudiantes perdidos`);
    } else {
      console.log(`✅ Ambos métodos devuelven la misma cantidad de estudiantes`);
    }
    
    // 4. Mostrar detalles de estudiantes
    console.log(`4️⃣ Detalles de estudiantes (Firestore):`, firestoreStudents.map(s => ({
      id: s.id,
      nombre: s.nombre,
      apellido: s.apellido
    })));
...
async function testAssistantTeacherAccess(teacherId, classId) {
  ...
  try {
    ...
    const isAssistant = classData.teachers?.some(t => 
      t.teacherId === teacherId && (t.role === 'assistant' || t.role === 'asistente')
    );
