...
class ObservationsTestManager {
  ...
  async generateStats(observations) {
    ...
    stats.avgTextLength =
      observations.length > 0 ? Math.round(totalTextLength / observations.length) : 0;
    stats.followUpRate =
      observations.length > 0 ? Math.round((followUpCount / observations.length) * 100) : 0;
...
async function runCompleteTest() {
  ...
  try {
    ...
    console.log(
      `✅ Filtro por fecha funcionando: ${dateRangeObs.length} observaciones (último mes)`,
    );
