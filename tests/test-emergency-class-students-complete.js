...
async function testEmergencyClassStudentLoading() {
  ...
  try {
    ...
    const classNameElements = document.querySelectorAll('h1, h2, h3, .class-name, [class*="titulo"]');
    ...
    const counterElements = document.querySelectorAll('[class*="counter"], [class*="count"], .badge');
    ...
    const actionButtons = document.querySelectorAll('button[class*="presente"], button[class*="ausente"], button[class*="tardanza"]');
      ...
      try {
        ...
        if (doc.exists) {
          ...
          if (data.selectedStudents && data.selectedStudents.length > 0) {
            ...
            const studentsSnapshot = await studentsRef.where(window.firebase.firestore.FieldPath.documentId(), 'in', data.selectedStudents.slice(0, 10)).get();
            
            console.log(`📋 Found ${studentsSnapshot.size} student documents in ALUMNOS collection`);
            studentsSnapshot.forEach(studentDoc => {
              const studentData = studentDoc.data();
              console.log(`  - ${studentDoc.id}: ${studentData.nombre || 'No name'} ${studentData.apellido || ''}`);
            });
          }
        }
      } catch (firebaseError) {
...
window.emergencyClassStudentTests = {
  ...
  emergencyClassId: emergencyClassId
};
