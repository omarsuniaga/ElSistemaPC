...
async function testUpdatedEmergencyClassIntegration() {
  ...
  try {
    ...
    if (currentUrl.includes(emergencyClassId)) {
      ...
      const errorElements = document.querySelectorAll('[class*="error"], [class*="red"], .bg-red-100');
      ...
      const loadingElements = document.querySelectorAll('[class*="loading"], [class*="spinner"], [class*="cargando"]');
