        ...
        if (Array.isArray(observaciones) && observaciones.length > 0) {
          observaciones.forEach((obs, index) => {
            try {
              ...
              console.error(`   ❌ Error procesando observación ${index} de ${docSnapshot.id}:`, error.message);
              this.stats.errors++;
            }
          });
        }
  ...
  async migrateToFirestore() {
    ...
    while (processed < this.allObservations.length) {
      ...
      try {
        ...
        console.error(`   ❌ Error en batch ${processed}-${processed + batchItems.length}:`, error.message);
        this.stats.errors++;
        break;
      }
    }
