  ...
  try {
    ...
    if (vueApp) {
      ...
      if (pinia && pinia._s) {
        ...
        const classesStore = Object.values(pinia._s).find(store => 
          store.$id === 'classes' || 
          store.classes || 
          store.fetchClasses
        );
    ...
    const classAnalysis = firestoreData.map((cls, index) => {
      const analysis = {
        ...
        isSharedBySharedWith: cls.sharedWith && Array.isArray(cls.sharedWith) && cls.sharedWith.length > 0,
      ...
      console.log(`   teachers: ${analysis.teachers ? JSON.stringify(analysis.teachers) : 'undefined'}`);
      console.log(`   sharedWith: ${analysis.sharedWith ? JSON.stringify(analysis.sharedWith) : 'undefined'}`);
    ...
    const sharedTab = document.querySelector('button:has([class*="ShareIcon"])') ||
    ...
    if (sharedTab) {
      ...
      setTimeout(() => {
        const sharedList = document.querySelector('[data-testid="shared-classes-list"]') ||
                          document.querySelector('.shared-classes-container');
        
        if (sharedList) {
          console.log('✅ Lista de clases compartidas visible');
          
          const items = sharedList.querySelectorAll('[data-testid="shared-class-item"], .shared-class-item');
          console.log(`   Items mostrados: ${items.length}`);
          
          if (items.length !== sharedByTeachers.length) {
            console.log(`⚠️  DISCREPANCIA: Se esperaban ${sharedByTeachers.length} items, se muestran ${items.length}`);
          }
        } else {
