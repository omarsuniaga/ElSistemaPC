...
async function generateAttendanceData() {
  ...
  for (const date of dates) {
    for (const classData of sampleClasses) {
      ...
      if (Math.random() < 0.6) {
        // 60% de probabilidad de que haya clase
        ...
        const attendanceDoc = {
          fecha: date,
          date, // Ambos campos para compatibilidad
          classId: classData.id,
          teacherId,
          uid: teacherId,
          ...
          data: {
            ...
            justificacion,
            observación:
              Math.random() < 0.3 ? `Observación para la clase ${classData.name} del ${date}` : '',
            observations: [],
          },
        };
        ...
        console.log(
          `📝 Documento generado: ${docId} - ${presentes.length} presentes, ${ausentes.length} ausentes, ${tarde.length} tarde`,
        );
      }
    }
  }
