...
async function testBrandingSystem() {
  ...
  try {
    ...
    addTest('Variables CSS aplicadas', !!primaryColor.trim(), primaryColor.trim() || 'No encontrada')
    ...
    addTest('Meta description presente', !!metaDesc, metaDesc?.content?.substring(0, 50) + '...' || 'No encontrada')
    ...
    if (typeof window.quickTestBranding === 'function') {
      try {
        ...
        addTest('Prueba rápida del sistema', quickResult, typeof quickResult === 'boolean' ? (quickResult ? 'PASS' : 'FAIL') : 'Resultado: ' + quickResult)
      } catch (error) {
    ...
    addTest('Ruta de branding accesible', isBrandingRoute || currentPath === '/', `Ruta actual: ${currentPath}`)
    ...
    addTest('Headers encontrados en DOM', hasHeaders, `${document.querySelectorAll('ion-header, [class*="header"]').length} elementos`)
    
    const hasIonElements = document.querySelectorAll('[class*="ion-"]').length > 0
    addTest('Elementos Ionic encontrados', hasIonElements, `${document.querySelectorAll('[class*="ion-"]').length} elementos`)
  ...
  const checks = [
    {
      ...
      check: () => typeof window.testBranding === 'function' && typeof window.quickTestBranding === 'function'
    },
    {
      name: 'Variables CSS aplicadas',
      check: () => !!getComputedStyle(document.documentElement).getPropertyValue('--ion-color-primary').trim()
    },
    {
