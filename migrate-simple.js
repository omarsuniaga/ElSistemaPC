...
if (missingVars.length > 0) {
  ...
  console.log('🔍 Variables disponibles:', Object.keys(process.env).filter(k => k.startsWith('VITE')));
  process.exit(1);
}
...
async function ejecutarMigracion() {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length >= header.length && values[1]) { // Verificar que tenga nombre
        const estudiante = {};
