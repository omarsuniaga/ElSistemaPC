...
async function debugEmergencyClass(classId = '3sf0mBLxcam45CbTgmvK') {
    ...
    try {
        ...
        try {
            const { getEmergencyClassByIdFirebase } = await import('./src/modulos/Attendance/service/emergencyClass.ts');
...
function checkCurrentContext() {
    ...
    console.log('   - Pinia stores disponibles:', window.$pinia?._s ? Object.keys(window.$pinia._s) : 'No disponible');
